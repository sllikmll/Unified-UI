...
(() => {
  ...
  function buildMihomoPrereleaseInstallCommand(tag, installMeta, coreLabel) {
    ...
    lines.push(
      ...
      '  fi',
      'fi',
      `  ${buildShellPrintfLine('[Unified UI] Останавливаем xkeen...')}`,
      'xkeen -stop',
      '__xk_stop_rc="$?"',
      'if [ "$__xk_stop_rc" -ne 0 ]; then',
      '  printf \'%s\\n\' "[Unified UI] xkeen -stop завершился с кодом $__xk_stop_rc, продолжаем замену бинарника."',
      'fi',
      ...
      '  exit 1',
      'fi',
      `  ${buildShellPrintfLine('[Unified UI] Запускаем xkeen...')}`,
      'xkeen -start',
      '__xk_start_rc="$?"',
      'if [ "$__xk_start_rc" -ne 0 ]; then',
      '  printf \'%s\\n\' "[Unified UI] xkeen -start завершился с кодом $__xk_start_rc."',
      '  if [ -n "$__xk_backup" ] && [ -f "$__xk_backup" ]; then',

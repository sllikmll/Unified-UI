...
def test_self_update_runner_keeps_stable_shell_pid_in_lock_and_status():
    ...
    assert 'export UNIFIED_UI_UPDATE_RUNNER_PID="$RUNNER_PID"' in runner_text
    assert 'os.environ.get("UNIFIED_UI_UPDATE_RUNNER_PID")' in runner_text
...
def test_self_update_runner_has_actionable_busybox_tar_backup_failure_and_skip_flag():
    ...
    assert 'SKIP_BACKUP="${UNIFIED_UI_UPDATE_SKIP_BACKUP:-0}"' in runner_text

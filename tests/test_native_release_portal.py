...
def test_native_release_manifest_distinguishes_windows_setup_and_standalone(tmp_path: Path):
    files = [
        'Unified-UI-Native-2.6.8-mac-arm64.zip',
        'Unified-UI-Native-Setup-2.6.8-x64.exe',
        'Unified-UI-Native-2.6.8-x64.exe',
        'Unified-UI-Native-2.6.8-windows-x64-portable.zip',
        'Unified-UI-Native-2.6.8-linux-x64-portable.tar.gz',
        'Unified-UI-Native-2.6.8-linux-x64.deb',
        'Unified-UI-Native-2.6.8-linux-x64.rpm',
    ]
    for name in files:
    ...
    assert by_key['win_setup_x64']['file'] == 'Unified-UI-Native-Setup-2.6.8-x64.exe'
    assert by_key['win_standalone_x64']['file'] == 'Unified-UI-Native-2.6.8-x64.exe'
...
def test_native_release_manifest_fails_when_required_artifacts_are_missing(tmp_path: Path):
    touch(tmp_path / 'Unified-UI-Native-2.6.8-mac-arm64.zip')
    result = subprocess.run(
...
def test_native_download_portal_and_npm_scripts_are_wired():
    ...
    assert 'Unified-UI-Native-2.6.8-linux-x64.deb' in html

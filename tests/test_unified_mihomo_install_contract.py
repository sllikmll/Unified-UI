...
def test_main_installer_runs_unified_mihomo_installer_by_default():
    ...
    assert 'UNIFIED_INSTALL_MIHOMO:-1' in text
    assert 'sh "$UI_DIR/scripts/install_mihomo_core.sh"' in text
    assert 'UNIFIED_INSTALL_MIHOMO=0' in text


def test_mihomo_core_installer_uses_user_fork_with_upstream_release_fallback():
    text = Path("unified-ui/scripts/install_mihomo_core.sh").read_text(encoding="utf-8")

    assert 'MIHOMO_REPO="${UNIFIED_MIHOMO_REPO:-sllikmll/mihomo}"' in text
    assert 'MIHOMO_FALLBACK_REPO="${UNIFIED_MIHOMO_FALLBACK_REPO:-MetaCubeX/mihomo}"' in text

...
def test_devtools_env_defaults_shell_to_zero_and_marks_live_apply():
    ...
    shell_block = env_py.split('if k == "UNIFIED_ALLOW_SHELL":', 1)[1].split("\n", 2)
    assert 'return "0"' in "\n".join(shell_block[:2])

    assert "ENV_HELP.UNIFIED_ALLOW_SHELL = 'Arbitrary shell" in env_js
    assert "ENV_NO_RESTART_KEYS.add('UNIFIED_ALLOW_SHELL');" in env_js
    assert "ENV_RESTART_KEYS.delete('UNIFIED_ALLOW_SHELL');" in env_js


def test_terminal_runtime_tracks_shell_policy_and_renders_disabled_notice():
    runtime_text = Path("unified-ui/static/js/terminal/runtime.js").read_text(encoding="utf-8")
    core_text = Path("unified-ui/static/js/terminal/_core.js").read_text(encoding="utf-8")
    caps_text = Path("unified-ui/static/js/terminal/capabilities.js").read_text(encoding="utf-8")
    controller_text = Path("unified-ui/static/js/terminal/modules/terminal_controller.js").read_text(encoding="utf-8")
    command_job_text = Path("unified-ui/static/js/util/command_job.js").read_text(encoding="utf-8")
    ...
    assert "UNIFIED_ALLOW_SHELL" in controller_text
...
def test_commands_route_uses_dynamic_shell_policy_payload():
    text = Path("unified-ui/routes/commands.py").read_text(encoding="utf-8")

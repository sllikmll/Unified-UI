...
def test_windows_nsis_script_is_unicode_and_has_no_mojibake(tmp_path: Path):
    app_exe = tmp_path / "Unified-UI-Native-2.6.8-x64.exe"
    app_exe.write_bytes(b"MZ\0fake")
    out_exe = tmp_path / "Unified-UI-Native-Setup-2.6.8-x64.exe"

    script = nsis.installer_script(version="2.6.8", app_dir=tmp_path, app_exe=app_exe, out_exe=out_exe)

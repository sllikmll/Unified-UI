...
Container должен видеть TUN/сетевые capabilities, нужные userspace AWG. На проверенном RB5009/RouterOS 7.23.3 `/dev/net/tun` и нужная capability доступны автоматически. `devices=/dev/net/tun` использовать нельзя: RouterOS отвергает path как invalid `container-dev-name`. Если runtime недоступен, startup пропускает native AWG restore и продолжает запуск UI/Mihomo; ручной AWG Apply падает с явной ошибкой preflight. Нужны veth gateway и masquerade/маршрут для container subnet.

...
- `Routing Xray` уже читает реальные backend-документы и остаётся первым глубоким editor-like модулем: без переноса строк, с виртуализированными номерами, инерционным/быстрым скроллингом, системным выделением, историей правок и переходом к строке.

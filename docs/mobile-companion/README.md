...
- самый глубокий модуль сейчас это `Routing Xray`: он уже читает реальные routing-документы и даёт полноэкранный editor-flow с нативными Android-жестами и быстрым скроллингом больших файлов;
...
Главный технический разрыв сейчас не в Android UI, persistence, auth/session или safe service controls, а в backend validation/write contract для Routing Xray и transport для streams.
Базовый controller использует dependency seam через `ConnectionsPort`, `SessionPort`, `ServiceActionsPort`, `RoutingWritePort` и `LogsPort`; первые три уже имеют production implementations, routing writes и logs будут заменяться по следующим этапам.
...
- `Routing Xray` уже читает реальные backend-документы и остаётся первым глубоким editor-like модулем: с мягким переносом длинных строк, компактным виртуализированным gutter, инерционным/быстрым скроллингом, системным выделением, историей правок и переходом к строке.
...
- Локальная валидация routing draft, полноэкранный редактор больших файлов, переключение между документами и подсветка JSON/JSONC видимой области.
...
- `Routing Xray validate`, `save` и `apply` пока не доходят до целевых backend write/validation flow.

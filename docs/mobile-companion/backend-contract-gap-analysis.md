...
| Service control | `GET /api/unified/status`, `GET /api/unified/core`, `GET /api/cores/status`, `GET /api/cores/versions`, `GET /api/cores/updates`, `POST /api/unified/start`, `POST /api/unified/stop`, `POST /api/unified/core`, `POST /api/restart`, `POST /api/restart-unified` | Quick actions уже используются Android-клиентом через единый port и server reread | Сохранить compatibility adapter; позже обернуть в агрегированный ready summary/action contract с operation semantics |

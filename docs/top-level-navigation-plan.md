...
- direct URL entry и hard reload сохранены для `/`, `/backups`, `/devtools`, `/unified` и `/mihomo_generator`;
- общий top-level shell/router, screen registry и shared host contract работают для всех пяти экранов;
- `backups` и `unified` доведены до того же keep-alive screen lifecycle, что уже был у `panel`, `devtools` и `mihomo_generator`;
...
- `P7` — `backups.html` и `unified.html` выровнены под shared top-level host contract.
- `P8` — `/backups` и `/unified` переведены на top-level screen bootstrap и общий screen host.
- `P9` — добран page-specific lifecycle/state retention для `backups` и `unified`.
...
- `backups` и `unified` сохраняют локальное runtime-состояние достаточно, чтобы возврат на экран не вёл к лишней переинициализации, duplicate polling или сбросу базового view/editor state.
...
- inventory-документация и `docs/frontend-page-inventory.json` синхронизированы с текущим source graph, где `/backups` и `/unified` уже входят в тот же top-level shell/runtime path;
...
- keep-alive screen lifecycle и screen registry распространяются на `/backups` и `/unified` так же, как на остальные top-level экраны;

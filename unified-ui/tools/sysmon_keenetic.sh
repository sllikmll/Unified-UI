#!/bin/sh
# unified-ui: System monitor for Keenetic/Entware-like routers (busybox-friendly)
#
# Usage (inside router):
#   sh /opt/etc/unified-ui/tools/sysmon_keenetic.sh
# Options:
#   --short     меньше вывода
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --json) AS_JSON=1;;
    -h|--help)
      echo "sysmon: sh /opt/etc/unified-ui/tools/sysmon_keenetic.sh [--short|--full] [--no-color] [--json]";
      exit 0
      ;;
  esac
  shift
...
get_banner_accessed_ip() {
  ...
  for raw in "${UNIFIED_UI_ACCESSED_IP:-}" "${UNIFIED_UI_HOST_IP:-}" "${HTTP_HOST:-}"; do
...
# unified-ui server (best-effort: run_server.py)
check_proc_contains "unified-ui" "unified-ui" || true
check_proc_contains "run_server.py" "unified-ui" || true
...
for xray_conf in /opt/etc/xray/config.json /opt/etc/unified/unified.conf; do
  if [ -f "$xray_conf" ]; then

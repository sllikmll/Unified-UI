...
[ -n "$UI_DIR" ] || UI_DIR="/opt/etc/unified-ui"

CACHE_DIR="${UNIFIED_UI_CACHE_DIR:-/opt/var/cache/unified-ui}"
CACHE_SCHEMA="${UNIFIED_UI_CACHE_NS:-v2}"
CACHE_TTL=43200
...
build_info_candidates() {
  printf '%s\n' \
    "$UI_DIR/BUILD.json" \
    "/opt/etc/unified-ui/BUILD.json" \
    "/opt/share/www/custom/unified-ui/BUILD.json"
}

panel_metadata_candidates() {
  printf '%s\n' \
    "$UI_DIR/package.json" \
    "/opt/etc/unified-ui/package.json" \
    "/opt/share/www/custom/unified-ui/package.json" \
    "$UI_DIR/version.txt" \
    "/opt/etc/unified-ui/version.txt" \
    "/opt/share/www/custom/unified-ui/version.txt"
}
...
# Получить версию UnifiedUI Beta из test/README.md в репозитории
get_unified_beta_version() {
  repo="$1"
  ...
  if [ -n "$readme" ]; then
    # Парсим "## UnifiedUI 2.0 Beta" → "2.0 Beta"
...
get_local_unified_version() {
  line=""
  display=""

  if has_cmd unified; then
    line=$(unified -v 2>/dev/null | head -n 1)
    [ -z "$line" ] && line=$(unified --version 2>/dev/null | head -n 1)
  fi

  if [ -z "$line" ] && [ -x /opt/sbin/unified ]; then
    line=$(/opt/sbin/unified -v 2>/dev/null | head -n 1)
    [ -z "$line" ] && line=$(/opt/sbin/unified --version 2>/dev/null | head -n 1)
  fi
  ...
  if [ -r /opt/sbin/unified ] && head -n 1 /opt/sbin/unified 2>/dev/null | grep -q '^#!'; then
    display=$(sed -n "s/^[[:space:]]*UNIFIED_VERSION=[\"']\([^\"']*\)[\"'].*/\1/p" /opt/sbin/unified 2>/dev/null | head -n 1)
    display=$(trim_text "$display")
    [ -n "$display" ] && {
      printf '%s' "$display"
      return 0
...
get_local_unified_ui_repo() {
  if [ -n "$UNIFIED_UI_UPDATE_REPO" ]; then
    printf '%s' "$UNIFIED_UI_UPDATE_REPO"
    return 0
  fi
...
normalize_component() {
  ...
  case "$value" in
    ""|all)
      printf '%s' ""
      ;;
    ui|panel|panel-ui|unified_ui|unifiedui)
      printf '%s' "unified-ui"
      ;;
    packages|package|pkg|pkgs|opkg)
      printf '%s' "entware"
      ;;
    unified|unified-ui|xray|mihomo|system|entware)
      printf '%s' "$value"
      ;;
    *)
      printf '%s' "$value"
...
needs_remote_versions() {
  ...
  case "$selected" in
    ""|unified|unified-ui|xray|mihomo)
      return 0
      ;;
    *)
      return 1
...
show_versions_section() {
  ...
  show_versions=0
  for item in system unified unified-ui xray mihomo; do
  ...
  if want_component "$selected" "unified"; then
    local_v=$(get_local_unified_version)
    remote_v=""
    beta_v=""
    if [ "$offline" != "1" ]; then
      remote_v=$(get_github_latest "jameszeroX/UnifiedUI")
      beta_v=$(get_unified_beta_version "jameszeroX/UnifiedUI")
    fi
    ...
    print_component "unified" "$local_v" "$show_v"
  fi

  if want_component "$selected" "unified-ui"; then
    local_v=$(get_local_unified_ui_version)
    remote_v=""
    panel_repo=$(get_local_unified_ui_repo)
    if [ "$offline" != "1" ] && [ -n "$panel_repo" ]; then
      remote_v=$(get_github_latest "$panel_repo")
    fi
    print_component "unified-ui" "$local_v" "$remote_v"
  fi
...
main() {
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        _out "  --component, -c ИМЯ       Один компонент: unified, unified-ui, xray, mihomo, system, entware"
  ...
  case "$selected" in
    ...
    *)
      ...
      _out "Доступно: unified, unified-ui, xray, mihomo, system, entware"
      return 1
      ;;
  esac

...
#   sh /opt/etc/unified-ui/tools/entware_backup.sh
# or via wrapper:
#   entware-backup
#
...
STATE_DIR="${UNIFIED_UI_STATE_DIR:-/opt/var/lib/unified-ui}"
STATE_FILE="$STATE_DIR/backup.state"
PROC_MOUNTS_FILE="${UNIFIED_PROC_MOUNTS_FILE:-/proc/mounts}"
...
backup_entware() {
  ...
  staging_dir="$selected_drive/.unified-entware-backup.$$"
  ...
  set -- "$@" "--exclude=$bn"
  set -- "$@" "--exclude=./$bn"
  set -- "$@" "--exclude=./.unified-entware-backup.*"

...
def _dbg(msg: str) -> None:
    ...
    Enable with env `UNIFIED_PATHS_DEBUG=1`.
    """

    if os.environ.get("UNIFIED_PATHS_DEBUG") in ("1", "true", "yes", "on"):
        try:
            print(f"[unified-ui][paths] {msg}", file=sys.stderr)
        except Exception:
            return
...
def get_ui_state_dir() -> str:
    ...
    Router default: /opt/etc/unified-ui
    Dev/macOS fallback: XDG/~/Library/Application Support/unified-ui (or ~/.config/unified-ui)

    Override with env:
    - UNIFIED_UI_STATE_DIR (preferred)
    - UNIFIED_UI_DIR (legacy)
    """
    env_dir = os.environ.get("UNIFIED_UI_STATE_DIR") or os.environ.get("UNIFIED_UI_DIR")
    if env_dir:
        return env_dir

    default_dir = "/opt/etc/unified-ui"
    ...
    if xdg:
        fallback = os.path.join(xdg, "unified-ui")
    elif sys.platform == "darwin":
        fallback = os.path.join(home, "Library", "Application Support", "unified-ui")
    else:
        fallback = os.path.join(home, ".config", "unified-ui")

    try:
        os.makedirs(fallback, exist_ok=True)
    except Exception as e:  # noqa: BLE001
        # Last resort: current working directory
        _dbg(f"fallback state dir create failed: {fallback} ({e}); using ./unified-ui-state")
        fallback = os.path.abspath("./unified-ui-state")
        os.makedirs(fallback, exist_ok=True)
    return fallback

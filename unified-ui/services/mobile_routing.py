...
saved revision, writes the live fragment and restarts unified.  Content hashes are opaque revision
...
class MobileRoutingService:
    def __init__(
        ...
        *,
        ...
        self.restart_unified = restart_unified
    ...
    def apply(
        ...
        *,
        ...
        with _WRITE_LOCK:
            ...
            restarted = False
            try:
                restarted = bool(self.restart_unified(source="mobile-routing-apply"))
            except Exception:
                restarted = False
            if not restarted:
                self._restore_paths(originals)
                try:
                    self.restart_unified(source="mobile-routing-rollback")
                except Exception:
                    pass
                raise MobileRoutingOperationFailure(

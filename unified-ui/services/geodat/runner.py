...
def _geodat_bin_path() -> str:
    return (os.getenv('UNIFIED_GEODAT_BIN', '') or '').strip() or '/opt/etc/unified-ui/bin/xk-geodat'


def _geodat_timeout_s() -> int:
    raw = (os.getenv('UNIFIED_GEODAT_TIMEOUT', '') or '').strip()
    try:
        v = int(float(raw))
    except Exception:
        v = 25
...
def _geodat_cache_ttl_s() -> int:
    raw = (os.getenv('UNIFIED_GEODAT_CACHE_TTL', '') or '').strip()
    try:
        v = int(float(raw))
    except Exception:
        v = 60

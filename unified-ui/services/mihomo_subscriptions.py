...
Mihomo cannot consume UnifiedUI/Xray JSON subscriptions directly as
...
def _refresh_config_subscription(
    ...
    *,
    ...
    restart_unified: RestartCallback | None,
    restart: bool,
    ...
    try:
        ...
        if changed:
            ...
            if restart and restart_unified is not None:
                try:
                    restart_unified(source="mihomo-subscription-refresh")
                except TypeError:
                    restart_unified("mihomo-subscription-refresh")
        ...
        return {
            ...
            "id": sub.get("id"),
            "changed": bool(changed),
            "restarted": bool(changed and restart and restart_unified is not None),
            "count": len(new_names),
            "skipped": skipped,
...
def start_subscription_scheduler(
    ...
    *,
    ...
    if not env_flag("UNIFIED_MIHOMO_SUBSCRIPTIONS_SCHEDULER", True):
        return False
    ...
    try:
        tick = int(os.environ.get("UNIFIED_MIHOMO_SUBSCRIPTIONS_SCHEDULER_TICK", "60") or "60")
    except Exception:
        tick = 60
    ...
    thread = threading.Thread(target=_loop, name="unified-mihomo-subscriptions", daemon=True)
    thread.start()
    return True

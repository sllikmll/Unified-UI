"""Read/write helper for unified *.lst lists (ports / excludes).
...
PORT_PROXYING_FILE = _env_or_default_file(
    ...
    os.path.join(BASE_ETC_DIR, "unified", "port_proxying.lst"),
)

PORT_EXCLUDE_FILE = _env_or_default_file(
    "UNIFIED_PORT_EXCLUDE_FILE",
    os.path.join(BASE_ETC_DIR, "unified", "port_exclude.lst"),
)

# ip_exclude.lst historically lived in two possible locations:
#   - /opt/etc/unified/ip_exclude.lst (new)
#   - /opt/etc/unified_exclude.lst (legacy; v1.1.3.8)
# We support explicit override via UNIFIED_IP_EXCLUDE_FILE and also auto-fallback
# to the legacy path if the new file doesn't exist.
_ip_exclude_default = os.path.join(BASE_ETC_DIR, "unified", "ip_exclude.lst")
_ip_exclude_legacy = os.path.join(BASE_ETC_DIR, "unified_exclude.lst")
IP_EXCLUDE_FILE = _env_or_default_file("UNIFIED_IP_EXCLUDE_FILE", _ip_exclude_default)
try:
    if "UNIFIED_IP_EXCLUDE_FILE" not in os.environ:
...
# unified.json (newer unified versions)
UNIFIED_CONFIG_FILE = _env_or_default_file(
    "UNIFIED_CONFIG_FILE",
    os.path.join(BASE_ETC_DIR, "unified", "unified.json"),
)
...
_KIND_TO_PATH = {
    ...
    KIND_CONFIG: UNIFIED_CONFIG_FILE,
}
...
_KIND_TO_DEFAULT = {
    ...
    KIND_CONFIG: DEFAULT_UNIFIED_CONFIG,
}

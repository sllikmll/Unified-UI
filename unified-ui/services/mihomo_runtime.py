...
def _mh_default_root() -> Path:
    ...
    if xdg:
        base = Path(xdg) / "unified-ui"
    elif sys.platform == "darwin":
        base = Path(home) / "Library" / "Application Support" / "unified-ui"
    else:
        base = Path(home) / ".config" / "unified-ui"
...
def _default_mihomo_restart_cmd() -> str:
    ...
    return shlex.join(build_unified_cmd("-restart"))

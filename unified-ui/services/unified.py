"""Service-level helpers for unified (restart, logs, etc.)."""
...
from services.unified_commands_catalog import build_unified_cmd, resolve_unified_init_script
...
def detect_unified_runtime_pids(core_name: str) -> tuple[str, ...]:
...
def detect_unified_runtime_core() -> str:
    """Return the currently running unified-managed core name, if detected."""
    for core_name in ("xray", "mihomo"):
        if detect_unified_runtime_pids(core_name):
            return core_name
    return ""


def is_unified_running() -> bool:
    """Return True when unified-managed core process is currently running."""
    return bool(detect_unified_runtime_core())


def get_unified_runtime_status() -> dict[str, object]:
    """Return compact runtime status fields for restart-log metadata."""
    core = detect_unified_runtime_core()
    running = bool(core)
    pids = detect_unified_runtime_pids(core) if core else ()
    return {
...
def build_unified_control_cmds(
    action: str,
    *,
    ...
    plain ``unified -start`` / ``unified -restart`` CLI command returned without
    ...
    script = resolve_unified_init_script()
    init_cmd = [script, normalized] if script else None
    cli_cmd = build_unified_cmd(f"-{normalized}")
...
def _dispatch_unified_control_command(
    cmd: Sequence[str],
    *,
...
def _wait_unified_running(expected_running: bool, *, timeout: float, poll_interval: float = 0.25) -> bool:
    deadline = time.monotonic() + max(0.2, float(timeout or 0))
    last_state = is_unified_running()
...
def control_unified_action(
    ...
    *,
    ...
    """Execute unified service-control action with runtime verification.
    ...
    before_core = detect_unified_runtime_core()
    before_pids = detect_unified_runtime_pids(before_core) if before_core else ()
    ...
    def _restart_verified() -> bool:
        ...
        if not core:
            return False
        after_pids = detect_unified_runtime_pids(core)
        if not before_pids:
    ...
    for cmd in build_unified_control_cmds(
        normalized,
        primary_cmd=primary_cmd,
        prefer_init=prefer_init,
    ):
        if not _dispatch_unified_control_command(cmd, dispatch_timeout=dispatch_timeout):
            continue
        if normalized == "restart":
            if _wait_restart_verified(timeout=settle):
                return True
            continue
        if _wait_unified_running(expected_running, timeout=settle):
            return True
...
def restart_unified(
    ...
    """Restart unified and log the result.
    ...
    append_restart_log(log_file, ok, source=source, duration_ms=duration_ms, **get_unified_runtime_status())
    return ok

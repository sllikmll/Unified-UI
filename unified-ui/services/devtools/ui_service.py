...
UI_INIT_OWNER_MARKER = 'UNIFIED_UI_INIT_OWNER="sllikmll/Unified-UI"'
UI_PID_FILE = os.environ.get("UNIFIED_UI_PID_FILE", "/opt/var/run/unified-ui.pid")
# Optional: custom command to control UI in dev (e.g. launchctl/systemd). Supports "{action}" placeholder.
UI_CONTROL_CMD = os.environ.get("UNIFIED_UI_CONTROL_CMD") or os.environ.get("UNIFIED_UI_RESTART_CMD")


def _runtime_mode() -> str:
    rt_env = (os.environ.get("UNIFIED_RUNTIME") or os.environ.get("UNIFIED_ENV") or "").strip().lower()
...
def _is_our_ui_init_script(path: str) -> bool:
    try:
        ...
        if UI_INIT_OWNER_MARKER in content:
            return True
        if 'UI_DIR="/opt/etc/unified-ui"' not in content:
            return False
...
def _resolve_ui_init_script() -> str | None:
    override = str(os.environ.get("UNIFIED_UI_INIT_SCRIPT", "") or "").strip()

...
def _ui_state_dir() -> str:
    ...
    return (
        (os.environ.get("UNIFIED_UI_STATE_DIR") or os.environ.get("UNIFIED_UI_DIR") or "/opt/etc/unified-ui").strip()
        or "/opt/etc/unified-ui"
    )


def _restart_log_path() -> str:
    return (os.environ.get("UNIFIED_RESTART_LOG_FILE") or os.path.join(_ui_state_dir(), "restart.log")).strip()


def _update_log_path() -> str:
    update_dir = (os.environ.get("UNIFIED_UI_UPDATE_DIR") or "").strip()
    if update_dir:
    ...
    try:
        ...
        base_var = "/opt/var"
    return os.path.join(base_var, "lib", "unified-ui", "update", "update.log")


def _log_dir() -> str:
    # Keep in sync with services/logging_setup.py and app.py
    return (os.environ.get("UNIFIED_LOG_DIR") or "/opt/var/log/unified-ui").strip() or "/opt/var/log/unified-ui"
...
def _log_targets() -> Mapping[str, str]:
    ...
    try:
        ...
        return {
            ...
            "unified-ui": "/opt/var/log/unified-ui.log",
        }
    except Exception:
        d = _log_dir()
        return {
...
def list_logs() -> List[Dict[str, Any]]:
    ...
    """
    targets = dict(_log_targets())
    order = ["core", "access", "ws", "stdout", "stderr", "update", "restart", "unified-ui"]

"""Xray subscription management for Unified UI.
...
def subscription_state_path(ui_state_dir: str) -> str:
    return os.path.join(str(ui_state_dir or "/opt/etc/unified-ui"), STATE_FILENAME)
...
def _refresh_error_retry_seconds(interval_hours: Any) -> int:
    ...
    try:
        retry = int(float(str(os.environ.get("UNIFIED_SUBSCRIPTION_ERROR_RETRY_SECONDS", DEFAULT_ERROR_RETRY_SECONDS))))
    except Exception:
...
def delete_subscription(
    ...
    *,
    ...
    restart_unified: RestartCallback | None = None,
) -> Dict[str, Any]:
    ...
    restarted = False
    if restart_unified and (output_removed or observatory_changed or routing_changed or outbounds_changed):
        try:
            restarted = bool(restart_unified(source="xray-subscription-delete"))
        except TypeError:
            restarted = bool(restart_unified())
        except Exception:
            restarted = False
...
def _subscription_policy() -> URLPolicy:
    return URLPolicy(
        allow_hosts=(),
        allow_http=env_flag("UNIFIED_SUBSCRIPTION_ALLOW_HTTP", False),
        allow_private_hosts=env_flag("UNIFIED_SUBSCRIPTION_ALLOW_PRIVATE_HOSTS", False),
...
def _fetch_subscription_body_once(url: str, request_headers: Dict[str, str] | None = None) -> Tuple[str, Dict[str, str]]:
    ...
    try:
        timeout = max(3, min(120, int(os.environ.get("UNIFIED_SUBSCRIPTION_FETCH_TIMEOUT", str(timeout)))))
    except Exception:
        timeout = DEFAULT_FETCH_TIMEOUT_SECONDS

    max_bytes = DEFAULT_MAX_BODY_BYTES
    try:
        max_bytes = max(64 * 1024, int(os.environ.get("UNIFIED_SUBSCRIPTION_MAX_BYTES", str(max_bytes))))
    except Exception:
        max_bytes = DEFAULT_MAX_BODY_BYTES

    opener = urllib.request.build_opener(_SafeRedirect(policy))
    headers = {"User-Agent": "Unified-UI Subscription Fetcher"}
...
def _subscription_html_landing_message(body: str, headers: Dict[str, str] | None = None) -> str:
    ...
    if helper_error == "happ_decryptor_not_configured":
        helper_hint = (
            ...
            "или положите внешний decryptor в unified-ui/bin. "
...
def _happ_helper_error_message(reason: Any) -> str:
    ...
    if code == "happ_decryptor_not_configured":
        return (
            ...
            "или положите drop-in decryptor в unified-ui/bin. "
...
def _happ_subscription_headers(headers: Dict[str, str] | None) -> Dict[str, str] | None:
    if not env_flag("UNIFIED_SUBSCRIPTION_HAPP_FALLBACK", True):
        return None
    ua = ""
    ...
    if "happ" in ua.lower():
        return None
    fallback_ua = str(os.environ.get("UNIFIED_SUBSCRIPTION_HAPP_USER_AGENT") or "Happ/1.0").strip()
    if not fallback_ua:
        return None
    out = dict(headers or {})
...
def _is_generated_jsonc_comment(text: str) -> bool:
    raw = str(text or "")
    return "Generated by Unified UI subscription" in raw or "Generated by Unified UI subscriptions" in raw
...
def _sync_subscription_only_base_outbounds(
    *,
    ...
    raw_changed = _write_jsonc_sidecar_if_changed(
        path,
        next_obj,
        header="// Generated by Unified UI subscriptions (subscription-only base outbounds)",
        snapshot=snapshot,
...
def sync_subscription_routing(
    *,
    ...
    raw_changed = _write_jsonc_sidecar_if_changed(
        routing_path,
        cfg,
        header="// Generated by Unified UI subscriptions (routing sync)",
        snapshot=snapshot,
        preserve_existing_comments=True,
    )
...
def sync_observatory_subjects(
    *,
    ...
    observatory_header = (
        "// Generated by Unified UI subscriptions (observatory subjects)"
...
def refresh_subscription(
    ...
    *,
    ...
    try:
        ...
        raw_path = jsonc_path_for(output_path)
        raw_text = (
            f"// Generated by Unified UI subscription: {sub.get('name') or sub.get('id')}\n"
        ...
        restarted = False
        if restart and restart_unified and (changed or observatory_changed or routing_changed or outbounds_changed):
            try:
                restarted = bool(restart_unified(source="xray-subscription-refresh"))
            except TypeError:
                restarted = bool(restart_unified())
            except Exception:
                restarted = False
...
def _probe_candidate_urls(primary_url: str) -> List[str]:
    ...
    for value in [primary_url, *DEFAULT_PROBE_FALLBACK_URLS]:
        ...
        if text and text not in urls:
            urls.append(text)
    extra_raw = os.environ.get("UNIFIED_PROBE_FALLBACK_URLS", "")
...
def _probe_request_attempts() -> int:
    try:
        raw = int(os.environ.get("UNIFIED_PROBE_REQUEST_ATTEMPTS", str(PROBE_REQUEST_ATTEMPTS)) or PROBE_REQUEST_ATTEMPTS)
    except Exception:
...
def _probe_once_via_local_proxy(port: int, probe_url: str, timeout_value: float) -> tuple[int | None, str]:
    ...
    try:
        ...
        req = urllib.request.Request(
            probe_url,
            headers={"User-Agent": "Unified-UI Latency Probe"},
            method="GET",
        )
...
def _probe_outbounds_batch(
    *,
    ...
    for _attempt_idx in range(max_attempts):
        ...
        with tempfile.TemporaryDirectory(prefix="unified-xray-probe-batch-") as tmpdir:
...
def start_subscription_scheduler(
    ...
    *,
    ...
    if not env_flag("UNIFIED_SUBSCRIPTIONS_SCHEDULER", True):
        return False
    ...
    try:
        tick = int(os.environ.get("UNIFIED_SUBSCRIPTIONS_SCHEDULER_TICK", "60") or "60")
    except Exception:
        tick = 60
    ...
    thread = threading.Thread(target=_loop, name="unified-xray-subscriptions", daemon=True)
    thread.start()
    return True

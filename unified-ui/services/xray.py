"""Service helpers for restarting the Xray core without restarting unified-ui.
...
(:func:`services.unified.control_unified_action`). That path prefers the init.d
...
bare ``unified -start``. On several routers that CLI start silently no-ops (unified's
supervisor state is out of sync after an external kill), so the core stayed down
and the user had to press "Restart" manually. Restarting through unified only
...
from services.unified import control_unified_action
...
def restart_xray_core(
    *,
    ...
    Strategy:
      ...
      2) Otherwise delegate to :func:`services.unified.control_unified_action` with a

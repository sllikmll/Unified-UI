"""Helpers for detecting and switching unified cores (xray / mihomo).
...
from services.unified_commands_catalog import build_unified_cmd
...
def get_cores_status() -> Tuple[List[str], Optional[str]]:
    """Return (available_cores, current_core) for unified.
...
def switch_core(core: str, error_log_path: str, runtime_log: Callable[[str], None] | None = None) -> None:
    """Switch unified core to `core` ('xray' or 'mihomo') and restart service.

    This replicates behaviour of the previous api_unified_core_set implementation:
    - determine current core;
    - when switching xray -> mihomo, clear error log;
    - run `unified -mihomo` or `unified -xray`;
    - run `unified -start`;
    ...
    timeout_switch = max(5, _env_int("UNIFIED_CORE_SWITCH_TIMEOUT", 25))
    timeout_start = max(5, _env_int("UNIFIED_CORE_START_TIMEOUT", 20))

    # Open log file handle for unified commands
    log_handle = None
    try:
        if log_file:
            try:
        ...
        def run_cmd(cmd, *, phase: str, timeout: int) -> None:
            ...
            _core_log("info", "unified.cmd_start", phase=phase, cmd=cmd_s, timeout=timeout)
            _write_diag(f"[unified-ui] {phase}: start cmd={cmd_s} timeout={timeout}s")
            t0 = time.monotonic()
            try:
                ...
                _core_log("error", "unified.cmd_timeout", phase=phase, cmd=cmd_s, elapsed_s=dt)
                _write_diag(f"[unified-ui] {phase}: TIMEOUT after {dt}s cmd={cmd_s}")
                ...
                dt = round(time.monotonic() - t0, 3)
                _write_command_output(getattr(exc, "output", ""), phase=phase, ok=False)
                _core_log("error", "unified.cmd_failed", phase=phase, cmd=cmd_s, rc=getattr(exc, "returncode", None), elapsed_s=dt)
                _write_diag(
                    f"[unified-ui] {phase}: FAILED rc={getattr(exc,'returncode',None)} after {dt}s cmd={cmd_s}"
                )
                raise CoreSwitchError(
                ...
                dt = round(time.monotonic() - t0, 3)
                _core_log("error", "unified.cmd_exception", phase=phase, cmd=cmd_s, elapsed_s=dt, error=str(exc))
                _write_diag(f"[unified-ui] {phase}: EXCEPTION after {dt}s cmd={cmd_s} err={exc}")
                raise CoreSwitchError(
            ...
            dt = round(time.monotonic() - t0, 3)
            _core_log("info", "unified.cmd_ok", phase=phase, cmd=cmd_s, elapsed_s=dt)
            _write_diag(f"[unified-ui] {phase}: ok elapsed={dt}s cmd={cmd_s}")
        ...
        def run_start_cmd(cmd, *, timeout: int) -> None:
            ...
            with tempfile.TemporaryFile() as output_file:
                ...
                while True:
                    ...
                    if detect_running_core() == core:
                        started = True
                        grace_s = max(0.0, _env_int("UNIFIED_CORE_START_GRACE_AFTER_RUNNING_MS", 1500) / 1000.0)
            ...
            if started:
                ...
                _core_log("info", "unified.cmd_ok", phase=phase, cmd=cmd_s, elapsed_s=dt, runtime_core=core)
                _write_diag(f"[unified-ui] {phase}: ok elapsed={dt}s cmd={cmd_s}")
                return
            ...
            if rc == 0:
                _core_log("error", "unified.cmd_not_ready", phase=phase, cmd=cmd_s, rc=rc, elapsed_s=dt)
                _write_diag(f"[unified-ui] {phase}: NOT_READY rc={rc} after {dt}s cmd={cmd_s}")
            else:
                _core_log("error", "unified.cmd_failed", phase=phase, cmd=cmd_s, rc=rc, elapsed_s=dt)
                _write_diag(f"[unified-ui] {phase}: FAILED rc={rc} after {dt}s cmd={cmd_s}")
            raise CoreSwitchError(
                "Команда завершилась с ошибкой",
        ...
        try:
            if core == "mihomo":
                run_cmd(["unified", "-mihomo"], phase="switch_core", timeout=timeout_switch)
            else:
                run_cmd(["unified", "-xray"], phase="switch_core", timeout=timeout_switch)
            ...
            if core == "xray":
                try:
                    ...
                    test_timeout = max(5, _env_int("UNIFIED_XRAY_TEST_TIMEOUT", _default_test_timeout))
                    cmd = [xray_bin, "-test", "-confdir", confdir]
                    cmd_s = " ".join(str(x) for x in cmd)
                    _core_log("info", "unified.cmd_start", phase="xray_test", cmd=cmd_s, timeout=test_timeout)
                    _write_diag(f"[unified-ui] xray_test: start cmd={cmd_s} timeout={test_timeout}s")
                    t0t = time.monotonic()
                    completed = subprocess.run(
                        cmd,
                        stdout=subprocess.PIPE,
                    ...
                    _core_log("info", "unified.cmd_ok", phase="xray_test", cmd=cmd_s, elapsed_s=dtt)
                    _write_diag(f"[unified-ui] xray_test: ok elapsed={dtt}s cmd={cmd_s}")
                except FileNotFoundError:
                    ...
                    _core_log("error", "unified.cmd_timeout", phase="xray_test", cmd=cmd_s if 'cmd_s' in locals() else 'xray -test', elapsed_s=dtt)
                    _write_diag(f"[unified-ui] xray_test: TIMEOUT after {dtt}s cmd={cmd_s}")
                    raise CoreSwitchError(
                    ...
                    dtt = round(time.monotonic() - t0t, 3) if 't0t' in locals() else None
                    _write_command_output(getattr(exc, "output", ""), phase="xray_test", ok=False)
                    _core_log("error", "unified.cmd_failed", phase="xray_test", cmd=cmd_s if 'cmd_s' in locals() else 'xray -test', rc=getattr(exc, 'returncode', None), elapsed_s=dtt)
                    _write_diag(f"[unified-ui] xray_test: FAILED rc={getattr(exc,'returncode',None)} after {dtt}s cmd={cmd_s}")
                    raise CoreSwitchError(
            ...
            run_start_cmd(build_unified_cmd("-start"), timeout=timeout_start)

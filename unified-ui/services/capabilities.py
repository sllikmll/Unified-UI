...
- WebSocket runtime is detected via env var UNIFIED_WS_RUNTIME=1
...
from services.unified_commands_catalog import get_full_shell_policy
...
def detect_remotefs_state(
    ...
    *,
    ...
    enabled = supported and _env_bool(env, "UNIFIED_REMOTEFM_ENABLE", True)
...
def _detect_runtime_mode(env: Dict[str, str]) -> str:
    # Can be overridden by env: UNIFIED_RUNTIME=router|dev
    rt_env = (env.get("UNIFIED_RUNTIME") or env.get("UNIFIED_ENV") or "").strip().lower()
...
def detect_terminal_state(
    ...
    *,
    ...
    ws_ok = bool(_env_bool(env, "UNIFIED_WS_RUNTIME", False) if ws_runtime is None else ws_runtime)
    machine_arch = _detect_machine_arch()
    pty_ok = bool(
        ws_ok
...
def detect_capabilities(
    ...
    *,
    ...
    ws_runtime = _env_bool(env, "UNIFIED_WS_RUNTIME", False)
    ...
    default_log_dir = os.path.join(str(BASE_VAR_DIR), "log", "unified-ui")
    ...
    restart_log_file = env.get("UNIFIED_RESTART_LOG_FILE") or os.path.join(UI_STATE_DIR, "restart.log")
    ...
    remote = {
        ...
        "limits": {
            "max_sessions": _env_int(env, "UNIFIED_REMOTEFM_MAX_SESSIONS", 6),
            "session_ttl_seconds": _env_int(env, "UNIFIED_REMOTEFM_SESSION_TTL", 900),
            "max_upload_mb": _env_int(env, "UNIFIED_REMOTEFM_MAX_UPLOAD_MB", 200),
        },
        "fileops": {
            "enabled": bool(enabled),
            "ws": bool(ws_runtime and enabled),
            "workers": _env_int(env, "UNIFIED_FILEOPS_WORKERS", 1),
            "max_jobs": _env_int(env, "UNIFIED_FILEOPS_MAX_JOBS", 100),
            "job_ttl_seconds": _env_int(env, "UNIFIED_FILEOPS_JOB_TTL", 3600),
            "ops": ["copy", "move", "delete"],
            "remote_to_remote": bool(enabled),
            "remote_to_remote_direct": _env_bool(env, "UNIFIED_FILEOPS_REMOTE2REMOTE_DIRECT", True),
            "fxp": _env_bool(env, "UNIFIED_FILEOPS_FXP", True),
            "spool_max_mb": _env_int(
                env,
                "UNIFIED_FILEOPS_SPOOL_MAX_MB",
                _env_int(env, "UNIFIED_REMOTEFM_MAX_UPLOAD_MB", 200),
            ),

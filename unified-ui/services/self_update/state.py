...
def get_update_dir(ui_state_dir: str) -> str:
    ...
    env_dir = os.environ.get("UNIFIED_UI_UPDATE_DIR")
    if env_dir:
        return env_dir
    ...
    return os.path.join(base_var, "lib", "unified-ui", "update")
...
def get_backup_dir(ui_state_dir: str) -> str:
    ...
    env_dir = os.environ.get("UNIFIED_UI_BACKUP_DIR")
    if env_dir:
        return env_dir
    ...
    return os.path.join(base_var, "backups", "unified-ui")
...
def list_backups(backup_dir: str, *, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        paths = glob.glob(os.path.join(backup_dir, "unified-ui-*.tgz"))
    except Exception:
        return []

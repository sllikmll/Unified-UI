...
def _cfg_github_api_base() -> str:
    base = os.environ.get("UNIFIED_UI_GITHUB_API_BASE", "https://api.github.com") or "https://api.github.com"
    return str(base).rstrip("/")


def _cfg_user_agent() -> str:
    ua = os.environ.get("UNIFIED_UI_HTTP_USER_AGENT", "unified-ui") or "unified-ui"
    return str(ua)


def _cfg_github_web_base() -> str:
    base = (os.environ.get("UNIFIED_UI_GITHUB_WEB_BASE") or "").strip()
    if base:
        return str(base).rstrip("/")
...
def _cfg_api_timeout() -> float:
    ...
    try:
        return max(3.0, float(os.environ.get("UNIFIED_UI_UPDATE_API_TIMEOUT", "10") or 10))
    except Exception:
        return 10.0
...
def _cfg_rel_ttl() -> int:
    ...
    try:
        return max(5, int(os.environ.get("UNIFIED_UI_UPDATE_CHECK_CACHE_TTL", "60") or 60))
    except Exception:
        return 60
...
def _cfg_max_body_chars() -> int:
    ...
    try:
        return max(256, int(os.environ.get("UNIFIED_UI_UPDATE_RELEASE_NOTES_MAX_CHARS", "4096") or 4096))
    except Exception:
        return 4096
...
def _cfg_prefer_asset_name() -> str:
    ...
    """
    v = (os.environ.get("UNIFIED_UI_UPDATE_ASSET_NAME", "") or "").strip()
    return v or "unified-ui-routing.tar.gz"


def _release_asset_candidates() -> List[str]:
    names: List[str] = []
    for nm in (_cfg_prefer_asset_name(), "unified-ui-routing.tar.gz", "unified-ui.tar.gz"):
        s = str(nm or "").strip()
...
def _sanitize_release(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Respect UNIFIED_UI_UPDATE_ASSET_NAME (runner also uses it), but keep backward
...
def _fetch_latest_main(repo: str, branch: Optional[str] = None) -> Dict[str, Any]:
    ...
    br = (branch or os.environ.get('UNIFIED_UI_UPDATE_BRANCH') or '').strip() or None
    default_branch = None
    try:
...
def github_get_latest_main(
    ...
    *,
    ...
    """
    repo = (repo or '').strip()
    bkey = (branch or os.environ.get('UNIFIED_UI_UPDATE_BRANCH') or 'default').strip() or 'default'
    key = f"{repo}#{bkey}"

...
This module runs `unified` (or optional shell commands) in a background thread and
...
def _build_exec_env(*, term: str | None = None) -> dict:
    ...
    - /opt/bin tools are preferred (opkg, python/pip, unified, etc.)
...
from services.unified_commands_catalog import (
    build_unified_cmd,
    SHELL_BIN,
    COMMAND_TIMEOUT,
)
...
def _restart_log_file() -> str:
    try:
        path = (os.environ.get("UNIFIED_RESTART_LOG_FILE") or "").strip()
    except Exception:
        path = ""
    return path or "/opt/etc/unified-ui/restart.log"
...
def _run_command_job(job_id: str, stdin_data: str | None) -> None:
    """Run unified or shell command in background and store result in JOBS."""
    with JOBS_LOCK:
        job = JOBS.get(job_id)
        if not job:
            return
    ...
    if job.cmd:
        ...
        cmd = build_unified_cmd(job.flag)
    else:
        with JOBS_LOCK:
            job = JOBS.get(job_id)
            if not job:
    ...
    COMMAND_MAX_OUTPUT_CHARS = int(os.environ.get("UNIFIED_COMMAND_MAX_OUTPUT_CHARS", "1048576"))  # 1 MiB

...
from services.unified_lists import PORT_PROXYING_FILE, PORT_EXCLUDE_FILE, IP_EXCLUDE_FILE, UNIFIED_CONFIG_FILE
...
def build_user_configs_bundle(
    *,
    ...
    # *.lst from /opt/etc/unified
    lst_files = {
        "unified/port_proxying.lst": PORT_PROXYING_FILE,
        "unified/port_exclude.lst": PORT_EXCLUDE_FILE,
        "unified/ip_exclude.lst": IP_EXCLUDE_FILE,
        "unified/unified.json": UNIFIED_CONFIG_FILE,
    }
...
def apply_user_configs_bundle(bundle: Dict[str, Any]) -> None:
    ...
    for item in files:
        ...
        # unified.json (lives in /opt/etc/unified)
        if basename == os.path.basename(UNIFIED_CONFIG_FILE):
            real_path = UNIFIED_CONFIG_FILE
        # JSON configs

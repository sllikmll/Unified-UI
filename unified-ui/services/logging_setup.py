...
- UNIFIED_LOG_DIR: directory for all UI logs (default: /opt/var/log/unified-ui)
- UNIFIED_LOG_CORE_ENABLE: 0/1 (default: 1)
- UNIFIED_LOG_CORE_LEVEL: ERROR|WARNING|INFO|DEBUG (default: INFO)
- UNIFIED_LOG_ACCESS_ENABLE: 0/1 (default: 0)
- UNIFIED_LOG_WS_ENABLE: 0/1 (default: 0)
- UNIFIED_LOG_ROTATE_MAX_MB: max size in MB for each log file before rotation (default: 2)
- UNIFIED_LOG_ROTATE_BACKUPS: number of rotated files to keep (default: 3)
...
DEFAULT_LOG_DIR = "/opt/var/log/unified-ui"
...
def get_log_dir(default_dir: Optional[str] = None) -> str:
    ...
    p = (os.environ.get("UNIFIED_LOG_DIR") or "").strip()
    if p:
        return p
...
def _mk_rotating_handler(path: str) -> RotatingFileHandler:
    max_mb = max(1, _env_int("UNIFIED_LOG_ROTATE_MAX_MB", DEFAULT_ROTATE_MAX_MB))
    backups = max(1, _env_int("UNIFIED_LOG_ROTATE_BACKUPS", DEFAULT_ROTATE_BACKUPS))
...
def setup_logging(log_dir: Optional[str] = None) -> None:
    ...
    # Core logger
    core = logging.getLogger("unifiedui")
    core.propagate = False
    core.addHandler(handlers["core"])

    # Access logger
    access = logging.getLogger("unifiedui.access")
    access.propagate = False
    access.addHandler(handlers["access"])

    # WS logger
    ws = logging.getLogger("unifiedui.ws")
    ws.propagate = False
...
def core_enabled() -> bool:
    return _env_bool("UNIFIED_LOG_CORE_ENABLE", default=True)
...
def refresh_runtime_from_env() -> None:
    ...
    if not core_enabled():
        ...
        try:
            ...
            pass
        lvl_name = os.environ.get("UNIFIED_LOG_CORE_LEVEL", DEFAULT_CORE_LEVEL)
        lvl = _parse_level(lvl_name)
        core.setLevel(lvl)

    # Access/ws are always DEBUG internally; the enable flag decides whether we log.
    logging.getLogger("unifiedui.access").setLevel(logging.INFO)
    logging.getLogger("unifiedui.ws").setLevel(logging.DEBUG)


def access_enabled() -> bool:
    return _env_bool("UNIFIED_LOG_ACCESS_ENABLE", default=False)


def ws_enabled() -> bool:
    return _env_bool("UNIFIED_LOG_WS_ENABLE", default=False)
...
def core_logger() -> logging.Logger:
    return logging.getLogger("unifiedui")


def access_logger() -> logging.Logger:
    return logging.getLogger("unifiedui.access")


def ws_logger() -> logging.Logger:
    return logging.getLogger("unifiedui.ws")

...
def _local_allowed_roots() -> List[str]:
    ...
    env = (os.getenv('UNIFIED_LOCALFM_ROOTS', '') or '').strip()
    if env:
...
#
# Set UNIFIED_PROTECT_MNT_LABELS=0 to disable this protection.
_PROTECT_MNT_LABELS = str(os.getenv('UNIFIED_PROTECT_MNT_LABELS', '1') or '1').strip().lower() not in ('0', 'false', 'no', 'off')
_PROTECTED_MNT_ROOT = str(os.getenv('UNIFIED_PROTECTED_MNT_ROOT', '/tmp/mnt') or '/tmp/mnt').strip() or '/tmp/mnt'
...
_TRASH_DIR = str(os.getenv('UNIFIED_TRASH_DIR', '/opt/var/trash') or '/opt/var/trash').strip() or '/opt/var/trash'
_TRASH_META_DIRNAME = '.unified_trashinfo'
...
#   UNIFIED_TRASH_MAX_BYTES / UNIFIED_TRASH_MAX_GB
#   UNIFIED_TRASH_TTL_DAYS
#   UNIFIED_TRASH_WARN_RATIO          (e.g. 0.9)
#   UNIFIED_TRASH_STATS_CACHE_SECONDS (e.g. 10)
#   UNIFIED_TRASH_PURGE_INTERVAL_SECONDS (e.g. 3600)
...
def _trash_cfg() -> Dict[str, Any]:
    ...
    try:
        b = str(os.getenv('UNIFIED_TRASH_MAX_BYTES', '') or '').strip()
        if b:
    ...
    if max_bytes is None:
        try:
            gb = str(os.getenv('UNIFIED_TRASH_MAX_GB', '') or '').strip()
            if gb:
    ...
    ttl_days = _read_int_env('UNIFIED_TRASH_TTL_DAYS', _TRASH_TTL_DAYS_DEFAULT)
    if ttl_days < 0:
        ttl_days = 0

    warn_ratio = _read_float_env('UNIFIED_TRASH_WARN_RATIO', _TRASH_WARN_RATIO_DEFAULT)
    if warn_ratio <= 0 or warn_ratio > 1.0:
        warn_ratio = float(_TRASH_WARN_RATIO_DEFAULT)

    stats_cache_s = _read_int_env('UNIFIED_TRASH_STATS_CACHE_SECONDS', _TRASH_STATS_CACHE_SECONDS_DEFAULT)
    if stats_cache_s < 0:
        stats_cache_s = _TRASH_STATS_CACHE_SECONDS_DEFAULT

    purge_interval_s = _read_int_env('UNIFIED_TRASH_PURGE_INTERVAL_SECONDS', _TRASH_PURGE_INTERVAL_SECONDS_DEFAULT)

...
_TEMPLATE_HEADER_RE = re.compile(r"^\s*//\s*unified-template:\s*(\{.*\})\s*$")
_SAFE_TEMPLATE_NAME_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$")


def _parse_template_header(text: str) -> Dict[str, str]:
    """Parse unified-template header from JSONC template text."""
    if not text:
        return {}
    try:
...
def _strip_existing_template_header(content: str) -> str:
    """Remove existing unified-template header at the very top to avoid duplicates."""
...
def _seed_routing_templates_once(routing_templates_dir: str) -> None:
    ...
    marker = os.path.join(routing_templates_dir, ".unified_seeded")
    try:
    ...
    try:
        # This file lives in unified-ui/services/routing/templates.py
...
def _paths_for_routing(
    ...
    """
    raw_override = os.environ.get("UNIFIED_XRAY_ROUTING_FILE_RAW", "")
...
def _compose_template_text(title: str, desc: str, content: str) -> str:
    ...
    header = "// unified-template: " + json.dumps(meta_obj, ensure_ascii=False)
    return header + "\n" + body.lstrip("\n")

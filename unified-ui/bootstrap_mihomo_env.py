...
def _dbg(msg: str) -> None:
    ...
    Enable with env `UNIFIED_BOOT_DEBUG=1`.
    """
    if os.environ.get("UNIFIED_BOOT_DEBUG") in ("1", "true", "yes", "on"):
        try:
            print(f"[unified-ui][boot] {msg}", file=sys.stderr)
        except Exception:
            return
...
def ensure_mihomo_root_env() -> None:
    ...
    if os.path.isdir(bundled):
        ...
        if xdg:
            base = os.path.join(xdg, "unified-ui")
        elif sys.platform == "darwin":
            base = os.path.join(home, "Library", "Application Support", "unified-ui")
        else:
            base = os.path.join(home, ".config", "unified-ui")

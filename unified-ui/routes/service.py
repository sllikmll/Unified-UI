"""Service-control API routes for unified as a Flask Blueprint."""
...
from services.unified import control_unified_action, get_unified_runtime_status
...
def create_service_blueprint(
    ...
    """Create blueprint with unified service-control endpoints."""
    ...
    def _restart_log_runtime_meta(started_at=None) -> dict[str, object]:
        ...
        try:
            meta.update(get_unified_runtime_status())
        except Exception:
            pass
    ...
    @bp.post("/api/unified/start")
    def api_unified_start() -> Any:
        started_at = time.monotonic()
        try:
            ok = control_unified_action("start", prefer_init=True)
            _append_restart_log(ok, source="api-start", **_restart_log_runtime_meta(started_at))
            if ok:
                _core_log("info", "unified.start", source="api-start")
                return jsonify({"ok": True}), 200
            _core_log("error", "unified.start_failed", source="api-start")
    ...
    @bp.post("/api/unified/stop")
    def api_unified_stop() -> Any:
        started_at = time.monotonic()
        try:
            ok = control_unified_action("stop", prefer_init=True)
            _append_restart_log(ok, source="api-stop", **_restart_log_runtime_meta(started_at))
            if ok:
                _core_log("info", "unified.stop", source="api-stop")
                return jsonify({"ok": True}), 200
            _core_log("error", "unified.stop_failed", source="api-stop")
            return jsonify({"ok": False}), 500
        except Exception:
    ...
    @bp.get("/api/unified/status")
    def api_unified_status() -> Any:
        """
        Возвращает статус работы сервиса unified и информацию
        ...
        try:
            ...
            _core_log("error", "unified.status_failed", error=str(e))
            return _service_exception(
                "Не удалось получить статус unified.",
                code="status_failed",
                hint="Подробности смотрите в server logs.",
                exc=e,
            )
    ...
    @bp.get("/api/unified/core")
    def api_unified_core_get() -> Any:
        """Список доступных ядер и текущее ядро unified (через сервисный модуль)."""
        try:
            ...
            return _service_exception(
                "Не удалось получить список ядер unified.",
    ...
    @bp.post("/api/unified/core")
    def api_unified_core_set() -> Any:
        """Смена ядра unified через сервисный модуль (switch_core)."""
        core = ""
        started_at = time.monotonic()
        previous_core = ""
        ...
        try:
            ...
            try:
                ...
                _core_log("error", "unified.core_set_failed", core=core, error=str(e), **(e.details or {}))
                return _service_exception(
                    "Не удалось переключить ядро unified.",
                ...
                _core_log("error", "unified.core_set_failed", core=core, error=str(e))
                return _service_exception(
                    "Не удалось переключить ядро unified.",
                    code="core_switch_failed",
                    hint="Подробности смотрите в server logs.",
            ...
            _core_log("info", "unified.core_set", core=core)
    ...
    # ---------- API: restart unified ----------
    @bp.post("/api/restart-unified")
    def api_restart_unified() -> Any:
        restarted = restart_unified(source="manual-mihomo")
        _core_log("info", "unified.restart", source="manual-mihomo", restarted=bool(restarted))

        # Сообщаем подписчикам, что произошёл перезапуск unified.
        _emit_event({"event": "unified_restarted", "ok": bool(restarted)})
    ...
    @bp.post("/api/restart")
    def api_restart() -> Any:
        """Restart unified (legacy endpoint).

        This endpoint historically returned HTTP 500 on failure.
        """
        ok = restart_unified(source="api-button")

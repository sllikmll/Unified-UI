...
def _detect_panel_core_ui() -> dict[str, object]:
    ...
    requested_sections = _parse_sections_whitelist(os.environ.get("UNIFIED_UI_PANEL_SECTIONS_WHITELIST"))
    effective_sections = (
        supported_sections
    ...
    *,
    ...
    def index():
        ...
        page_ctx = {
            ...
            "unified_runtime_debug": str(os.environ.get("UNIFIED_DEV", "")).strip().lower() in {"1", "true", "yes", "on"},
            "unified_terminal_enable_optional_addons": str(os.environ.get("UNIFIED_ENABLE_XTERM_OPTIONAL_ADDONS", "")).strip().lower() in {"1", "true", "yes", "on"},
    ...
    def unified_page():
        try:
            return _no_cache(make_response(render_template("unified.html")))
        except Exception:

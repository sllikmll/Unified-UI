...
def create_fileops_blueprint(
    *,
    ...
    FILEOPS_SPOOL_DIR = os.getenv("UNIFIED_FILEOPS_SPOOL_DIR", os.path.join(tmp_dir, "unified_fileops_spool"))
    FILEOPS_SPOOL_MAX_MB = _read_int_env("UNIFIED_FILEOPS_SPOOL_MAX_MB", int(max_upload_mb))
    ...
    FILEOPS_SPOOL_CLEANUP_AGE_SECONDS = _read_int_env("UNIFIED_FILEOPS_SPOOL_CLEANUP_AGE", 21600)
    ...
    FILEOPS_REMOTE2REMOTE_DIRECT = _env_bool("UNIFIED_FILEOPS_REMOTE2REMOTE_DIRECT", True)
    FILEOPS_FXP_ENABLED = _env_bool("UNIFIED_FILEOPS_FXP", True)
    ...
    try:
        ...
        try:
            _SPOOL_BASE = _local_resolve(os.path.join(tmp_dir, 'unified_fileops_spool'), LOCALFS_ROOTS)
        except Exception:
            _SPOOL_BASE = os.path.join(tmp_dir, 'unified_fileops_spool')
    ...
    jobmgr = FileOpJobManager(
        max_jobs=int(os.getenv('UNIFIED_FILEOPS_MAX_JOBS', '100') or '100'),
        ttl_seconds=int(os.getenv('UNIFIED_FILEOPS_JOB_TTL', '3600') or '3600'),
        workers=int(os.getenv('UNIFIED_FILEOPS_WORKERS', '1') or '1'),
    )

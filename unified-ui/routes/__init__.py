...
def register_blueprints(app, ctx: Optional[AppContext] = None):
    ...
    from .unified_lists import create_unified_lists_blueprint
    ...
    from .keenetic_dns_routes import create_keenetic_dns_routes_blueprint
    ...
    app.register_blueprint(create_unified_lists_blueprint(restart_unified=ctx.restart_unified))
    ...
    app.register_blueprint(
        create_routing_blueprint(
            ...
            restart_unified=ctx.restart_unified,
    ...
    app.register_blueprint(create_keenetic_dns_routes_blueprint())
    ...
    try:
        fs_bp = create_fs_blueprint(
            tmp_dir=str(os.getenv("UNIFIED_REMOTEFM_TMP_DIR", "/tmp") or "/tmp"),
            max_upload_mb=int(os.getenv("UNIFIED_REMOTEFM_MAX_UPLOAD_MB", "200") or "200"),
            xray_configs_dir=ctx.xray_configs_dir,
            backup_dir=ctx.backup_dir,
        )
    ...
    try:
        ...
        app.extensions["unified.capabilities"] = _caps
    except Exception as _e:  # noqa: BLE001
        _caps = None
    ...
    if _remote_enabled:
        try:
            ...
            remotefs_bp, remotefs_mgr = create_remotefs_blueprint(
                enabled=True,
                lftp_bin=_lftp_bin,
                max_sessions=int(os.getenv("UNIFIED_REMOTEFM_MAX_SESSIONS", "6")),
                ttl_seconds=int(os.getenv("UNIFIED_REMOTEFM_SESSION_TTL", "900")),
                max_upload_mb=int(os.getenv("UNIFIED_REMOTEFM_MAX_UPLOAD_MB", "200")),
                tmp_dir=str(os.getenv("UNIFIED_REMOTEFM_TMP_DIR", "/tmp") or "/tmp"),
                return_mgr=True,
            )
            app.extensions["unified.remotefs_mgr"] = remotefs_mgr

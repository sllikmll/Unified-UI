...
def _xray_test_timeout_seconds() -> int:
    default_timeout = 30
    raw = os.environ.get('UNIFIED_XRAY_TEST_TIMEOUT', str(default_timeout))
    try:
...
def _xray_asset_lookup_dir(dat_dir: str, asset_dir: str) -> str:
    ...
    for raw in (
        os.environ.get('UNIFIED_XRAY_PREFLIGHT_ASSET_DIR'),
...
def _run_xray_preflight(
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='unified-xray-test-') as tmpdir:
            ...
            try:
                ...
                if is_oom:
                    ...
                    hint = (
                        ...
                        'Попробуйте увеличить таймаут в DevTools → ENV через UNIFIED_XRAY_TEST_TIMEOUT.'
                    )
    ...
    *,
    ...
    def api_get_routing() -> Any:
        ...
        def _wrap(text: str, *, found: bool, using_raw: bool, notice: Optional[str] = None, kind: str = "info"):
            ...
            try:
                resp.headers["X-UnifiedUI-JSONC"] = "1" if found else "0"
                resp.headers["X-UnifiedUI-JSONC-Using"] = "1" if using_raw else "0"
                if found:
                    bn = ""
                    try:
                        if raw_exists:
                        ...
                        bn = ""
                    if bn:
                        resp.headers["X-UnifiedUI-JSONC-File"] = bn
            except Exception:
                pass
        ...
        if raw_for_read and main_exists:
            try:
                ...
                if main_mtime_ns > raw_mtime_ns:
                    ...
                    # or unified restart just touched the file.  Compare content
    ...
    def api_set_routing() -> Any:
        ...
        # Restart unified: legacy sync mode or async job mode (?async=1).
        ...
        restarted = restart_flag and restart_unified(source="routing")
        _core_log(
            "info",
            "routing.save",

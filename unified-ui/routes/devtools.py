...
def _tar_supports_exclude() -> bool:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="unified-tar-probe-") as tmp:
...
def create_devtools_blueprint(ui_state_dir: str) -> Blueprint:
    ...
    def _find_update_runner() -> str:
        ...
        cand = os.path.join(project_root, "scripts", "update_unified_ui.sh")
        if os.path.isfile(cand):
            return cand
        cand2 = "/opt/etc/unified-ui/scripts/update_unified_ui.sh"
    ...
    def api_devtools_update_info() -> Any:
        ...
        settings = {
            "repo": str(os.environ.get("UNIFIED_UI_UPDATE_REPO") or build.get("repo") or "sllikmll/Unified-UI"),
            "channel": str(os.environ.get("UNIFIED_UI_UPDATE_CHANNEL") or build.get("channel") or "stable"),
            "branch": str(os.environ.get("UNIFIED_UI_UPDATE_BRANCH") or "main"),
        }
    ...
    def api_devtools_update_check() -> Any:
        ...
        repo_env = os.environ.get("UNIFIED_UI_UPDATE_REPO")
        channel_env = os.environ.get("UNIFIED_UI_UPDATE_CHANNEL")
        branch_env = os.environ.get("UNIFIED_UI_UPDATE_BRANCH")
            ...
            if isinstance(latest, dict):
                if ch == "stable":
                    ...
                    if surl:
                        ...
                        if str(os.environ.get("UNIFIED_UI_UPDATE_REQUIRE_SHA") or "1").strip() == "1":
    ...
    def api_devtools_update_run() -> Any:
        ...
        # Acquire lock for this run (runner will adopt it using UNIFIED_UI_LOCK_PRECREATED=1).
        ...
        env = os.environ.copy()
        env["UNIFIED_UI_LOCK_PRECREATED"] = "1"
        env["UNIFIED_UI_UPDATE_DIR"] = paths["update_dir"]
        env["UNIFIED_UI_UPDATE_ACTION"] = "update"
        if skip_backup:
            env["UNIFIED_UI_UPDATE_SKIP_BACKUP"] = "1"

        # Pass effective update settings to runner (so it works without restarting UI).
        build_eff = get_build_info(ui_state_dir)
        repo_eff = str(os.environ.get("UNIFIED_UI_UPDATE_REPO") or build_eff.get("repo") or "sllikmll/Unified-UI")
        channel_eff = str(os.environ.get("UNIFIED_UI_UPDATE_CHANNEL") or build_eff.get("channel") or "stable")
        branch_eff = str(os.environ.get("UNIFIED_UI_UPDATE_BRANCH") or "main")
        env["UNIFIED_UI_UPDATE_REPO"] = repo_eff
        env["UNIFIED_UI_UPDATE_CHANNEL"] = channel_eff
        env["UNIFIED_UI_UPDATE_BRANCH"] = branch_eff
        ...
        if asset_url:
            env["UNIFIED_UI_UPDATE_ASSET_URL"] = asset_url
            tag = _s(resolved.get("tag"))
            if tag:
                env["UNIFIED_UI_UPDATE_TAG"] = tag
            sha_url = _s(resolved.get("sha_url"))
            if sha_url:
                env["UNIFIED_UI_UPDATE_SHA_URL"] = sha_url
            sha_kind = _s(resolved.get("sha_kind"))
            if sha_kind:
                env["UNIFIED_UI_UPDATE_SHA_KIND"] = sha_kind
            asset_name = _s(resolved.get("asset_name"))
            if asset_name:
                env["UNIFIED_UI_UPDATE_ASSET_NAME"] = asset_name
    ...
    def api_devtools_update_rollback() -> Any:
        ...
        env["UNIFIED_UI_LOCK_PRECREATED"] = "1"
        env["UNIFIED_UI_UPDATE_DIR"] = paths["update_dir"]
        env["UNIFIED_UI_UPDATE_ACTION"] = "rollback"

...
def _rule_provider_cache_dir() -> Path:
    raw = str(os.environ.get("UNIFIED_RULE_PROVIDER_CACHE_DIR") or "").strip()
    if raw:
        return Path(raw).expanduser().resolve()
    return Path(os.environ.get("BASE_VAR_DIR", "/opt/var")).expanduser().resolve() / "cache" / "unified-ui" / "rule-providers"
...
def _download_rule_provider_source(url: str, *, max_bytes: int = 16 * 1024 * 1024, timeout: float | None = None) -> bytes:
    ...
    if timeout is None:
        try:
            timeout = float(os.environ.get("UNIFIED_RULE_PROVIDER_DOWNLOAD_TIMEOUT") or "4")
        except Exception:
            timeout = 5.0
...
def _decode_mrs_provider(meta: dict[str, Any], resolved: str) -> tuple[str, dict[str, Any]]:
    ...
    with tempfile.TemporaryDirectory(prefix="unified-rp-") as td:

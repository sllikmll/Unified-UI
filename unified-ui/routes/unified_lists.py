...
PR13: moved /api/unified/* endpoints out of app.py.
...
from services.unified_lists import (
    KIND_IP_EXCLUDE,
    KIND_CONFIG,
    KIND_PORT_EXCLUDE,
    KIND_PORT_PROXYING,
...
def create_unified_lists_blueprint(restart_unified: Callable[..., Any]) -> Blueprint:
    """Create blueprint that serves /api/unified/*.lst endpoints.

    ``restart_unified`` must be a callable compatible with restart_unified(source="...") -> bool.
    """
    bp = Blueprint("unified_lists", __name__)
        ...
        if restart_flag and async_flag:
            try:
                ...
                return exception_response(
                    "Не удалось поставить перезапуск unified в очередь.",
                    500,
                    ok=False,
                    code="restart_schedule_failed",
                    hint="Повторите попытку позже. Подробности смотрите в server logs.",
                    exc=e,
                    log_tag="unified_lists.restart_schedule_failed",
                )

        restarted = restart_flag and restart_unified(source=source)
        return jsonify({"ok": True, "restarted": restarted}), 200

    # ----- port-proxying -----

    @bp.get("/api/unified/port-proxying")
    def api_get_port_proxying():
        content = get_list_content(KIND_PORT_PROXYING)
        return jsonify({"content": content}), 200

    @bp.post("/api/unified/port-proxying")
    ...
    # ----- unified config (unified.json) -----

    @bp.get("/api/unified/config")
    def api_get_unified_config():
        content = get_list_content(KIND_CONFIG)
        return jsonify({"content": content}), 200

    @bp.post("/api/unified/config")
    def api_set_unified_config():
        payload = _get_json_payload()
        content = payload.get("content", "")
        set_list_content(KIND_CONFIG, content)
        return _restart_response(payload, "unified-config")

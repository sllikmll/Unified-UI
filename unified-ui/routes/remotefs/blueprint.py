...
def _choose_state_dir(tmp_dir: str) -> str:
    ...
    env = (os.getenv("UNIFIED_REMOTEFM_STATE_DIR", "") or "").strip()
    candidates = []
    if env:
        candidates.append(env)
    candidates.append("/opt/var/lib/unified-ui/remotefs")
    candidates.append(os.path.join(tmp_dir or "/tmp", "unified-ui-remotefs"))
    last_err = None
    for c in candidates:
        try:
            ...
            last_err = e
            continue
    fallback = os.path.abspath("./unified-ui-remotefs")
    try:
...
def _detect_default_ca_bundle() -> str | None:
    ...
    env = (os.getenv("UNIFIED_REMOTEFM_CA_FILE", "") or "").strip()
...
def create_remotefs_blueprint(
    *,
    ...
    known_hosts_path = _ensure_known_hosts_file(
        (os.getenv("UNIFIED_REMOTEFM_KNOWN_HOSTS", "") or "").strip() or os.path.join(state_dir, "known_hosts")
    )

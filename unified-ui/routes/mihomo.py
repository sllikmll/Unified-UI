...
import unified_mihomo_service as mihomo_svc
...
def _subscription_landing_page_probe_error(result: Dict[str, Any], *, mode: str):
    ...
    hint = (
        "Для импорта в UnifiedUI нужен прямой URL, который отдает proxy-provider YAML, "
        "список URI-узлов или Xray JSON. Этот адрес похож на лендинг для Happ/INCY."
    )
    if "happ_decryptor_not_configured" in failure_text:
        hint += (
            " Настройте UNIFIED_HAPP_DECRYPTOR_CMD, положите внешний decryptor "
            "в unified-ui/bin или укажите UNIFIED_HAPP_DECRYPTOR_REMOTE_URL для "
            "осознанного HTTP fallback."
        )
    elif "happ_helper_not_configured" in failure_text:
        hint += " Настройте UNIFIED_HAPP_HELPER_CMD, чтобы панель могла обработать Happ landing page."
...
_MIHOMO_HWID_URL_POLICY_ENV_PREFIX = "UNIFIED_MIHOMO_HWID"
_MIHOMO_PROVIDER_URL_POLICY_ENV_PREFIX = "UNIFIED_MIHOMO_PROVIDER"
...
def _mihomo_ui_loopback_port_from_request() -> int:
    candidates = [
        os.environ.get("UNIFIED_UI_PORT"),
...
def create_mihomo_blueprint(
    *,
    ...
    def api_set_mihomo_config():
        ...
        restarted = restart_flag and restart_unified(source="mihomo-config")
    ...
    def api_mihomo_hwid_apply():
        ...
        If restart=true, schedules unified -restart via background job and returns 202.
        """
    ...
    def api_mihomo_generate_apply():
        """Generate+save mihomo config and restart unified via background job.

        Why background job?
          - unified restart may take ~60s on routers;
        ...
        try:
            ...
            # Schedule unified restart in background.
            job = create_command_job(flag="-restart", stdin_data=None, cmd=None, use_pty=True)
    ...
    def api_mihomo_restart_raw():
        """Save arbitrary YAML and restart mihomo (unified -restart)."""
        data = request.get_json(silent=True) or {}
        cfg = (data.get("config") or "").rstrip()
        if not cfg:
            return _api_error("config is required", 400, ok=False)
    ...
    def api_mihomo_parse_xray_json():
        ...
        Distinct error codes the frontend can branch on:
          ...
          - ``size_limit`` (413): subscription body exceeded ``UNIFIED_SUBSCRIPTION_MAX_BYTES``.
    ...
    _MIHOMO_UI_ALLOWED_PORTS_ENV = "UNIFIED_MIHOMO_UI_ALLOWED_PORTS"
    _MIHOMO_UI_PUBLIC_SCHEME_ENV = "UNIFIED_MIHOMO_UI_PUBLIC_SCHEME"
    _MIHOMO_UI_ALLOW_PROXY_FALLBACK_ENV = "UNIFIED_MIHOMO_UI_ALLOW_PROXY_FALLBACK"
    ...
    def api_mihomo_profiles_activate(name: str):
        "Activate given Mihomo profile and restart unified."
        try:
            _mh_activate_profile(name)
            restarted = restart_unified(source="mihomo-profile-activate")
    ...
    def api_mihomo_backup_restore(filename: str):
        try:
            ...
            restarted = restart_unified(source="mihomo-backup-restore")
            return jsonify({"ok": True, "restarted": restarted})
        except FileNotFoundError:
            return _api_error("backup not found", 404, ok=False)
        except Exception as e:

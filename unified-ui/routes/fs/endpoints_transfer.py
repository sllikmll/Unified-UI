    ...
    def api_fs_download() -> Any:
        ...
        if target == "local":
            ...
            if os.path.isdir(rp):
                ...
                zip_name = base + ".zip"
                tmp_zip = os.path.join(TMP_DIR, f"unified_zip_local_{uuid.uuid4().hex}.zip")
                try:
        ...
        if is_dir:
            ...
            zip_name = base + ".zip"
            tmp_root = os.path.join(TMP_DIR, f"unified_zip_remote_{sid}_{uuid.uuid4().hex}")
            tmp_dir = os.path.join(tmp_root, base)
            tmp_zip = os.path.join(TMP_DIR, f"unified_zip_remote_{sid}_{uuid.uuid4().hex}.zip")
            try:
                est_bytes, est_items, est_trunc = None, 0, False
                if MAX_ZIP_BYTES is not None or dry_run:
                ...
                if msg == "tmp_limit_exceeded":
                    return error_response(
                        ...
                        ok=False,
                        max_bytes=MAX_ZIP_BYTES,
                        message="Создание архива прервано: превышен лимит использования /tmp (см. UNIFIED_MAX_ZIP_MB).",
                    )
    ...
    def api_fs_upload() -> Any:
        ...
        try:
            tmp_path, total = save_filestorage_to_tmp(
                f,
                tmp_dir=TMP_DIR,
                prefix=f"unified_upload_{sid}_",
                max_bytes=max_bytes,
            )
        except ValueError as e:
            if str(e) == "too_large":

...
- Remote operations are available only when the RemoteFS manager is present
  in app.extensions as "unified.remotefs_mgr" and is enabled.
...
def create_fs_blueprint(
    *,
    ...
    #   - UNIFIED_MAX_ZIP_MB (preferred)
    #   - MAX_ZIP_MB (fallback)
    # 0 / empty disables the limit.

    MAX_ZIP_MB = _read_int_env('UNIFIED_MAX_ZIP_MB', _read_int_env('MAX_ZIP_MB', 0))
    ...
    MAX_ZIP_ESTIMATE_ITEMS = max(10_000, min(500_000, _read_int_env('UNIFIED_MAX_ZIP_ESTIMATE_ITEMS', 200_000)))
    ...
    mgr = LocalProxy(lambda: current_app.extensions.get("unified.remotefs_mgr"))
    ...
    def _require_enabled() -> Optional[Any]:
        ...
        if target != "remote":
            return None
        mgr_obj = current_app.extensions.get("unified.remotefs_mgr")

...
def create_backups_blueprint(
    ...
    def _extract_target_file_from_backup(data: Any) -> Optional[str]:
        ...
        if not isinstance(data, dict):
            return None
        meta = data.get("__unified_target_file__")
        if not meta:
            return None
        try:
    ...
    def api_xray_snapshots_restore() -> Any:
        ...
        restarted = restart_flag and restart_unified(source="snapshot-restore")
        _core_log(
            "info",
            "snapshot.restore",
    ...
    def api_create_backup() -> Any:
        ...
        # - new: {"__unified_raw_jsonc__": "<raw>", "__unified_data__": <obj?>}
        if raw_text is not None:
            payload: Any = {"__unified_raw_jsonc__": raw_text}
            if data is not None:
                payload["__unified_data__"] = data
        else:
            payload = data
    ...
    def api_create_backup_fragment() -> Any:
        ...
        Backup payload is a wrapper with metadata:
          - __unified_target_file__: basename of the fragment to restore into
          - __unified_raw_jsonc__: raw JSONC (if exists in JSONC_DIR)
          - __unified_data__: cleaned JSON object (if parsable)
        """
        ...
        payload: dict[str, Any] = {"__unified_target_file__": os.path.basename(main_json)}
        if raw_text is not None:
            payload["__unified_raw_jsonc__"] = raw_text
        if data is not None:
            payload["__unified_data__"] = data
    ...
    def api_restore_backup() -> Any:
        ...
        if isinstance(data, dict) and "__unified_raw_jsonc__" in data:
            raw = data.get("__unified_raw_jsonc__") or ""
            try:
                ...
                cleaned_obj = data.get("__unified_data__")
                if cleaned_obj is not None:
                    save_json(target_file, cleaned_obj)
        else:
            # If it's a wrapper payload, prefer __unified_data__.
            if isinstance(data, dict) and "__unified_data__" in data:
                save_json(target_file, data.get("__unified_data__"))
            else:
    ...
    def api_restore_auto_backup() -> Any:
        ...
        if target == "routing" and isinstance(data, dict) and "__unified_raw_jsonc__" in data:
            raw = data.get("__unified_raw_jsonc__") or ""
            try:
                ensure_xray_jsonc_dir()
    ...
    def api_restore_auto_backup_fragment() -> Any:
        ...
        if isinstance(data, dict) and "__unified_raw_jsonc__" in data:
            ...
            if isinstance(data, dict) and "__unified_data__" in data and "__unified_target_file__" in data:
                save_json(config_path, data.get("__unified_data__"))
            else:
                save_json(config_path, data)
    ...
    def restore_from_backups_page() -> Any:
        ...
        if path and os.path.isfile(path):
            ...
            if data is not None:
                ...
                restart_unified(source="backups-page")

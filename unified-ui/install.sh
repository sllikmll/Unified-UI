...
UI_DIR="/opt/etc/unified-ui"
SRC_DIR="$(cd "$(dirname "$0")" && pwd)"
INIT_DIR="/opt/etc/init.d"
INIT_SCRIPT_DEFAULT="$INIT_DIR/S99unified-ui001"
LEGACY_INIT_SCRIPT="$INIT_DIR/S99unified-ui"
INIT_SCRIPT="${UNIFIED_UI_INIT_SCRIPT:-$INIT_SCRIPT_DEFAULT}"
...
is_our_ui_init_script() {
  ...
  if grep -q 'UNIFIED_UI_INIT_OWNER="sllikmll/Unified-UI"' "$_path" 2>/dev/null; then
    return 0
  fi

  if grep -q 'UI_DIR="/opt/etc/unified-ui"' "$_path" 2>/dev/null; then
...
assert_safe_ui_init_target() {
  ...
  echo "    Используй UNIFIED_UI_INIT_SCRIPT с отдельным именем, если нужен кастомный путь."
  return 1
}
...
# (По умолчанию: /opt/etc/unified-ui/xray-jsonc)
JSONC_DIR_DEFAULT="$UI_DIR/xray-jsonc"

# Если это апгрейд и пользователь уже сохранял env overrides через DevTools,
# подтянем UNIFIED_XRAY_JSONC_DIR (только если не задано явно в окружении).
if [ -z "${UNIFIED_XRAY_JSONC_DIR:-}" ] && [ -f "$UI_DIR/devtools.env" ]; then
  # shellcheck disable=SC1090
  . "$UI_DIR/devtools.env" 2>/dev/null || true
fi

JSONC_DIR="${UNIFIED_XRAY_JSONC_DIR:-$JSONC_DIR_DEFAULT}"
if [ -z "$JSONC_DIR" ]; then
...
WANT_GEVENT=1
GEVENT_PIP_SPEC="${UNIFIED_GEVENT_PIP_SPEC:-gevent}"
GEVENT_PIN_REASON=""
case "$ARCH" in
  mipsel*|mips*)
  ...
  aarch64|arm64)
    ...
    if [ -z "${UNIFIED_GEVENT_PIP_SPEC:-}" ]; then
...
PY_WHEELHOUSE_DIR="${UNIFIED_PY_WHEELHOUSE_DIR:-$SRC_DIR/third_party/wheels}"
...
build_pip_index_candidates() {
  ...
  append_pip_index_candidate "${UNIFIED_PIP_INDEX_URL:-}"
  if python_ssl_available; then
    append_pip_index_candidate "$PIP_PRIMARY_INDEX_DEFAULT"
    append_pip_index_candidate "${UNIFIED_PIP_FALLBACK_INDEX_URL:-$PIP_FALLBACK_INDEX_DEFAULT}"
  else
    append_pip_index_candidate "${UNIFIED_PIP_HTTP_INDEX_URL:-$PIP_HTTP_FALLBACK_INDEX_DEFAULT}"
    append_pip_index_candidate "${UNIFIED_PIP_HTTP_EXTRA_INDEX_URL:-$PIP_HTTP_EXTRA_INDEX_DEFAULT}"
    append_pip_index_candidate "${UNIFIED_PIP_FALLBACK_INDEX_URL:-}"
  fi
}
...
build_pip_trusted_host_candidates() {
  ...
  if [ -n "${UNIFIED_PIP_TRUSTED_HOSTS:-}" ]; then
    for HOST in $UNIFIED_PIP_TRUSTED_HOSTS; do
...
pip_install_with_fallback() {
  ...
  if [ -d "$PY_WHEELHOUSE_DIR" ] && ls "$PY_WHEELHOUSE_DIR"/*.whl >/dev/null 2>&1; then
    ...
    PIP_LOG="${TMPDIR:-/tmp}/unified-pip-$PHASE-wheelhouse-$$.log"
  ...
  for INDEX_URL in $PIP_INDEX_CANDIDATES; do
    ...
    PIP_LOG="${TMPDIR:-/tmp}/unified-pip-$PHASE-$$.log"
    rm -f "$PIP_LOG" 2>/dev/null || true
    ...
    if "$PYTHON_BIN" -m pip install \
      ...
      --index-url "$INDEX_URL" \
      $PIP_TRUSTED_HOST_ARGS \
      --default-timeout "${UNIFIED_PIP_TIMEOUT:-60}" \
      "$@" >"$PIP_LOG" 2>&1; then
      cat "$PIP_LOG"
      rm -f "$PIP_LOG" 2>/dev/null || true
...
if [ "$NEED_FLASK" -eq 1 ] || [ "$NEED_GEVENT" -eq 1 ]; then
  ...
  if command -v opkg >/dev/null 2>&1; then
    ...
    echo "      export UNIFIED_PIP_INDEX_URL=${UNIFIED_PIP_INDEX_URL:-$PIP_FALLBACK_INDEX_DEFAULT}"
    echo "      export UNIFIED_GEVENT_PIP_SPEC=${UNIFIED_GEVENT_PIP_SPEC:-$GEVENT_PIP_SPEC}"
    if [ "$WANT_GEVENT" -eq 1 ]; then
      echo "      $PYTHON_BIN -m pip install --upgrade --index-url \"\$UNIFIED_PIP_INDEX_URL\" pip setuptools wheel"
      echo "      $PYTHON_BIN -m pip install --upgrade --index-url \"\$UNIFIED_PIP_INDEX_URL\" flask"
      echo "      $PYTHON_BIN -m pip install --upgrade --index-url \"\$UNIFIED_PIP_INDEX_URL\" \"\$UNIFIED_GEVENT_PIP_SPEC\" gevent-websocket"
    else
      echo "      $PYTHON_BIN -m pip install --upgrade --index-url \"\$UNIFIED_PIP_INDEX_URL\" pip setuptools wheel"
      echo "      $PYTHON_BIN -m pip install --upgrade --index-url \"\$UNIFIED_PIP_INDEX_URL\" flask"
    fi
  ...
  if [ "$NEED_FLASK" -eq 1 ]; then
    if ! pip_install_with_fallback "flask" flask; then
      ...
      echo "      UNIFIED_PIP_INDEX_URL=$PIP_FALLBACK_INDEX_DEFAULT sh install.sh"
      exit 1
    fi
  else
  ...
  if [ "$WANT_GEVENT" -eq 1 ]; then
    ...
    if ! pip_install_with_fallback "gevent" "$GEVENT_PIP_SPEC" gevent-websocket; then
      ...
      echo "      UNIFIED_PIP_INDEX_URL=$PIP_FALLBACK_INDEX_DEFAULT UNIFIED_GEVENT_PIP_SPEC=$GEVENT_PIP_SPEC $PYTHON_BIN -m pip install --upgrade \"\$UNIFIED_GEVENT_PIP_SPEC\" gevent-websocket"
    fi
  fi
fi
...
backup_config_file() {
  ...
  echo "[backup] $SRC -> $DEST" >> "$LOG_DIR/unified-ui.log"
}
...
migrate_legacy_jsonc_files() {
  ...
  # Создаём JSONC_DIR (может быть переопределён через UNIFIED_XRAY_JSONC_DIR)
  ...
  echo "[install] JSONC миграция: moved=$MOVED archived=$ARCHIVED skipped=$SKIPPED jsonc_dir=$JSONC_DIR" >> "$LOG_DIR/unified-ui.log"
...
patterns = [
    r'UNIFIED_UI_PORT["\']\)\s*or\s*["\']([0-9]{2,5})["\']',
    r'UNIFIED_UI_PORT["\']\)\s*or\s*([0-9]{2,5})',
    r'"0\.0\.0\.0",\s*([0-9]{2,5})',
...
if [ -z "$EXISTING_PORT" ] && [ -n "${UNIFIED_UI_PORT:-}" ]; then
  EXISTING_PORT="$UNIFIED_UI_PORT"
fi

if [ -z "$EXISTING_PORT" ] && [ -f "$EXISTING_ENV_FILE" ]; then
  EXISTING_PORT="$(extract_env_numeric_field "UNIFIED_UI_PORT" "$EXISTING_ENV_FILE")"
fi
...
if [ -n "$EXISTING_PORT" ]; then
  ...
  if is_port_in_use "$PANEL_PORT"; then
    OUR_PANEL=0
    PID_FILE="$RUN_DIR/unified-ui.pid"
  ...
  if [ "$USE_EXISTING" -eq 1 ]; then
    ...
    echo "[install] Текущий порт панели: $PANEL_PORT" >> "$LOG_DIR/unified-ui.log"
  fi
fi
...
echo "[*] Сохраняю порт панели в $EXISTING_ENV_FILE (UNIFIED_UI_PORT=$PANEL_PORT)..."
write_env_numeric_field "$EXISTING_ENV_FILE" "UNIFIED_UI_PORT" "$PANEL_PORT"
...
#   UNIFIED_UI_UPDATE_REPO, UNIFIED_UI_UPDATE_CHANNEL, UNIFIED_UI_VERSION, UNIFIED_UI_COMMIT
...
BUILD_REPO="${UNIFIED_UI_UPDATE_REPO:-${OLD_REPO:-sllikmll/Unified-UI}}"
BUILD_CHANNEL="${UNIFIED_UI_UPDATE_CHANNEL:-${OLD_CHANNEL:-stable}}"
BUILD_VERSION="${UNIFIED_UI_VERSION:-$OLD_VERSION}"
BUILD_COMMIT="${UNIFIED_UI_COMMIT:-$OLD_COMMIT}"
...
if [ -f "$SYS_MON_SRC" ]; then
  ...
  cat > "$SYS_MON_BIN" <<'EOF'
#!/bin/sh
# sysmon — UnifiedUI router monitor
SCRIPT="/opt/etc/unified-ui/tools/sysmon_keenetic.sh"
if [ ! -f "$SCRIPT" ]; then
...
#!/bin/sh
SCRIPT="/opt/etc/unified-ui/tools/entware_backup.sh"
if [ ! -f "$SCRIPT" ]; then
...
#!/bin/sh
SCRIPT="/opt/etc/unified-ui/tools/storage_dashboard.sh"
if [ ! -f "$SCRIPT" ]; then
...
#!/bin/sh
SCRIPT="/opt/etc/unified-ui/tools/device_lock_detector.sh"
if [ ! -f "$SCRIPT" ]; then
...
#!/bin/sh
SCRIPT="/opt/etc/unified-ui/tools/memory_check.sh"
if [ ! -f "$SCRIPT" ]; then
...
#!/bin/sh
SCRIPT="/opt/etc/unified-ui/tools/version_check.sh"
if [ ! -f "$SCRIPT" ]; then
...
#!/bin/sh
SCRIPT="/opt/etc/unified-ui/tools/backup_monitor.sh"
if [ ! -f "$SCRIPT" ]; then
...
cleanup_legacy_xray_templates() {
  # Некоторые версии unified/xray могут подхватывать *.jsonc из /opt/etc/xray (recursive scan)
  ...
  for f in \
    ...
    "$LEGACY_ROOT/routing/.unified_seeded" \
    "$LEGACY_ROOT/observatory/07_observatory_base.jsonc" \
    "$LEGACY_ROOT/observatory/.unified_seeded" \
    ; do
...
# Отключить можно явно: UNIFIED_INSTALL_MIHOMO=0 sh install.sh
if [ "${UNIFIED_INSTALL_MIHOMO:-1}" != "0" ]; then
  ...
  echo "[*] UNIFIED_INSTALL_MIHOMO=0 — пропускаю установку Mihomo core."
fi
...
if [ "${UNIFIED_INSTALL_PROXY_CLIENTS:-1}" != "0" ]; then
  ...
  echo "[*] UNIFIED_INSTALL_PROXY_CLIENTS=0 — optional proxy-клиенты пропущены."
fi
...
UNIFIED_UI_INIT_OWNER="sllikmll/Unified-UI"
ENABLED=yes
UI_DIR="/opt/etc/unified-ui"
PYTHON_BIN="/opt/bin/python3"
RUN_SERVER="$UI_DIR/run_server.py"
APP_PY="$UI_DIR/app.py"
PANEL_PORT="__UNIFIED_UI_PORT__"

LOG_DIR_DEFAULT="/opt/var/log/unified-ui"
LOG_DIR="$LOG_DIR_DEFAULT"
STDOUT_LOG="$LOG_DIR/stdout.log"
STDERR_LOG="$LOG_DIR/stderr.log"
PID_FILE="/opt/var/run/unified-ui.pid"
...
audit_boot() {
  ...
  echo "$(date '+%Y-%m-%d %H:%M:%S') $*" >> /opt/var/log/unified-ui-boot.log 2>/dev/null || true
}
...
start_service() {
  ...
  ENV_FILE="${UNIFIED_UI_ENV_FILE:-$ENV_FILE_DEFAULT}"
  if [ -f "$ENV_FILE" ]; then
    # shellcheck disable=SC1090
    . "$ENV_FILE"
  fi
  ...
  export UNIFIED_UI_PORT="${UNIFIED_UI_PORT:-$PANEL_PORT}"

  # Re-resolve log dir after env overrides (DevTools can set UNIFIED_LOG_DIR).
  ...
  LOG_DIR="${UNIFIED_LOG_DIR:-$LOG_DIR_DEFAULT}"
  ...
  audit_boot "[start] spawn: $PYTHON_BIN $TARGET (port=$UNIFIED_UI_PORT, log=$LOG_DIR)"
...
if grep -q "__UNIFIED_UI_PORT__" "$INIT_SCRIPT" 2>/dev/null; then
  sed -i -E "s/__UNIFIED_UI_PORT__/${PANEL_PORT}/g" "$INIT_SCRIPT" || true
fi
...
echo "Логи (install):     $LOG_DIR/unified-ui.log"
echo "Логи (runtime):     /opt/var/log/unified-ui/core.log (и access.log/ws.log)"
echo "========================================"
...
if [ -n "$INSTALL_PARENT_DIR" ] && [ -d "$INSTALL_PARENT_DIR" ]; then
  for ARCH in "$INSTALL_PARENT_DIR"/unified-ui*.tar.gz "$INSTALL_PARENT_DIR"/unified-ui-*.tar.gz; do
    [ -f "$ARCH" ] || continue

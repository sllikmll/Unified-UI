...
def _init_xray_startup_migrations(*, base_etc_dir: str, base_var_dir: str, ui_state_dir: str) -> Dict[str, Any]:
    ...
    unified_restart_cmd = _preferred_unified_restart_cmd(base_etc_dir, build_unified_cmd("-restart"))
    restart_log_file = os.environ.get(
        "UNIFIED_RESTART_LOG_FILE", os.path.join(ui_state_dir, "restart.log")
    )
    ...
    _xray_log_tz_env = os.environ.get("UNIFIED_XRAY_LOG_TZ_OFFSET", "3")
    try:
    ...
    return {
        ...
        "UNIFIED_RESTART_CMD": unified_restart_cmd,
...
def _preferred_unified_restart_cmd(base_etc_dir: str, fallback_cmd: list[str]) -> list[str]:
    """Resolve primary restart command for the current installation.

    Clean unified Mihomo installs do not have legacy `unified`; use the standalone
    Mihomo restart script when configured or present.
    """
    import shlex

    for key in ("MIHOMO_RESTART_CMD", "UNIFIED_RESTART_CMD"):
...
def create_app(*, ws_runtime: bool = False):
    ...
    UNIFIED_RESTART_CMD = xray_ctx["UNIFIED_RESTART_CMD"]
    ...
    from services.unified_lists import PORT_PROXYING_FILE

    XRAY_CONFIG_DIR = os.path.dirname(ROUTING_FILE)
    UNIFIED_CONFIG_DIR = os.path.dirname(PORT_PROXYING_FILE)

    GITHUB_OWNER = os.environ.get("UNIFIED_GITHUB_OWNER", "sllikmll")
    GITHUB_REPO = os.environ.get("UNIFIED_GITHUB_REPO", "unified-ui-community-configs")
    GITHUB_BRANCH = os.environ.get("UNIFIED_GITHUB_BRANCH", "main")

    GITHUB_REPO_URL = os.environ.get(
        "UNIFIED_GITHUB_REPO_URL", f"https://github.com/{GITHUB_OWNER}/{GITHUB_REPO}"
    )

    # -------- Command catalog + background jobs
    from services.unified_commands_catalog import COMMAND_GROUPS
    ...
    from services.unified import (
        ...
        restart_unified as _svc_restart_unified,
    )
    ...
    def restart_unified(source: str = "api"):
        return _svc_restart_unified(UNIFIED_RESTART_CMD, RESTART_LOG_FILE, source=source)

...
(function () {
  ...
  // --------------------
  const KEY_BASE_SID = 'unified_pty_session_id_v1';
  const KEY_BASE_SEQ = 'unified_pty_last_seq_v1';
...
// --------------------
  ...
  function disconnect(opts = {}) {
    ...
    if (ws) {
      try { ws.__unified_manual_close = true; } catch (e) {}
  ...
  async function connect(term, opts = {}) {
    ...
    ws.onclose = (ev) => {
      stopKeepalive();
      try { if (ev && ev.target && ev.target.__unified_manual_close) return; } catch (e) {}

...
(function () {
  ...
  const UNIFIED_TERMINAL_QC_MODE_KEY = 'unified_terminal_qc_mode_v1';
  ...
  function qcGetMode() {
    try {
      const value = localStorage.getItem(UNIFIED_TERMINAL_QC_MODE_KEY);
  ...
  function qcSetMode(mode, ctx) {
    ...
    try { localStorage.setItem(UNIFIED_TERMINAL_QC_MODE_KEY, next); } catch (error) {}

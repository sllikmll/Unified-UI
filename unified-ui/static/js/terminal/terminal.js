...
// UnifiedUI terminal island bootstrap (Stage 8.5)
//
...
(function () {
  ...
  function bootstrapTerminal() {
    ...
    try {
      ...
      if (reg && !reg.__unifiedSetup) {
        reg.__unifiedSetup = true;
    ...
    try {
      if (!ctx.__unifiedCommandsSetup) {
        ctx.__unifiedCommandsSetup = true;
  ...
  // --------------------
  // Compatibility wrappers (global functions + UnifiedUI.terminal.*)
  // --------------------
  ...
  // UnifiedUI.terminal.open/close/toggle for older callers

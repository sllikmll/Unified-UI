...
(function () {
  ...
  function createCommandRegistry(ctx) {
    ...
    function bindToRouter(router) {
      ...
      router.__unifiedBoundCommands = router.__unifiedBoundCommands || {};
      for (const c of getAll()) {
        try { if (router.__unifiedBoundCommands[c.id]) continue; } catch (e0) {}
        try { router.__unifiedBoundCommands[c.id] = true; } catch (e1) {}
        router.register(
          c.matcher,

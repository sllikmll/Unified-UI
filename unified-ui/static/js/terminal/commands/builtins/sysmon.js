...
// Builtin command: `sysmon` (unified-ui)
(function () {
  'use strict';

  const CMD_ID = 'sysmon';
  const CMD_RE = /^(sysmon|unified\s+-sysmon)(\s|$)/i;
  ...
  async function run(ctx, args) {
    ...
    let argv = [];
    try {
      let rest = cmdText.replace(/^\s*(?:unified\s+-sysmon|sysmon)\b/i, '').trim();
    ...
    const cmd = 'sh /opt/etc/unified-ui/tools/sysmon_keenetic.sh' + (safeArgs.length ? (' ' + safeArgs.join(' ')) : '');

...
(function () {
  ...
  function buildShellDisabledNotice(ctx, requestedCommand) {
    ...
    const envName = String(policy.env || 'UNIFIED_ALLOW_SHELL');
  ...
  function createController(ctx) {
    const state = {
      currentCommandMode: 'shell', // 'shell' | 'unified' | 'pty'
    ...
    function setMode(mode, meta) {
      ...
      // unified flag parsing (for lite_runner + quick commands)
      if (m === 'unified') {
        const initialCommand = String(info.initialCommand || '');
        const mm = initialCommand.match(/^unified\s+(.+)$/);
        state.currentCommandFlag = mm ? String(mm[1] || '').trim() : null;
        state.currentCommandLabel = initialCommand || (state.currentCommandFlag ? ('unified ' + state.currentCommandFlag) : 'unified');
      } else {
    ...
    async function open(initialCommand, mode) {
      ...
      if (m === 'pty') {
        ...
        // Lite (shell/unified): show inputs and keep the plain <pre> output active.
        showInputs(c, true);

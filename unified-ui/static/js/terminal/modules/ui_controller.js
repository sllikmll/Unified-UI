...
(function () {
  ...
  function createModule(ctx) {
    ...
    function bindPersistentClickById(id, handler) {
      ...
      if (!el) return;
      try {
        if (el.dataset && el.dataset.unifiedBoundPersistentClick === '1') return;
      } catch (e) {}
      try {
        if (el.dataset) el.dataset.unifiedBoundPersistentClick = '1';
      } catch (e2) {}
      ...
      secs.forEach((sec) => {
        ...
        const key = (() => {
          try {
            ...
            if (k) return 'unified.term.menu.' + String(k);
          } catch (e4) {}
          try {
            const t = (title.textContent || '').trim();
            if (t) return 'unified.term.menu.' + t;
          } catch (e5) {}
          return null;
        })();

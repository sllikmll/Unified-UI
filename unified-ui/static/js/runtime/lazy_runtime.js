...
(() => {
  ...
  function ensureFeature(name) {
    ...
    featureLoading[key] = (async () => {
      ...
      if (buildLoader && typeof buildLoader.ensureReady === 'function') {
        ...
        emitLazyEvent('unified:lazy-feature-ready', { name: key, api: getFeatureApi(key) || managed });
        return true;
      }
      ...
      featureReady[key] = true;
      emitLazyEvent('unified:lazy-feature-ready', { name: key, api: loadedApi });
      return true;
    })().catch((err) => {
      try { console.error('[UnifiedUI] lazy feature failed:', key, err); } catch (e) {}
      featureReady[key] = false;
      emitLazyEvent('unified:lazy-feature-failed', { name: key, error: err || null });
      return false;
    }).finally(() => {
    ...
    terminalPromise = (async () => {
      ...
      safeInvoke(() => {
        ...
        if (api && api.__xkLazyStubInstalled && terminalRoot && terminalRoot.core && typeof terminalRoot.core.createPublicApi === 'function') {
          console.warn('[UnifiedUI] terminal.api is still a lazy stub after init; reinstalling public API (regression guard)');
          terminalRoot.api = terminalRoot.core.createPublicApi();
        }
      });

      if (initResult === false) {
        try { console.warn('[UnifiedUI] terminal bundle loaded but init is deferred: terminal UI/context not ready yet'); } catch (e) {}
        terminalLoaded = false;
        terminalPromise = null;
        emitLazyEvent('unified:lazy-terminal-deferred', { terminal: getXkeenTerminalRoot() || XK.terminal || null });
        return false;
      }

      terminalLoaded = true;
      emitLazyEvent('unified:lazy-terminal-ready', { terminal: getXkeenTerminalRoot() || XK.terminal || null });
      return true;
    })().catch((err) => {
      try { console.error('[UnifiedUI] terminal lazy load failed:', err); } catch (e) {}
      terminalLoaded = false;
      terminalPromise = null;
      emitLazyEvent('unified:lazy-terminal-failed', { error: err || null });
      return false;
    });
  ...
  function ensureFileManagerReady() {
    ...
    fileManagerPromise = (async () => {
      ...
      emitLazyEvent('unified:lazy-file-manager-ready', { fileManager: fileManagerApi });
      return true;
    })().catch((err) => {
      try { console.error('[UnifiedUI] file manager lazy load failed:', err); } catch (e) {}
      fileManagerLoaded = false;
      fileManagerPromise = null;
      emitLazyEvent('unified:lazy-file-manager-failed', { error: err || null });
      return false;
    });
  ...
  function ensureEditorSupport(engine, opts) {
    ...
    editorSupportPromises[next] = (async () => {
      ...
      try { console.error('[UnifiedUI] ' + next + ' lazy load failed:', err); } catch (e) {}
      return false;
    }).finally(() => {
    ...
    try {
      ...
      function normalizeOpenArgs(a, b) {
        ...
        if (typeof a === 'string' && b == null) {
          const s = a.trim();
          const known = { shell: 1, pty: 1, unified: 1 };

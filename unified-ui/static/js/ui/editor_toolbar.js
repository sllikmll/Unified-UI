...
//
// Keeps legacy UnifiedUI globals for feature modules while exposing a neutral
// UnifiedUI.ui.editorToolbar API backed by UnifiedUI.ui.editorActions.
...
(() => {
  ...
  function ensureHelpDrawer() {
    if (document.getElementById('unified-cm-help-overlay')) return;

    const overlay = document.createElement('div');
    overlay.id = 'unified-cm-help-overlay';
    overlay.className = 'unified-cm-help-overlay';

    const drawer = document.createElement('div');
    drawer.id = 'unified-cm-help-drawer';
    drawer.className = 'unified-cm-help-drawer';
    drawer.setAttribute('role', 'dialog');
    drawer.setAttribute('aria-modal', 'true');
    drawer.setAttribute('aria-label', 'Справка по редактору');

    drawer.innerHTML = `
      <div class="unified-cm-help-head">
        <div class="unified-cm-help-title">Справка по редактору</div>
        <button type="button" class="unified-cm-help-close" aria-label="Закрыть">✕</button>
      </div>
      <div class="unified-cm-help-body" id="unified-cm-help-body"></div>
    `;
    ...
    drawer.querySelector('.unified-cm-help-close').addEventListener('click', close);
  ...
  function helpBlock(title, html) {
    return `
      <section class="unified-cm-help-section">
        <h3>${title}</h3>
        ${html}
      </section>
    `;
  ...
  function buildHelpHtml(editor) {
    ...
    const caps = api && typeof api.detectCapabilities === 'function'
      ...
      : { readOnly: false };
    const toolbarItems = editor && editor._unifiedToolbarItems ? editor._unifiedToolbarItems : null;
  ...
  function openHelp(editor) {
    ensureHelpDrawer();
    const overlay = document.getElementById('unified-cm-help-overlay');
    const drawer = document.getElementById('unified-cm-help-drawer');
    const body = document.getElementById('unified-cm-help-body');
  ...
  function openDiffForEditor(editor) {
    ...
    try {
      const scope = (editor && (editor._unifiedDiffScope
        || (editor.getWrapperElement && editor.getWrapperElement().dataset && editor.getWrapperElement().dataset.unifiedDiffScope)
        || (editor.getDomNode && editor.getDomNode().dataset && editor.getDomNode().dataset.unifiedDiffScope))) || '';
      if (scope && window.UnifiedUI && UnifiedUI.ui && UnifiedUI.ui.diff && typeof UnifiedUI.ui.diff.openForScope === 'function') {
        UnifiedUI.ui.diff.openForScope(scope).catch(() => {});
      }
    } catch (e) {}
  }

  const UNIFIED_CM_TOOLBAR_DEFAULT = [
    { id: 'find', svg: UNIFIED_CM_ICONS.search, label: 'Поиск', command: 'findPersistent', fallbackHint: 'Ctrl+F' },
    { id: 'next', svg: UNIFIED_CM_ICONS.down, label: 'Следующее', command: 'findNext', fallbackHint: 'Ctrl+G' },
    { id: 'prev', svg: UNIFIED_CM_ICONS.up, label: 'Предыдущее', command: 'findPrev', fallbackHint: 'Shift+Ctrl+G' },
    { id: 'replace', svg: UNIFIED_CM_ICONS.replace, label: 'Замена', command: 'replace', fallbackHint: 'Ctrl+H' },
    { id: 'comment', svg: UNIFIED_CM_ICONS.comment, label: 'Коммент', command: 'toggleComment', fallbackHint: 'Ctrl+/' },
    { id: 'compare', svg: UNIFIED_CM_ICONS.compare, label: 'Сравнить', fallbackHint: 'Diff', requiresDiffScope: true, onClick: (editor) => openDiffForEditor(editor) },
    { id: 'help', svg: UNIFIED_CM_ICONS.help, label: 'Справка', fallbackHint: '?', isHelp: true, onClick: (editor) => openHelp(editor) },
    { id: 'fs', svg: UNIFIED_CM_ICONS.fullscreen, label: 'Фулскрин', fallbackHint: 'F11 / Esc', onClick: (editor) => {
      const api = actions();
      if (api && typeof api.toggleFullscreen === 'function') api.toggleFullscreen(editor);
    } },
  ];

  const UNIFIED_CM_TOOLBAR_MINI = [
    { id: 'find', svg: UNIFIED_CM_ICONS.search, label: 'Поиск', command: 'findPersistent', fallbackHint: 'Ctrl+F' },
    { id: 'compare', svg: UNIFIED_CM_ICONS.compare, label: 'Сравнить', fallbackHint: 'Diff', requiresDiffScope: true, onClick: (editor) => openDiffForEditor(editor) },
    { id: 'help', svg: UNIFIED_CM_ICONS.help, label: 'Справка', fallbackHint: '?', isHelp: true, onClick: (editor) => openHelp(editor) },
    { id: 'fs', svg: UNIFIED_CM_ICONS.fullscreen, label: 'Фулскрин', fallbackHint: 'F11 / Esc', onClick: (editor) => {
  ...
  function unifiedAttachCmToolbar(editor, items, opts) {
    const api = actions();
    if (!api || typeof api.attachToolbar !== 'function') return null;
    return api.attachToolbar(editor, items || UNIFIED_CM_TOOLBAR_DEFAULT, opts || {});
  }

  UnifiedUI.ui.editorToolbar = Object.assign({}, UnifiedUI.ui.editorToolbar || {}, {
    buildCommonKeys: buildCmExtraKeysCommon,
    openHelp,
    attach: unifiedAttachCmToolbar,
  ...
  window.unifiedOpenCmHelp = openHelp;
  window.unifiedAttachCmToolbar = unifiedAttachCmToolbar;
  window.UNIFIED_CM_ICONS = window.UNIFIED_CM_ICONS || UNIFIED_CM_ICONS;
  window.UNIFIED_CM_TOOLBAR_DEFAULT = window.UNIFIED_CM_TOOLBAR_DEFAULT || UNIFIED_CM_TOOLBAR_DEFAULT;
  window.UNIFIED_CM_TOOLBAR_MINI = window.UNIFIED_CM_TOOLBAR_MINI || UNIFIED_CM_TOOLBAR_MINI;
})();

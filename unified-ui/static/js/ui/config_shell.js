...
(() => {
  ...
  const tabs = (UnifiedUI.state.configShellTabs && typeof UnifiedUI.state.configShellTabs === 'object')
    ? UnifiedUI.state.configShellTabs
    : Object.create(null);
  UnifiedUI.state.configShellTabs = tabs;

  const lifecycles = (UnifiedUI.state.configFeatureLifecycles && typeof UnifiedUI.state.configFeatureLifecycles === 'object')
    ? UnifiedUI.state.configFeatureLifecycles
    : Object.create(null);
  UnifiedUI.state.configFeatureLifecycles = lifecycles;
  ...
  function getConfirmApi() {
    try {
      if (window.UnifiedUI && UnifiedUI.ui && typeof UnifiedUI.ui.confirm === 'function') return UnifiedUI.ui.confirm;
    } catch (e) {}
    return null;
  }
  ...
  function emitLifecycleChange(lifecycle, reason) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('unified:config-lifecycle-change', {
        detail: {
  ...
  function syncLegacyState(tab) {
    ...
    try {
      window.UnifiedUI = window.UnifiedUI || {};
      window.UnifiedUI.state = window.UnifiedUI.state || {};
      window.UnifiedUI.state.fragments = window.UnifiedUI.state.fragments || {};
      if (name) window.UnifiedUI.state.fragments[name] = activeFragment;
    } catch (e) {}
  ...
  function emitTabChange(tab) {
    if (!tab) return;
    try {
      document.dispatchEvent(new CustomEvent('unified:config-shell-change', {
        detail: {
          name: tab.name,
          label: tab.label,

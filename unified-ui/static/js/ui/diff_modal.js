(() => {
  ...
  //   })
  //   UnifiedUI.ui.diffModal.close()
  //   UnifiedUI.ui.diffModal.isOpen()

  window.UnifiedUI = window.UnifiedUI || {};
  const UnifiedUI = window.UnifiedUI;
  UnifiedUI.ui = UnifiedUI.ui || {};

  const ROOT_ID = 'unified-diff-modal';
  const HOST_ID = 'unified-diff-modal-host';
  ...
  function getModalApi() {
    try { return (UnifiedUI.ui && UnifiedUI.ui.modal) ? UnifiedUI.ui.modal : null; } catch (e) {}
    return null;
  }

  function getEditorEngine() {
    try { return (UnifiedUI.ui && UnifiedUI.ui.editorEngine) ? UnifiedUI.ui.editorEngine : null; } catch (e) {}
    return null;
  }
  ...
  function showFeedback(message, kind) {
    ...
    try {
      if (window.UnifiedUI && UnifiedUI.ui && isFn(UnifiedUI.ui.toast)) return UnifiedUI.ui.toast(text, tone);
    } catch (e) {}
    try { if (isFn(window.toast)) return window.toast(text, tone); } catch (e2) {}
    try { console.log('[unified-diff]', text); } catch (e3) {}
  }
  ...
  function ensureDom() {
    ...
    modal.id = ROOT_ID;
    modal.className = 'modal hidden unified-diff-modal';
    modal.setAttribute('role', 'dialog');
    modal.setAttribute('aria-modal', 'true');
    modal.setAttribute('aria-labelledby', ROOT_ID + '-title');

    const card = document.createElement('div');
    card.className = 'modal-content unified-diff-card';

    const head = document.createElement('div');
    head.className = 'modal-header unified-diff-head';
    ...
    headRight.className = 'unified-diff-head-actions';

    const modeGroup = document.createElement('div');
    modeGroup.className = 'unified-diff-mode';
    modeGroup.setAttribute('role', 'group');
    modeGroup.setAttribute('aria-label', 'Режим сравнения');

    _modeBtnSplitEl = makeBtn('Бок-о-бок', 'btn-secondary unified-diff-mode-btn is-active',
      () => setMode('split'),
      'Бок-о-бок · оригинал и новая версия в двух колонках');
    _modeBtnSplitEl.dataset.mode = 'split';
    _modeBtnInlineEl = makeBtn('Подряд', 'btn-secondary unified-diff-mode-btn',
      () => setMode('inline'),
    ...
    ignoreWhitespaceToggle.className = 'unified-diff-ignore-toggle';
    ...
    navGroup.className = 'unified-diff-nav';
    ...
    applyGroup.className = 'unified-diff-apply-group';
    _applyAllToLeftBtnEl = makeBtn('Все ←', 'btn-secondary unified-diff-apply-btn hidden',
      () => applyAllChangesToSide('left'),
      'Перенести все изменения из правой версии в левую');
    _applyAllToRightBtnEl = makeBtn('Все →', 'btn-secondary unified-diff-apply-btn hidden',
      () => applyAllChangesToSide('right'),
      'Перенести все изменения из левой версии в правую');
    _applyToLeftBtnEl = makeBtn('← Влево', 'btn-secondary unified-diff-apply-btn hidden',
      () => applyHunkToSide('left'),
      'Перенести текущий хунк из правой версии в левую');
    _applyToRightBtnEl = makeBtn('Вправо →', 'btn-secondary unified-diff-apply-btn hidden',
    ...
    _xBtnEl = makeBtn('×', 'btn-icon unified-diff-close-x', () => close('x'), 'Закрыть окно сравнения (Esc)');
    ...
    labels.className = 'unified-diff-labels';
    _labelsRowEl = labels;

    const leftWrap = document.createElement('div');
    leftWrap.className = 'unified-diff-side unified-diff-side-left';
    _leftLabelEl = document.createElement('span');
    _leftLabelEl.className = 'unified-diff-label unified-diff-label-left';
    _leftLabelEl.textContent = 'Слева';
    _leftSelectEl = document.createElement('select');
    _leftSelectEl.className = 'unified-diff-source';
    ...
    sep.className = 'unified-diff-side-sep';
    sep.textContent = '↔';

    const rightWrap = document.createElement('div');
    rightWrap.className = 'unified-diff-side unified-diff-side-right';
    _rightLabelEl = document.createElement('span');
    _rightLabelEl.className = 'unified-diff-label unified-diff-label-right';
    _rightLabelEl.textContent = 'Справа';
    _rightSelectEl = document.createElement('select');
    _rightSelectEl.className = 'unified-diff-source';
    ...
    helper.className = 'unified-diff-helper';
    ...
    errorBanner.className = 'unified-diff-error hidden';
    errorBanner.setAttribute('role', 'alert');
    _errorEl = errorBanner;

    const host = document.createElement('div');
    host.id = HOST_ID;
    host.className = 'unified-diff-host';
    _hostEl = host;

    const summary = document.createElement('div');
    summary.className = 'unified-diff-summary';
    _summaryEl = summary;

    const foot = document.createElement('div');
    foot.className = 'modal-actions unified-diff-foot';
    const footActions = document.createElement('div');
    footActions.className = 'unified-diff-foot-actions';
    _revertBtnEl = makeBtn('Отменить изменения', 'btn-secondary unified-diff-revert-btn hidden',
      () => revertComparedChanges(),
      'Вернуть обе стороны к состоянию на момент открытия сравнения');
    _saveBtnEl = makeBtn('Сохранить файл', 'btn-primary unified-diff-save-btn hidden',
      () => saveComparedFile(),
  ...
  async function onSourceChanged(side) {
    ...
    let label = opt.label || '';
    const diffApi = (UnifiedUI.ui && UnifiedUI.ui.diff) || null;
  ...
  function setMode(mode) {
    ...
    refreshActionButtons();
    try {
      const lazy = window.UnifiedUI && UnifiedUI.runtime && UnifiedUI.runtime.lazy;
  ...
  function _createMonacoHunkDecoration(side, change) {
    ...
    return {
      ...
      options: {
        description: 'unified-diff-active-hunk-' + side,
        isWholeLine: true,
        className: 'unified-diff-active-hunk-line unified-diff-active-hunk-line-' + side,
        linesDecorationsClassName: 'unified-diff-active-hunk-gutter unified-diff-active-hunk-gutter-' + side,
      },
    };
  }
  ...
  function reportBackendFallback(fromKind, err) {
    ...
    try {
      console.warn('[unified-diff] backend fallback', {
  ...
  function pickBackend() {
    try {
      const eng = (UnifiedUI.ui && UnifiedUI.ui.editorEngine) ? UnifiedUI.ui.editorEngine : null;
  ...
  function cm6EditorRuntime() {
    try {
      if (UnifiedUI.ui && UnifiedUI.ui.cm6Runtime) return UnifiedUI.ui.cm6Runtime;
    } catch (e) {}
    try {
      if (window.__UNIFIED_CM6_RUNTIME__) return window.__UNIFIED_CM6_RUNTIME__;
    } catch (e2) {}
    return null;
  }
  ...
  function cm6ApplyEditorTheme(host) {
    ...
    try { host.classList.add('CodeMirror', 'unified-cm', 'unified-cm6-host'); } catch (e) {}
    ...
    try {
      host.classList.remove('cm-s-default', 'cm-s-material-darker', 'unified-cm6-light', 'unified-cm6-dark');
      host.classList.add(theme === 'default' ? 'cm-s-default' : 'cm-s-material-darker');
      host.classList.add(theme === 'default' ? 'unified-cm6-light' : 'unified-cm6-dark');
      host.setAttribute('data-unified-cm6-theme', theme === 'default' ? 'light' : 'dark');
    } catch (e3) {}
  }
  ...
  function cm6ClearEditorTheme(host) {
    ...
    try {
      ...
      host.removeAttribute('data-unified-cm6-theme');
    } catch (e) {}
  }
  ...
  function _ensureCm6ActiveHunkSupport(rt) {
    ...
    function buildForState(side, chunk, cmState) {
      ...
      for (let i = 0; i < lines.length; i += 1) {
        ...
        if (!line) continue;
        const classes = [
          'unified-diff-cm6-active-hunk-line',
          'unified-diff-cm6-active-hunk-line-' + side,
        ];
        if (i === 0) classes.push('unified-diff-cm6-active-hunk-start');
        if (i === lines.length - 1) classes.push('unified-diff-cm6-active-hunk-end');
  ...
  async function _confirmDiscardDraft() {
    const ui = (window.UnifiedUI && UnifiedUI.ui && isFn(UnifiedUI.ui.confirm)) ? UnifiedUI.ui.confirm : null;
  ...
  UnifiedUI.ui.diffModal = UnifiedUI.ui.diffModal || {};
  UnifiedUI.ui.diffModal.open = open;
  UnifiedUI.ui.diffModal.close = close;
  UnifiedUI.ui.diffModal.isOpen = isOpen;
  UnifiedUI.ui.diffModal.setMode = setMode;
})();

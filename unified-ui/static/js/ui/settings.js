(() => {
  ...
  //
  // Usage (future):
  //   const s = UnifiedUI.ui.settings.get();
  //   await UnifiedUI.ui.settings.fetch();
  //   await UnifiedUI.ui.settings.patch({ logs: { ansi: true } });

  window.UnifiedUI = window.UnifiedUI || {};
  const UnifiedUI = window.UnifiedUI;
  UnifiedUI.ui = UnifiedUI.ui || {};
  UnifiedUI.core = UnifiedUI.core || {};
  ...
  function getHttpApi() {
    try {
      if (UnifiedUI.core && UnifiedUI.core.http) return UnifiedUI.core.http;
    } catch (e) {}
    return null;
  }
  ...
  function getUiSettingsStoreApi() {
    try {
      const api = UnifiedUI.core && UnifiedUI.core.uiSettings;
      if (!api) return null;
  ...
  function emitSettingsChanged(settings, source) {
    try {
      document.dispatchEvent(new CustomEvent('unified:ui-settings-changed', {
        detail: {
  ...
  function subscribe(listener, options) {
    ...
    try { document.addEventListener('unified:ui-settings-changed', fn); } catch (e) {}
    return () => {
      try { document.removeEventListener('unified:ui-settings-changed', fn); } catch (e) {}
    };
  }

  try { applyEditorCssVars(_ensureCache()); } catch (e) {}

  UnifiedUI.ui.settings = UnifiedUI.ui.settings || {};
  UnifiedUI.ui.settings.DEFAULTS = clone(DEFAULTS);
  UnifiedUI.ui.settings.get = get;
  UnifiedUI.ui.settings.isLoadedFromServer = isLoadedFromServer;
  UnifiedUI.ui.settings.setLocal = setLocal;
  UnifiedUI.ui.settings.patchLocal = patchLocal;
  UnifiedUI.ui.settings.fetch = fetchFromServer;
  UnifiedUI.ui.settings.fetchOnce = fetchOnceFromServer;
  UnifiedUI.ui.settings.patch = patchToServer;
  UnifiedUI.ui.settings.subscribe = subscribe;
  UnifiedUI.ui.settings.clampEditorFontScale = clampEditorFontScale;
  UnifiedUI.ui.settings.getEditorFontScale = getEditorFontScale;
  UnifiedUI.ui.settings.isEditorExpertModeEnabled = isEditorExpertModeEnabled;
})();

(() => {
  ...
  function isFacade(value) {
    ...
    return !!(value && value.__unifiedEditorFacade);
  }
  ...
  function inferEngine(target) {
    ...
    try {
      if (raw && (isFn(raw.getWrapperElement) || raw.__unifiedCm6Bridge || isFn(raw.execCommand))) return 'codemirror';
  ...
  function runCodeMirrorCommand(target, name) {
    ...
    try {
      ...
      if (command === 'links') {
        ...
        if (typeof window.unifiedCmLinksSetEnabled === 'function') {
          const next = !(raw.getLinksEnabled ? raw.getLinksEnabled() : true);
          window.unifiedCmLinksSetEnabled(raw, next);
          return true;
        }
      }
  ...
  function runMonacoCommand(target, name) {
    ...
    try {
      ...
      if (command === 'save') {
        try {
          const evt = new CustomEvent('unified-editor-save-request', { detail: { editor: raw, engine: 'monaco' } });
  ...
  function detectCapabilities(target) {
    ...
    try {
      ...
      caps.links = !!(raw && (isFn(raw.setLinksEnabled) || typeof window.unifiedCmLinksSetEnabled === 'function'));
    } catch (e) {}
    return caps;
  }
  ...
  function fullscreenState(target) {
    const raw = rawEditor(target);
    if (!raw) return null;
    raw.__unifiedFullscreenState = raw.__unifiedFullscreenState || { active: false };
    return raw.__unifiedFullscreenState;
  }
  ...
  function syncToolbarFullscreen(target) {
    const raw = rawEditor(target);
    const bar = raw && raw._unifiedToolbarEl ? raw._unifiedToolbarEl : null;
  ...
  function portalToolbarForFullscreen(toolbar, active, fallbackParent) {
    ...
    const st = toolbar.__unifiedFullscreenPortal || (toolbar.__unifiedFullscreenPortal = {
      active: false,
      parent: null,
      next: null,
    });
  ...
  function setFullscreen(target, flag, opts) {
    ...
    if (next) {
      ...
      placeholder.className = 'unified-editor-fs-placeholder';
    ...
    try {
      document.dispatchEvent(new CustomEvent('unified-editor-fullscreen-change', {
  ...
  function diffApi() {
    try { return (window.UnifiedUI && UnifiedUI.ui && UnifiedUI.ui.diff) ? UnifiedUI.ui.diff : null; } catch (e) {}
    return null;
  }
  ...
  function setDiffScope(target, scope) {
    ...
    try { raw._unifiedDiffScope = key || null; } catch (e) {}
    try {
      const wrapper = getWrapperElement(raw);
      if (wrapper && wrapper.dataset) {
        if (key) wrapper.dataset.unifiedDiffScope = key;
        else delete wrapper.dataset.unifiedDiffScope;
      }
    } catch (e2) {}
    try {
      if (raw._unifiedToolbarItemsBase) attachToolbar(raw, raw._unifiedToolbarItemsBase, { force: true });
    } catch (e3) {}
    return true;
  }
  ...
  function getDiffScope(target) {
    const raw = rawEditor(target);
    return (raw && raw._unifiedDiffScope) ? String(raw._unifiedDiffScope) : '';
  }
  ...
  function openDiff(target, opts) {
    ...
    const scope = (raw && raw._unifiedDiffScope) ? String(raw._unifiedDiffScope) : '';
  ...
  function detachToolbar(target) {
    const raw = rawEditor(target);
    try {
      if (raw && raw._unifiedToolbarEl && raw._unifiedToolbarEl.parentNode) raw._unifiedToolbarEl.parentNode.removeChild(raw._unifiedToolbarEl);
    } catch (e) {}
    try { if (raw) raw._unifiedToolbarEl = null; } catch (e2) {}
    return true;
  }
  ...
  function filterToolbarItems(target, items) {
    ...
    const raw = rawEditor(target);
    const hasDiffScope = !!(raw && raw._unifiedDiffScope);
    const out = [];
    (items || []).forEach((item) => {
      if (!item) return;
  ...
  function attachToolbar(target, items, opts) {
    ...
    const force = !!o.force;
    if (!force && raw._unifiedToolbarEl && (raw._unifiedToolbarEl.isConnected || document.body.contains(raw._unifiedToolbarEl))) {
      return raw._unifiedToolbarEl;
    }
    if (force) detachToolbar(raw);

    const effectiveItems = filterToolbarItems(raw, Array.isArray(items) ? items : []);
    raw._unifiedToolbarItemsBase = Array.isArray(items) ? items : [];
    raw._unifiedToolbarItems = effectiveItems;

    const bar = document.createElement('div');
    bar.className = 'unified-cm-toolbar';
    ...
    raw._unifiedToolbarEl = bar;
    syncToolbarFullscreen(raw);
    syncToolbarReadonly(raw);

    try {
      if (raw && isFn(raw.setOption) && !raw._unifiedToolbarSetOptionWrapped) {
        ...
        raw.setOption = function patchedSetOption(name, value) {
          ...
          if (key === 'fullScreen' || key === 'readOnly' || key === 'mode' || key === 'links') {
            if (key === 'mode') {
              try { attachToolbar(raw, raw._unifiedToolbarItemsBase || effectiveItems, { force: true }); } catch (e) {}
            } else {
              syncToolbarFullscreen(raw);
              syncToolbarReadonly(raw);
            }
        ...
        raw._unifiedToolbarSetOptionWrapped = true;
      }
    } catch (e) {}
  ...
  UnifiedUI.ui.editorActions = api;
  window.unifiedPortalEditorToolbarForFullscreen = portalToolbarForFullscreen;
})();

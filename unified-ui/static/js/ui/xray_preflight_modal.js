(() => {
  ...
  function detectIssuePattern(text, payload) {
    ...
    if (payload && payload.timed_out) {
      return {
        ...
        action: 'Увеличьте таймаут в DevTools -> ENV (UNIFIED_XRAY_TEST_TIMEOUT), уменьшите GeoSite-списки или отключите observatory.',
  ...
  function getRetryWithoutPreflightAction(payload) {
    ...
    try {
      if (
        window.UnifiedUI &&
        UnifiedUI.routing &&
        typeof UnifiedUI.routing.retrySaveWithoutPreflight === 'function'
      ) {
        return UnifiedUI.routing.retrySaveWithoutPreflight;
      }
    } catch (e) {}
    return null;
  }
  ...
  UnifiedUI.ui.showXrayPreflightError = function showXrayPreflightError(payload = {}) {

...
(() => {
  ...
  // window.UnifiedUIMonacoConfig = { version: '0.55.1', prefer: 'local', localVsPaths: [...] }
  const _cfg = {
  ...
  function applyExternalConfig() {
    try {
      const ext = window.UnifiedUIMonacoConfig;
  ...
  function loadClassicScriptTag(url) {
    return new Promise((resolve) => {
      ...
      if (!script) {
        ...
        script.src = url;
        script.async = true;
        try { script.dataset.unifiedMonacoLoader = '1'; } catch (error) {}
      }
      ...
      const onLoad = () => {
        try { script.dataset.unifiedLoaded = '1'; } catch (error) {}
        cleanup();
        resolve(true);
      };
      ...
      if (!attachedByLoader) {
        try {
          if (script.dataset && script.dataset.unifiedLoaded === '1') {
            cleanup();
            return resolve(true);
          }
        } catch (error) {}

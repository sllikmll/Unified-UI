import { isXkeenMipsRuntime } from '../features/unified_runtime.js';
import {
...
(() => {
  ...
  // Do not re-init.
  if (UnifiedUI.ui.monacoShared) return;
  ...
  function _schemaRegistrationUriForModel(modelUri) {
    return `unified://json-schema/${encodeURIComponent(String(modelUri || 'model'))}`;
  }
  ...
  function _applyManagedJsonDiagnostics(monaco) {
    try {
      ...
      const externalSchemas = Array.isArray(current.schemas)
        ? current.schemas.filter((entry) => !String((entry && entry.uri) || '').startsWith('unified://json-schema/'))
        : [];
  ...
  function _normalizeModelUri(monaco, rawUri, language) {
    ...
    try {
      ...
      return monaco.Uri.parse(`unified://json/model-${nextId}.jsonc`);
    } catch (e) {}
    return null;
  }
  ...
  function _getThemeName() {
    try {
      ...
      return (t === 'light') ? 'unified-light' : 'unified-dark';
    } catch (e) {}
    return 'unified-dark';
  }
  ...
  function ensureThemes(monaco) {
    try {
      ...
      monaco.editor.defineTheme('unified-light', {
        base: 'vs',
        inherit: true,
        rules: [],
        colors: {
      ...
      try { window.__unifiedMonacoThemesDefined = true; } catch (e) {}
    } catch (e) {}
  }
  ...
  function wireThemeSync(monaco) {
    try {
      if (_state.themeWired || window.__unifiedMonacoThemeWired) return;
      _state.themeWired = true;
      try { window.__unifiedMonacoThemeWired = true; } catch (e) {}

      document.addEventListener('unified-theme-change', () => {
        try {
  ...
  async function _ensureMonaco() {
    if (!window.UnifiedUI || !UnifiedUI.monacoLoader || typeof UnifiedUI.monacoLoader.ensureMonaco !== 'function') {
      throw new Error('Monaco loader not found (monaco_loader.js)');
    }
    const monaco = await UnifiedUI.monacoLoader.ensureMonaco();
  ...
  function _getSettingsApi() {
    try {
      return (window.UnifiedUI && UnifiedUI.ui && UnifiedUI.ui.settings) ? UnifiedUI.ui.settings : null;
    } catch (e) {
      return null;
    }
  }
  ...
  function _clearSemanticMarkers(monaco, model) {
    try {
      ...
      monaco.editor.setModelMarkers(model, 'unified-semantic', []);
    } catch (e) {}
  }
  ...
  function _runSemanticDiagnosticsForModel(monaco, model) {
    ...
    try { monaco.editor.setModelMarkers(model, 'unified-semantic', markers); } catch (e) {}
  }
        ...
        if (editor) {
          ...
          editor.applyQuickFix = (fix) => {
            ...
            try {
              ...
              editor.pushUndoStop();
              editor.executeEdits('unified-quickfix', operations);
              editor.pushUndoStop();
              return true;
            } catch (e) {}
            return false;
  ...
  function getEditorEngineHelper() {
    try {
      if (window.UnifiedUI && UnifiedUI.ui && UnifiedUI.ui.editorEngine) return UnifiedUI.ui.editorEngine;
    } catch (e) {}
    return null;
  }

...
(() => {
  ...
  const THEME_KEY = 'unified-theme';
  const TOP_LEVEL_ROUTE_CHANGE_EVENT = 'unified:top-level-route-change';
  ...
  function getEditorsForThemeSync() {
    const s = (UnifiedUI && UnifiedUI.state) ? UnifiedUI.state : {};
    const editors = [
      s.routingEditor,
    ...
    try {
      if (window.__unifiedEditors && Array.isArray(window.__unifiedEditors)) {
        window.__unifiedEditors.forEach((cm) => editors.push(cm));
      }
    } catch (e) {}
  ...
  function applyTheme(theme, opts) {
    ...
    if (notify) {
      try {
        document.dispatchEvent(new CustomEvent('unified-theme-change', { detail: { theme: next, cmTheme } }));
      } catch (e) {}
    }
  ...
  function bindThemeToggleLifecycle() {
    ...
    document.addEventListener('unified-editors-ready', () => {
  ...
  function initThemeToggle() {
    ...
    // Prime the page theme early; heavy editor refresh happens later via unified-editors-ready.
  ...
  UnifiedUI.ui.applyTheme = applyTheme;
  UnifiedUI.ui.initThemeToggle = initThemeToggle;
  UnifiedUI.ui.syncThemeToggleButtons = syncThemeToggleButtons;

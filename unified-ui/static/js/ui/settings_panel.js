...
(() => {
  ...
  function showModal(modal) {
    ...
    if (isEmbeddedPanel(modal)) {
      try { window.UnifiedUI?.ui?.tabs?.show?.('ui-settings'); } catch (e) {}
      return;
    }
  ...
  function hideModal(modal) {
    if (!modal) return;
    if (isEmbeddedPanel(modal)) {
      try { window.UnifiedUI?.ui?.tabs?.show?.('mihomo'); } catch (e) {}
      return;
    }
    ensureModalBinding(modal);
    const api = getModalApi();
...
export function getUiSettingsPanelApi() {
  try {
    return window.UnifiedUI && window.UnifiedUI.ui ? (window.UnifiedUI.ui.settingsPanel || null) : null;
  } catch (error) {
    return null;
  }
}

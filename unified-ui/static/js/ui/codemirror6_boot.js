...
const GLOBAL_KEY = '__UNIFIED_CM6_RUNTIME__';
...
function ensureGlobalScope() {
  const win = getWindow();
  if (!win) return null;
  win.UnifiedUI = win.UnifiedUI || {};
  win.UnifiedUI.ui = win.UnifiedUI.ui || {};
  return win;
}
...
function readUiSettingsSnapshot() {
  const win = getWindow();
  try {
    const api = win && win.UnifiedUI && win.UnifiedUI.ui ? win.UnifiedUI.ui.settings : null;
...
function getUiSettingsApi() {
  const win = getWindow();
  try {
    return win && win.UnifiedUI && win.UnifiedUI.ui ? (win.UnifiedUI.ui.settings || null) : null;
  } catch (e) {}
  return null;
}
...
function normalizeDiagnostics(view, list) {
  ...
  return source.map((item) => {
    ...
    return {
      ...
      severity,
      message,
      source: raw.source ? asString(raw.source) : 'unified-cm6',
    };
  });
}
...
function resolveThemeMode(theme) {
  ...
  if (next === 'default' || next === 'light' || next === 'unified-light') return 'light';
  if (next === 'material-darker' || next === 'dark' || next === 'unified-dark') return 'dark';
  try {
...
function applyThemeClasses(host, theme) {
  ...
  try {
    ...
    host.classList.add(mode === 'light' ? 'unified-cm6-light' : 'unified-cm6-dark');
    host.setAttribute('data-unified-cm6-theme', mode);
  } catch (e) {}
}
...
function ensureLinkTooltipEl() {
  let el = document.getElementById('unified-cm6-linktip');
  if (el) return el;
  el = document.createElement('div');
  el.id = 'unified-cm6-linktip';
  el.className = 'unified-cm-linktip';
...
function hideLinkTooltip() {
  try {
    const el = document.getElementById('unified-cm6-linktip');
...
function createLinksExtension() {
  ...
  const handlers = EditorView.domEventHandlers({
    mousemove(event, view) {
      ...
      try {
        view.dom.classList.toggle('unified-cm-link-hover', !!token);
        view.dom.classList.toggle('unified-cm-link-armed', !!token && isCtrlLike(event));
      } catch (e) {}
      if (!token) {
        hideLinkTooltip();
        return false;
    ...
    mouseleave(_event, view) {
      try { view.dom.classList.remove('unified-cm-link-hover', 'unified-cm-link-armed'); } catch (e) {}
      hideLinkTooltip();
      return false;
    },
...
function setBridgeFullscreen(bridge, enabled) {
  ...
  try {
    if (bridge._unifiedToolbarEl) {
      if (typeof window.unifiedPortalEditorToolbarForFullscreen === 'function') {
        window.unifiedPortalEditorToolbarForFullscreen(bridge._unifiedToolbarEl, next);
      }
      bridge._unifiedToolbarEl.classList.toggle('is-fullscreen', next);
    }
  } catch (e) {}
...
function buildBridge(view, ctx) {
  ...
  bridge = {
    __unifiedCm6Bridge: true,
    __unified_cm6_bridge: true,
    engine: 'codemirror',
    backend: BACKEND,
    version: VERSION,
    view,
...
function createBridge(target, opts = {}) {
  ...
  try { view.dom.classList.add('unified-cm6-editor'); } catch (e) {}
  ...
  try { document.addEventListener('unified-theme-change', themeListener); } catch (e) {}
  try { document.addEventListener('unified:ui-settings-changed', uiSettingsListener); } catch (e) {}
  try {
  ...
  bridge.dispose = function disposeWithThemeListener() {
    try { document.removeEventListener('unified-theme-change', themeListener); } catch (e) {}
    try { document.removeEventListener('unified:ui-settings-changed', uiSettingsListener); } catch (e) {}
    return baseDispose();
  };
...
function getEditorEngine() {
  const win = getWindow();
  try { return win && win.UnifiedUI && win.UnifiedUI.ui ? win.UnifiedUI.ui.editorEngine : null; } catch (e) {}
  return null;
}
...
if (win) {
  ...
  win.UnifiedUI.ui.cm6Runtime = runtime;
}

(() => {
  ...
  // Usage:
  //   const ok = await UnifiedUI.ui.confirm({ title, message, okText, cancelText, danger });
  ...
  function emitConfirmFeedback(message, kind) {
    ...
    try {
      if (window.UnifiedUI && UnifiedUI.ui && typeof UnifiedUI.ui.toast === 'function') return UnifiedUI.ui.toast(text, tone);
    } catch (e3) {}
    try { console.log('[unified]', text); } catch (e4) {}
  }
  ...
  function ensureModal() {
    ...
    if (!_bound) {
      ...
      modal.addEventListener('unified:modal-request-close', (e) => {
  ...
  UnifiedUI.ui.confirm = function confirmModal(opts = {}) {
    const options = (opts && typeof opts === 'object') ? opts : {};

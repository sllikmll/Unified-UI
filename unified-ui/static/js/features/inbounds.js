...
(() => {
  ...
  inboundsModuleApi = (() => {
    ...
    function rememberActiveFragment(name) {
      try {
        if (name) localStorage.setItem('unified.inbounds.fragment', String(name));
      } catch (e) {}
    }

    function restoreRememberedFragment() {
      try {
        const v = localStorage.getItem('unified.inbounds.fragment');
    ...
    function setCollapsedFromStorage() {
      ...
      try {
        if (window.localStorage) {
          const stored = localStorage.getItem('unified_inbounds_open');
    ...
    function toggleCard() {
      ...
      try {
        if (window.localStorage) {
          localStorage.setItem('unified_inbounds_open', willOpen ? '1' : '0');
        }
      } catch (e) {
        // ignore
      }
    ...
    function wireModeAutosave() {
      ...
      radios.forEach((r) => {
        if (r.dataset && r.dataset.unifiedWiredMode === '1') return;
        ...
        if (r.dataset) r.dataset.unifiedWiredMode = '1';
      });
    }
        ...
        try {
          ...
          if (res.ok && data && data.ok) {
            ...
            if (restart && jobId) {
              ...
              if (statusEl) statusEl.textContent = 'Режим сохранён. Перезапуск unified...';
              const result = await streamRestartJob(jobId, 'unified -restart (job)...\n');
              const ok = !!(result && result.ok);
              if (ok) {
                msg = 'Режим сохранён: ' + (data.mode || mode) + '. unified перезапущен.';
                if (statusEl) statusEl.textContent = msg;
                ...
                msg = 'Режим сохранён, но перезапуск unified завершился с ошибкой.';
                if (statusEl) statusEl.textContent = msg;

...
(() => {
  ...
  function tagEditorWithDiffScope(editor) {
    ...
    try { editor._unifiedDiffScope = 'routing'; } catch (e2) {}
  }
  ...
  const IDS = {
    ...
    // Global flag
    autoRestart: 'global-autorestart-unified',
  ...
  // Async restart job state (unified -restart) for save with auto-restart.
  ...
  const VIEW_STATE_LS_PREFIX = 'unified.routing.viewstate.v1::';
  ...
  function syncRoutingCodeMirrorOverlays(enabled) {
    ...
    try {
      ...
      } else if (typeof window.unifiedCmLinksSetEnabled === 'function') {
        window.unifiedCmLinksSetEnabled(_cm, allowRichOverlays);
      }
    } catch (e) {}
  }
  ...
  function rememberActiveFragment(name) {
    try {
      if (name) _storeSet('unified.routing.fragment', String(name));
    } catch (e) {}
  }

  function restoreRememberedFragment() {
    try {
      const v = _storeGet('unified.routing.fragment');
  ...
  function forgetRememberedFragmentIfRemoved(removedFiles) {
    ...
    try {
      ...
      if (isRemovedFragmentName(remembered, removedFiles)) {
        _storeRemove('unified.routing.fragment');
      }
    } catch (e) {}
  }
...
function _setRoutingMode(mode, reason) {
  ...
  try {
    const ev = new CustomEvent('unified-routing-mode', { detail: { mode: _routingMode, reason: String(reason || '') } });
  ...
  function _applyCommentsHeaders(res) {
    try {
      const found = (res && res.headers && res.headers.get('X-UnifiedUI-JSONC') === '1');
      const using = (res && res.headers && res.headers.get('X-UnifiedUI-JSONC-Using') === '1');
      const bn = (res && res.headers) ? (res.headers.get('X-UnifiedUI-JSONC-File') || '') : '';
  ...
  function _getSavedContent() {
    ...
    try { setRoutingDirty(!!dirty); } catch (e) {}
      ...
      try { document.dispatchEvent(new CustomEvent('unified:routing-editor-dirty', { detail: { dirty: !!dirty } })); } catch (e) {}
	  }
	  ...
	  function dispatchEditorContentEvent(reason) {
	    try {
	      ...
	      document.dispatchEvent(new CustomEvent('unified:routing-editor-content', {
	        detail: {
...
const HELP_MODAL_ID = 'unified-routing-help-modal';
const HELP_CONTENT_ID = 'unified-routing-help-content';
...
function ensureHelpModal() {
  ...
  function getRoutingHelpTheme() {
    ...
    try {
      const stored = window.localStorage && localStorage.getItem('unified-theme');
...
function closeHelp() {
  ...
  function getMonacoModelMarkers(opts) {
    try {
      ...
      const markers = api.editor.getModelMarkers({
        resource: model.uri,
        owner: o.anyOwner ? undefined : String(o.owner || 'unified'),
      }) || [];
  ...
  function jumpToJsonErrorLocation() {
    ...
    if ((!loc || (!loc.line && loc.index == null)) && _engine === 'monaco') {
      loc = getMonacoMarkerErrorLocation({ anyOwner: true, preferNonOwner: 'unified', excludeOwner: '' });
    }
    if (!loc) return false;
  ...
  function clearMonacoMarkers() {
    try {
      ...
      const model = (_monaco.getModel && _monaco.getModel()) ? _monaco.getModel() : null;
      if (!model) return;
      api.editor.setModelMarkers(model, 'unified', []);
    } catch (e) {}
  }
  ...
  function setMonacoMarkers(markers) {
    try {
      ...
      const model = (_monaco.getModel && _monaco.getModel()) ? _monaco.getModel() : null;
      if (!model) return;
      api.editor.setModelMarkers(model, 'unified', Array.isArray(markers) ? markers : []);
    } catch (e) {}
  }
  ...
  async function load() {
    ...
    try {
      ...
      try {
        const b64 = res.headers.get('X-UnifiedUI-Notice-B64');
        if (b64) {
          const kind = res.headers.get('X-UnifiedUI-Notice-Kind') || 'info';
    ...
    try {
      ...
      if (res.ok && data && data.ok) {
        ...
        if (restart && jobId) {
          ...
          await appendRestartLog('⏳ Запуск unified -restart (job)…\n');
  ...
  function runMonacoDiagnostics() {
    ...
    const nativeMarker = safari
      ? getMonacoMarkerErrorLocation({ anyOwner: true, preferNonOwner: 'unified', excludeOwner: 'unified' })
      : null;
    ...
    if (safari) {
      try {
        setTimeout(() => {
          ...
          const refreshed = getMonacoMarkerErrorLocation({ anyOwner: true, preferNonOwner: 'unified', excludeOwner: 'unified' });
  ...
  function wireButton(btnId, handler) {
    const btn = $(btnId);
    if (!btn) return;
    if (btn.dataset && btn.dataset.unifiedWired === '1') return;
    ...
    if (btn.dataset) btn.dataset.unifiedWired = '1';
  }
  ...
  function wireRoutingScenarioCollapse() {
    ...
    if (header.dataset) header.dataset.unifiedScenarioCollapseWired = '1';
  ...
  function wireRoutingScenarioSwitcher() {
    ...
    try {
      document.addEventListener('unified:routing-editor-content', () => {
        try { syncRoutingScenarioUiFromEditor('content'); } catch (e) {}
      });
      window.addEventListener('unified-routing-mode', () => {
  ...
  function wireUI() {
    ...
    if (fragSel && !(fragSel.dataset && fragSel.dataset.unifiedWired === '1')) {
      ...
      if (fragSel.dataset) fragSel.dataset.unifiedWired = '1';
    }
    ...
    if (help && !(help.dataset && help.dataset.unifiedWired === '1')) {
      help.addEventListener('click', (e) => {
        e.preventDefault();
        openHelp();
      });
      if (help.dataset) help.dataset.unifiedWired = '1';
    }
  ...
  function createEditor() {
    ...
    try {
      if (cm.getWrapperElement) {
        ...
        wrapper.classList.add('unified-cm');
  ...
  function moveToolbarToHost(cm) {
    try {
      ...
      if (!host || !cm || !cm._unifiedToolbarEl) return;
      if (host.contains(cm._unifiedToolbarEl)) return;
      host.appendChild(cm._unifiedToolbarEl);
      // Mark so CSS can adjust spacing when toolbar lives inside the header.
      try { cm._unifiedToolbarEl.classList.add('xk-toolbar-in-host'); } catch (e) {}
  ...
  function ensureRoutingMonacoToolbar() {
    try {
      ...
      if (!bar) {
        ...
        bar.className = 'unified-cm-toolbar xk-routing-monaco-toolbar';
      ...
      if (!host.contains(bar) && !(bar.__unifiedFullscreenPortal && bar.__unifiedFullscreenPortal.active)) {
  ...
  function syncRoutingToolbarUi(engine) {
    ...
    try {
      if (_cm && _cm._unifiedToolbarEl) {
        ...
        if (isMonaco) {
          _cm._unifiedToolbarEl.style.display = 'none';
        } else {
          _cm._unifiedToolbarEl.style.display = '';
    ...
    try {
      ...
      if (_routingMonacoToolbarEl) {
        ...
        const btns = _routingMonacoToolbarEl.querySelectorAll('button.unified-cm-tool');
        (btns || []).forEach((btn) => {
  ...
  function syncToolbarForEngine(engine) {
    try {
      if (!_cm || !_cm._unifiedToolbarEl || !_cm._unifiedToolbarEl.querySelectorAll) return;
      const bar = _cm._unifiedToolbarEl;
      ...
      const btns = bar.querySelectorAll('button.unified-cm-tool');
      (btns || []).forEach((btn) => {
  ...
  function _syncToolbarFsClass(isFs) {
    try {
      if (_cm && _cm._unifiedToolbarEl) {
        ...
        _cm._unifiedToolbarEl.classList.toggle('is-fullscreen', cmToolbarFullscreen);
      }
    } catch (e) {}
    try {
      ...
      if (_routingMonacoToolbarEl) {
        ...
        try {
          if (typeof window.unifiedPortalEditorToolbarForFullscreen === 'function') {
            window.unifiedPortalEditorToolbarForFullscreen(
  ...
  function wireRoutingUiSettingsSyncOnce() {
    ...
    if (!_routingScenarioSettingsEventWired) {
      ...
      try {
        document.addEventListener('unified:ui-settings-changed', (event) => {
  ...
  const LEGACY_LS_KEY = 'unified.routing.editor.engine';
  const GLOBAL_LS_KEY = 'unified.editor.engine';
  ...
  function wireGlobalEngineSyncOnce() {
    ...
    try {
      if (window.__unifiedRoutingEngineSyncWired) return;
      window.__unifiedRoutingEngineSyncWired = true;
      ...
      document.addEventListener('unified-editor-engine-change', (ev) => {
        try {
  ...
  function getRoutingMonacoModelUri() {
    try {
      ...
      return `unified://routing/${safeName}`;
    } catch (e) {}
    return 'unified://routing/routing.jsonc';
  }
  ...
  function getRoutingCodeMirrorWrappers() {
    ...
    try {
      ...
      const all = Array.from(document.querySelectorAll('.CodeMirror, .unified-cm6-host'));
  ...
  function wirePageReturnOnce() {
    ...
    try {
      if (window.__unifiedRoutingPageReturnWired) return;
      window.__unifiedRoutingPageReturnWired = true;
  ...
  function showCmToolbar(show) {
    try {
      if (_cm && _cm._unifiedToolbarEl) {
        _cm._unifiedToolbarEl.style.display = show ? '' : 'none';
      }
    } catch (e) {}
  }
  ...
  function init() {
    ...
    if (!_commentsUxWired) {
      try {
        document.addEventListener('unified:routing-comments-ux', (ev) => {
      ...
      const finishInit = () => {
        ...
        Promise.resolve(bootWithMonaco ? _engineSwitchChain : null)
          ...
          .finally(() => {
            ...
            try { syncRoutingToolbarUi(_engine); } catch (e) {}
            try { validate(); } catch (e) {}
            try { document.dispatchEvent(new CustomEvent('unified-editors-ready')); } catch (e) {}
            try { updateEditorMetaStatus(); } catch (e) {}

...
(() => {
  ...
  function showCmToolbar(show) {
    try {
      ...
      if (cm && cm._unifiedToolbarEl) cm._unifiedToolbarEl.style.display = show ? '' : 'none';
    } catch (e) {}
  }
  ...
  function ensureHeaderFsButton() {
    try {
      ...
      btn.type = 'button';
      btn.className = 'unified-cm-tool xk-mihomo-fs-btn';
  ...
  function syncHeaderFsButton(engine) {
    try {
      ...
      const cm = getSharedEditor();
      const hasToolbar = !!(cm && cm._unifiedToolbarEl);
  ...
  function repositionCmToolbarForEngine(engine) {
    try {
      const cm = getSharedEditor();
      const bar = cm && cm._unifiedToolbarEl;
      ...
      if (hostSlot) {
        if (!hostSlot.contains(bar) && !(bar.__unifiedFullscreenPortal && bar.__unifiedFullscreenPortal.active)) {
  ...
  function syncToolbarForEngine(engine) {
    try {
      const cm = getSharedEditor();
      if (!cm || !cm._unifiedToolbarEl || !cm._unifiedToolbarEl.querySelectorAll) return;
      const bar = cm._unifiedToolbarEl;
      const isMonaco = (String(engine || '').toLowerCase() === 'monaco');
  ...
  function ensureEditor() {
    ...
    // Dirty tracking (user edits only).
    try {
      if (!_cm._unifiedDirtyWired) {
        _cm.on('change', () => {
          if (!_suppressDirty) _editorDirty = true;
          scheduleMihomoSchemaValidation();
        });
        _cm._unifiedDirtyWired = true;
      }
    } catch (e) {}
  ...
  function tagMihomoEditorWithDiffScope(editor) {
    ...
    try { editor._unifiedDiffScope = 'mihomo'; } catch (e2) {}
  }
  ...
  function getViewStateStore() {
    ...
    try {
      _viewStateStore = helper.createViewStateStore({
        buildKey: () => 'unified.mihomo.panel.viewstate.v1::config',
      });
    } catch (e) {
  ...
  function clearMihomoSchemaDiagnostics() {
    ...
    try {
      ...
      if (monaco && monaco.editor && typeof monaco.editor.setModelMarkers === 'function' && model) {
        monaco.editor.setModelMarkers(model, 'unified-mihomo-schema', []);
      }
    } catch (e) {}
  }
  ...
  function applyMihomoSchemaDiagnostics(result) {
    ...
    try {
      ...
      if (monaco && monaco.editor && typeof monaco.editor.setModelMarkers === 'function' && model) {
        ...
        monaco.editor.setModelMarkers(model, 'unified-mihomo-schema', markers);
      }
    } catch (e) {}
  ...
  MP.saveConfig = async function saveConfig() {
    ...
    try {
      if (restart) {
        ...
        await appendSharedRestartLog('⏳ Запуск unified -restart (job)…\n');
      } else {
  ...
  function attachProfilesHandlers() {
    const tbody = $(IDS.profilesList);
    if (!tbody) return;
    if (tbody.dataset && tbody.dataset.unifiedBound === '1') return;
    ...
    if (tbody.dataset) tbody.dataset.unifiedBound = '1';
  }
  ...
  MP.init = function init() {
    ...
    if (_inited || (root.dataset && root.dataset.unifiedMihomoPanelInited === '1')) return;
    _inited = true;
    if (root.dataset) root.dataset.unifiedMihomoPanelInited = '1';
    ...
    const finishInit = () => {
      ...
      if (tplSel && (!tplSel.dataset || tplSel.dataset.unifiedAutoLoad !== '1')) {
        ...
        if (tplSel.dataset) tplSel.dataset.unifiedAutoLoad = '1';
      }
      ...
      if (header && panel && (!header.dataset || header.dataset.unifiedWired !== '1')) {
      ...
      if (filter && (!filter.dataset || filter.dataset.unifiedWired !== '1')) {
        filter.addEventListener('change', () => {
          MP.loadBackups();
        });
        if (filter.dataset) filter.dataset.unifiedWired = '1';
      }

...
import { getXkeenEditorEngineApi, getXkeenPageName } from './unified_runtime.js';

let unifiedTextsModuleApi = null;

(() => {
  unifiedTextsModuleApi = (() => {
    ...
    function isCm6Editor(editor) {
      try {
        ...
        if (editor.__unifiedCm6Bridge || editor.backend === 'cm6') return true;
        const wrap = (typeof editor.getWrapperElement === 'function') ? editor.getWrapperElement() : null;
        return !!(wrap && wrap.classList && wrap.classList.contains('unified-cm6-editor'));
      } catch (e) {}
      return false;
    }
    ...
    function shouldRestartAfterSave() {
      const cb = document.getElementById('global-autorestart-unified');
      // On unified.html this checkbox is absent; default restart=true so changes apply immediately.
      if (!cb) return true;
      return !!cb.checked;
    }
    ...
    async function loadText(url, textareaId, statusId, label, stateKey) {
        ...
        if (res.ok && data && data.ok) {
          ...
          if (statusEl) {
            statusEl.textContent = (restart && restartJobId)
              ? `${label} сохранён. Перезапуск unified...`
              : msg;
          }
          ...
          if (restart && restartJobId) {
            ...
            if (ok) {
              const successMsg = `${label} сохранён, unified перезапущен.`;
              ...
              const errorMsg = `${label} сохранён, но перезапуск unified завершился с ошибкой.`;
      ...
      const opts = {
        ...
        // Keep rendering sane for potentially larger files (unified.json).
        viewportMargin: 80,
      };
      ...
      if (!ed) return null;
      try { ed.getWrapperElement().classList.add('unified-cm'); } catch (e) {}
    ...
    function init() {
      ...
      const finishInit = () => {
        ...
        const unifiedConfigEd = attachEditor('unified-config-editor', 'unifiedConfigEditor', { mode: 'application/jsonc' });

        // Save buttons
        wireButton('port-proxying-save-btn', () => saveText('/api/unified/port-proxying', 'port-proxying-editor', 'port-proxying-status', 'port_proxying.lst', 'portProxyingEditor'));
        wireButton('port-exclude-save-btn', () => saveText('/api/unified/port-exclude', 'port-exclude-editor', 'port-exclude-status', 'port_exclude.lst', 'portExcludeEditor'));
        wireButton('ip-exclude-save-btn', () => saveText('/api/unified/ip-exclude', 'ip-exclude-editor', 'ip-exclude-status', 'ip_exclude.lst', 'ipExcludeEditor'));
        wireButton('unified-config-save-btn', () => saveText('/api/unified/config', 'unified-config-editor', 'unified-config-status', 'unified.json', 'unifiedConfigEditor'));

        // Initial load
        loadText('/api/unified/port-proxying', 'port-proxying-editor', 'port-proxying-status', 'port_proxying.lst', 'portProxyingEditor');
        loadText('/api/unified/port-exclude', 'port-exclude-editor', 'port-exclude-status', 'port_exclude.lst', 'portExcludeEditor');
        loadText('/api/unified/ip-exclude', 'ip-exclude-editor', 'ip-exclude-status', 'ip_exclude.lst', 'ipExcludeEditor');
        loadText('/api/unified/config', 'unified-config-editor', 'unified-config-status', 'unified.json', 'unifiedConfigEditor');
    ...
    function reloadPortProxying() {
      ensureInited();
      return loadText('/api/unified/port-proxying', 'port-proxying-editor', 'port-proxying-status', 'port_proxying.lst', 'portProxyingEditor');
    }

    function reloadPortExclude() {
      ensureInited();
      return loadText('/api/unified/port-exclude', 'port-exclude-editor', 'port-exclude-status', 'port_exclude.lst', 'portExcludeEditor');
    }

    function reloadIpExclude() {
      ensureInited();
      return loadText('/api/unified/ip-exclude', 'ip-exclude-editor', 'ip-exclude-status', 'ip_exclude.lst', 'ipExcludeEditor');
    }
    function reloadXkeenConfig() {
      ensureInited();
      return loadText('/api/unified/config', 'unified-config-editor', 'unified-config-status', 'unified.json', 'unifiedConfigEditor');
    }
    ...
    function getToggleHeaders() {
      try {
        return Array.from(document.querySelectorAll('[data-xk-toggle="unified-settings"]'));
      } catch (e) {
        return [];
      }
    }
    ...
    function applyCardOpenState(isOpen) {
      const body = document.getElementById('unified-body');
      const arrow = document.getElementById('unified-arrow');
    ...
    function refreshAttachedEditors() {
      ['portProxyingEditor', 'portExcludeEditor', 'ipExcludeEditor', 'unifiedConfigEditor'].forEach((k) => {
        try {
    ...
    function getEditorViewStateStore() {
      ...
      try {
        editorViewStateStore = helper.createViewStateStore({ prefix: 'unified.texts.viewstate.v1::' });
      } catch (e) {
    ...
    function getEditorStateKeys() {
      return ['portProxyingEditor', 'portExcludeEditor', 'ipExcludeEditor', 'unifiedConfigEditor'];
    }
    ...
    function wireCardToggle() {
      getToggleHeaders().forEach((header) => {
        if (!header || (header.dataset && header.dataset.unifiedToggleWired === '1')) return;
        const onToggle = (e) => {
          if (e) {
            if (e.type === 'keydown') {
        ...
        if (header.dataset) header.dataset.unifiedToggleWired = '1';
      });
    }
...
export function getXkeenTextsApi() {
  try {
    if (unifiedTextsModuleApi && typeof unifiedTextsModuleApi.init === 'function') return unifiedTextsModuleApi;
  } catch (error) {
    return null;
  }
  return null;
...
export const unifiedTextsApi = Object.freeze({

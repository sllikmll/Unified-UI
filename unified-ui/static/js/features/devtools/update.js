...
(() => {
  ...
  const confirmAction = SH.confirmAction || (async (opts) => {
    if (window.UnifiedUI && UnifiedUI.ui && typeof UnifiedUI.ui.confirm === 'function') return !!(await UnifiedUI.ui.confirm(opts || {}));
  ...
  function _renderCheck(data) {
    ...
    if (kind !== 'main') {
      try {
        ...
        const wanted = ['unified-ui-routing.tar.gz', 'unified-ui.tar.gz'];
        const haveAsset = assets.some((a) => {
          const name = a && a.name ? String(a.name) : '';
          return wanted.indexOf(name) >= 0;
        });
        if (!haveAsset) hintParts.push('в релизе нет unified-ui-routing.tar.gz / unified-ui.tar.gz');
      } catch (e) {}
    }

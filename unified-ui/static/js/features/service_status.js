...
(() => {
  // UnifiedUI service status lamp + core selection modal
  // Public API:
  ...
  function syncCoreModalSelectionUi() {
    ...
    setCoreModalStatus(`Будет активировано ядро ${selectedCore}. После применения сервис unified перезапустится.`, 'hint');
  }
  ...
  async function fetchServiceStatusSnapshot() {
    ...
    let res;
    try {
      res = await fetch('/api/unified/status', {
        cache: 'no-store',
  ...
  function notifyServiceActionPending(action) {
    const message = action === 'start'
      ? 'Запускаем unified...'
      : action === 'stop'
        ? 'Останавливаем unified...'
        : 'Перезапускаем unified...';

    return toast({
      id: 'unified-service-action',
      message,
      kind: 'info',
      sticky: false,
  ...
  function notifyServiceActionResult(message, ok) {
    try { dismissXkeenToast('unified-service-action'); } catch (e) {}
  ...
  function controlActionTimeoutMessage(action) {
    return action === 'start'
      ? 'Истекло время ожидания запуска unified.'
      : action === 'stop'
        ? 'Истекло время ожидания остановки unified.'
        : 'Истекло время ожидания перезапуска unified.';
  }

  // ---------- Xkeen control (start/stop/restart) ----------

  function shouldAutoRestartAfterSave() {
    const cb = $('global-autorestart-unified');
    return !!(cb && cb.checked);
  }

  function getStatusElForControl() {
    // Prefer routing-status on panel; fall back to unified-status on unified.html.
    return $('routing-status') || $('unified-status') || null;
  }
    ...
    if (normalizedState === 'pending') {
      return normalizedAction === 'start'
        ...
        : normalizedAction === 'stop'
          ? 'unified: останавливаем...'
          : normalizedAction === 'restart'
            ? 'unified: перезапускаем...'
            : 'unified: ожидаем...';
    }
    if (normalizedState === 'running') return 'unified: работает';
    if (normalizedState === 'stopped') return 'unified: остановлен';
    if (normalizedState === 'error') return 'unified: ошибка';
    return '';
  }
  ...
  function controlActionErrorMessage(action) {
    return action === 'start'
      ? 'Не удалось запустить unified.'
      : action === 'stop'
        ? 'Не удалось остановить unified.'
        : 'Не удалось перезапустить unified.';
  }

  function controlActionMismatchMessage(action, finalState) {
    const state = String(finalState || 'неизвестно');
    return action === 'start'
      ? `Запросили запуск unified, но сервис сейчас: ${state}.`
      : action === 'stop'
        ? `Запросили остановку unified, но сервис сейчас: ${state}.`
        : `Запросили перезапуск unified, но сервис сейчас: ${state}.`;
  }
  ...
  async function finalizeControlLifecycle(action, requestId, outcome) {
    ...
    try { notifyServiceActionResult(fallbackMessage || 'Ошибка управления unified.', false); } catch (e3) {}
    return false;
  }
  ...
  function controlXkeen(action) {
    const normalizedAction = normalizeAction(action);
    const map = {
      start: '/api/unified/start',
      stop: '/api/unified/stop',
      restart: '/api/restart',
    };
      ...
      try {
        if (normalizedAction === 'start' || normalizedAction === 'restart') {
          ...
          const controlIntro = isRestartAction
            ? '⏳ Запуск unified -restart (job)…\n'
            : '⏳ Запуск unified -start (job)…\n';
          const controlFailureText = isRestartAction
            ? 'Ошибка перезапуска unified'
            : 'Ошибка запуска unified';
  ...
  function bindControlButtons() {
    const startBtn = $('unified-start-btn');
    const stopBtn = $('unified-stop-btn');
    const restartBtn = $('unified-restart-btn');

    if (startBtn && (!startBtn.dataset || startBtn.dataset.unifiedBound !== '1')) {
      startBtn.addEventListener('click', (e) => {
        e.preventDefault();
        if (startBtn.disabled) return;
        void controlXkeen('start');
      });
      if (startBtn.dataset) startBtn.dataset.unifiedBound = '1';
    }

    if (stopBtn && (!stopBtn.dataset || stopBtn.dataset.unifiedBound !== '1')) {
      stopBtn.addEventListener('click', (e) => {
        e.preventDefault();
        if (stopBtn.disabled) return;
        void controlXkeen('stop');
      });
      if (stopBtn.dataset) stopBtn.dataset.unifiedBound = '1';
    }

    if (restartBtn && (!restartBtn.dataset || restartBtn.dataset.unifiedBound !== '1')) {
      restartBtn.addEventListener('click', (e) => {
        e.preventDefault();
        if (restartBtn.disabled) return;
        void controlXkeen('restart');
      });
      if (restartBtn.dataset) restartBtn.dataset.unifiedBound = '1';
    }
  }
  ...
  function renderXkeenServiceStatus(state, core, controlState) {
    const lamp = $('unified-service-lamp');
    const textEl = $('unified-service-text');
    const coreEl = $('unified-core-text');
    const startBtn = $('unified-start-btn');
    const stopBtn = $('unified-stop-btn');
    const restartBtn = $('unified-restart-btn');
  ...
  async function confirmXkeenCoreChange() {
    ...
    try {
      const res = await fetch('/api/unified/core', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  function bindCoreModalUI() {
    const coreTextEl = $('unified-core-text');

    ...
    try {
      const store = helper.createViewStateStore({
        buildKey: (ctx) => {
          ...
          if (!filename) return '';
          return 'unified.routing.templates.viewstate.v1::'
            + slot
            + '::'
...
function stripTemplateHeader(text) {
  // Remove the first-line meta header if present: // unified-template: {...}
  if (typeof text !== 'string') return '';
  return text.replace(/^\s*\/\/\s*unified-template:\s*\{[^\n]*\}\s*\n?/m, '');
}
  ...
  try {
    ...
    try {
      if (_editCm.getWrapperElement) {
        _editCm.getWrapperElement().classList.add('unified-cm');
      ...
      try {
        ...
        window.__unifiedEditors.push(_editCm);
      } catch (e3) {}
    ...
    try {
      ...
      try {
        if (_previewCm.getWrapperElement) {
          _previewCm.getWrapperElement().classList.add('unified-cm');
      ...
      try {
        window.__unifiedEditors = window.__unifiedEditors || [];
        window.__unifiedEditors.push(_previewCm);
      } catch (e3) {}

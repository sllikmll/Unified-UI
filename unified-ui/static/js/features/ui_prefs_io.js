...
(() => {
  ...
  function toast(msg, kindOrError) {
    ...
    try { console.log('[unified]', msg); } catch (e) {}
  }
  ...
  const STATIC_KEYS = [
    'unified-theme',
    'unified-typography-v1',
    'unified-layout-v1',
    // branding cache (current builds)
    'unified-branding-cache-v1',
    // legacy only (older builds)
    'unified-branding-v1',
    'unified_ui_hide_donate',
    'unified.devtools.logs.live',
    'unified.devtools.logs.live_touched',
  ];
  ...
  function shouldExportKey(k) {
    ...
    if (k.startsWith('unified.devtools.')) return true;
    return false;
  }
  ...
  function applyAll() {
    ...
    try {
      if (XK && XK.ui && typeof XK.ui.applyTheme === 'function') {
        let t = null;
        try { t = localStorage.getItem('unified-theme'); } catch (e) {}
    ...
    try {
      document.dispatchEvent(new CustomEvent('unified-ui-prefs-applied'));
    } catch (e) {}
  }
    ...
    if (dlBtn) {
      dlBtn.addEventListener('click', async () => {
        try {
          ...
          const name = `unified-ui-prefs-${ts.getFullYear()}${pad(ts.getMonth()+1)}${pad(ts.getDate())}-${pad(ts.getHours())}${pad(ts.getMinutes())}${pad(ts.getSeconds())}.json`;

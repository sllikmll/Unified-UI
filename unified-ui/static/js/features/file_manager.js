    ...
    try {
      ...
      try {
        ...
        if (volumesBtn) {
          ...
          //   - URL: ?usbdisks=1
          //   - or localStorage: unified.fm.usbdisks=1
          ...
          try {
            if (!forceEnable && window.localStorage) {
              forceEnable = (String(window.localStorage.getItem('unified.fm.usbdisks') || '') === '1');
            }
          } catch (e2) {}
      ...
      try {
        ...
        else hideUnused = !!(storageGetJSON('unified-layout-v1', null) || {}).hideUnused;
      } catch (e) {}
      ...
      if (note) {
        if (isMips) {
          ...
          else if (reason === 'disabled') msg += ' Включите UNIFIED_REMOTEFM_ENABLE=1.';

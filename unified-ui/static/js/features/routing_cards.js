...
(function () {
  ...
  function _syncFocusGroupVisibility(guiEnabled) {
    try {
      const focusGroup = document.querySelector('.unified-ctrl-group-routing-focus');
  ...
  function applyFocusMode(mode, opts) {
    ...
    try {
      document.dispatchEvent(new CustomEvent('unified:routing-focus-mode', { detail: { mode: next, guiEnabled } }));
    } catch (e) {}
    return next;
  }
  ...
  function init() {
    ...
    if (firstRun) {
      try {
        document.addEventListener('unified:ui-settings-changed', () => {
          try { initAvailableModules(); } catch (e2) {}
          try { applyUiSettings(); } catch (e2) {}
        });
      } catch (e) {}
      try {
        document.addEventListener('unified:panel-view-changed', () => {
          try { initAvailableModules(); } catch (e2) {}
          try { applyUiSettings(); } catch (e2) {}
        });
      } catch (e) {}

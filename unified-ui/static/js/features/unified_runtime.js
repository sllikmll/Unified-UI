...
function getWindowUnifiedUI() {
  try {
    const win = getWindowRef();
    return win && win.UnifiedUI ? win.UnifiedUI : null;
  } catch (e) {
    return null;
  }
}
...
export function ensureXkeenRoot() {
  try {
    const win = getWindowRef();
    if (!win) return null;
    if (!win.UnifiedUI || typeof win.UnifiedUI !== 'object') win.UnifiedUI = {};
    return win.UnifiedUI;
  } catch (e) {
    return null;
  }
}
...
export function attachXkeenEditorToolbar(editor, items, options) {
  ...
  try {
    const win = getWindowRef();
    if (win && typeof win.unifiedAttachCmToolbar === 'function') {
      return win.unifiedAttachCmToolbar(editor, items, options);
    }
  } catch (e2) {}
  return null;
}
...
export function openXkeenModal(modal, source, fallbackLocked) {
  ...
  try {
    ...
    if (modalApi && typeof modalApi.open === 'function') {
      modalApi.open(modal, { source: source || 'unified_runtime' });
      return true;
    }
  } catch (e) {}
...
export function closeXkeenModal(modal, source, fallbackLocked) {
  ...
  try {
    ...
    if (modalApi && typeof modalApi.close === 'function') {
      modalApi.close(modal, { source: source || 'unified_runtime' });
      return true;
    }
  } catch (e) {}

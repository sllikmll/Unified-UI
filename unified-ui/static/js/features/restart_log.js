...
import { ansiToXkeenHtml, escapeXkeenHtml, getXkeenCommandJobApi, getXkeenUiApi, syncXkeenBodyScrollLock, toastXkeen } from './unified_runtime.js';
...
(() => {
  ...
  const PREFLIGHT_PAYLOAD_STORAGE_PREFIX = 'unified.restartLog.preflight.';
  const PREFLIGHT_PAYLOAD_INDEX_KEY = 'unified.restartLog.preflight.index';
  ...
  const RESTART_SOURCE_META = Object.freeze({
    ...
    'core-switch': {
      label: 'Ядро Xkeen',
      successText: 'ядро переключено, unified перезапущен',
      failureText: 'смена ядра завершилась ошибкой',
      bucket: 'core',
    },
    routing: {
      label: 'Routing',
      successText: 'сохранён, unified перезапущен',
      failureText: 'сохранён, но перезапуск unified завершился ошибкой',
      bucket: 'routing',
    },
    inbounds: {
      label: 'Inbounds',
      successText: 'сохранены, unified перезапущен',
      failureText: 'сохранены, но перезапуск unified завершился ошибкой',
      bucket: 'xray',
    },
    outbounds: {
      label: 'Outbounds',
    ...
    'observatory-preset': {
      label: 'Observatory',
      successText: 'пресет применён, unified перезапущен',
      failureText: 'пресет применён, но перезапуск unified завершился ошибкой',
      bucket: 'xray',
    },
    'mihomo-config': {
      label: 'Mihomo',
      successText: 'конфиг применён, unified перезапущен',
      failureText: 'конфиг применён, но перезапуск unified завершился ошибкой',
      bucket: 'mihomo',
    },
    'mihomo-profile-activate': {
      label: 'Mihomo',
      successText: 'профиль активирован, unified перезапущен',
      failureText: 'профиль активирован, но перезапуск unified завершился ошибкой',
      bucket: 'mihomo',
    },
    'mihomo-backup-restore': {
      label: 'Mihomo',
      successText: 'бэкап восстановлен, unified перезапущен',
      failureText: 'бэкап восстановлен, но перезапуск unified завершился ошибкой',
      bucket: 'mihomo',
    },
    'manual-mihomo': {
      label: 'Xkeen',
    ...
    'snapshot-restore': {
      label: 'Бэкап',
      successText: 'восстановлен, unified перезапущен',
      failureText: 'восстановлен, но перезапуск unified завершился ошибкой',
      bucket: 'backup',
    },
    'backups-page': {
      label: 'Бэкап',
      successText: 'операция выполнена, unified перезапущен',
      failureText: 'операция выполнена, но перезапуск unified завершился ошибкой',
      bucket: 'backup',
    },
    'port-proxying': {
      label: 'Port proxying',
      successText: 'список сохранён, unified перезапущен',
      failureText: 'список сохранён, но перезапуск unified завершился ошибкой',
      bucket: 'service',
    },
    'port-exclude': {
      label: 'Port exclude',
    ...
    'xray-subscription-delete': {
      label: 'Подписка Xray',
      successText: 'удалена, unified перезапущен',
      failureText: 'удалена, но перезапуск unified завершился ошибкой',
      bucket: 'subscription',
    },
    'xray-subscription-refresh': {
      label: 'Подписка Xray',
      successText: 'обновлена, unified перезапущен',
      failureText: 'обновлена, но перезапуск unified завершился ошибкой',
      bucket: 'subscription',
      toastSuccess: 'Подписка Xray обновлена. unified перезапущен.',
      toastFailure: 'Подписка Xray обновлена, но перезапуск unified завершился ошибкой.',
    },
    'xray-preflight': {
      label: 'Xray preflight',
      successText: 'конфиг прошёл проверку',
      failureText: 'конфиг отклонён, unified не перезапускался',
      bucket: 'preflight',
    },
  });
  ...
  function buildRestartSummaryMessage(source, ok, baseMessage, details) {
    ...
    return ok
      ? `переключено на ${core}, unified перезапущен`
  ...
  function bindLogInteractions() {
    getLogEls().forEach((el) => {
      if (!el) return;
      try {
        if (el.dataset && el.dataset.unifiedRestartLogInteractions === '1') return;
        ...
        if (el.dataset) el.dataset.unifiedRestartLogInteractions = '1';
      } catch (error) {}
    });
  }
  ...
  function bindOnce(el, handler) {
    if (!el) return;
    try {
      if (el.dataset && el.dataset.unifiedBound === '1') return;
      el.addEventListener('click', handler);
      if (el.dataset) el.dataset.unifiedBound = '1';
    } catch (error) {}
  }
  ...
  RL.init = function init() {
    ...
    try {
      document.querySelectorAll('[data-xk-restart-log-search]').forEach((input) => {
        if (input.dataset && input.dataset.unifiedRestartLogSearchBound === '1') return;
        input.addEventListener('input', () => {
          RL.setSearch(input.value || '');
        });
        if (input.dataset) input.dataset.unifiedRestartLogSearchBound = '1';
      });
      document.querySelectorAll('[data-xk-restart-log-search-clear]').forEach((btn) => {
        bindOnce(btn, (event) => {
          event.preventDefault();

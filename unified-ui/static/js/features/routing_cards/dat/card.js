...
(function () {
  ...
  function initDatCard() {
    ...
    if (b7) b7.addEventListener('click', (e) => {
      e.preventDefault();
      try {
        if (window.UnifiedUI && UnifiedUI.ui && UnifiedUI.ui.datContents && typeof UnifiedUI.ui.datContents.open === 'function') {
          UnifiedUI.ui.datContents.open('geosite');
        } else {
    ...
    if (b8) b8.addEventListener('click', (e) => {
      ...
      try {
        if (window.UnifiedUI && UnifiedUI.ui && UnifiedUI.ui.datContents && typeof UnifiedUI.ui.datContents.open === 'function') {
          UnifiedUI.ui.datContents.open('geoip');
        } else {
          toast('DAT: модуль просмотра содержимого не загружен.', true);
        }
      } catch (err) {

...
(() => {
  ...
  const LS_KEY_HIDE = 'unified_ui_hide_donate';
  const TOP_LEVEL_ROUTE_CHANGE_EVENT = 'unified:top-level-route-change';
  const DONATE_PREF_CHANGE_EVENT = 'unified:donate-pref-change';
  ...
  function wireModal() {
    ...
    if (!btn.dataset || btn.dataset.unifiedDonateWired !== '1') {
    ...
    const wireClose = (el) => {
      if (!el) return;
      if (el.dataset && el.dataset.unifiedDonateWired === '1') return;
      el.addEventListener('click', (e) => {
        e.preventDefault();
        hideDonateModal();
      });
      if (el.dataset) el.dataset.unifiedDonateWired = '1';
    };
    ...
    addrEls.forEach((el) => {
      if (el.dataset && el.dataset.unifiedDonateCopyWired === '1') return;
      el.addEventListener('click', (e) => {
        e.preventDefault();
        copyToClipboard(el.dataset.copy || el.textContent || '', el);
      });
      if (el.dataset) el.dataset.unifiedDonateCopyWired = '1';
    });
  }
  ...
  function wireDevtoolsToggle() {
    ...
    if (!toggle.dataset || toggle.dataset.unifiedDonateWired !== '1') {

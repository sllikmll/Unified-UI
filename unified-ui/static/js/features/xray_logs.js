...
(() => {
  ...
  const STORAGE_KEY = 'unified.ui.xrayLogs.v1';
  const SEED_KEY = 'unified.seed.logsPrefs.v1';
  ...
  const XRAY_DOMAIN_CACHE_STORAGE_KEY = 'unified.ui.xrayLogs.domainCache.v1';
  ...
  const XRAY_DOMAINS_DISPLAY_STORAGE_KEY = 'unified.ui.xrayLogs.domainsWanted.v1';
  ...
  const XRAY_LIVE_WANTED_STORAGE_KEY = 'unified.ui.xrayLogs.liveWanted.v1';
  ...
  async function xrayLogsEnable() {
    ...
    try {
      ...
      // рестарт (warning→info): два быстрых `unified -restart` подряд конфликтуют,

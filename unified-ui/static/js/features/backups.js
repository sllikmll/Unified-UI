...
(() => {
    ...
    try {
      _snapViewStateStore = helper.createViewStateStore({
        buildKey: (ctx) => {
          ...
          if (!name) return '';
          return 'unified.backups.snapshot.viewstate.v1::' + encodeURIComponent(name);
        },
      });
    } catch (e) {
  ...
  function toast(message, kindOrError) {
    ...
    try { console.log('[unified]', text); } catch (e5) {}
    return null;
  }
    ...
    try {
      ...
      if (restartLog && typeof restartLog.streamJob === 'function') {
        ...
        if (!createRes.ok || createData.ok === false || !createData.job_id) {
          return 'Не удалось запустить PTY-перезапуск unified. Обновите страницу позже или перезапустите unified вручную.';
        }

        const result = await restartLog.streamJob(String(createData.job_id), {
          clear: true,
          reveal: true,
          intro: 'unified -restart (job)...\n',
          maxWaitMs: 5 * 60 * 1000,
        });
        ...
        if (err) return 'Перезапуск unified завершился с ошибкой: ' + err;
        if (exitCode !== null) return 'Перезапуск unified завершился с ошибкой (exit_code=' + exitCode + ').';
        return 'Не удалось подтвердить перезапуск unified. Обновите страницу позже или перезапустите unified вручную.';
      }

      const res = await fetch('/api/restart', { method: 'POST' });
      if (!res || !res.ok) {
        return 'Запрос на перезапуск unified не подтвердился. Обновите страницу позже или перезапустите unified вручную.';
      }
      return '';
    } catch (e) {
      return 'Не удалось подтвердить перезапуск unified. Обновите страницу позже или перезапустите unified вручную.';
  ...
  async function restoreBackup(filename) {
    ...
    const ok = await confirmAction({
      ...
      details: [
        ...
        'После восстановления будет запрошен перезапуск unified.',
      ],
      okText: 'Восстановить',
      cancelText: 'Отменить',
      danger: true,
  ...
  function ensureSnapEditor() {
    ...
    try {
      ...
      try {
        ...
        window.__unifiedEditors.push(_snapCm);
      } catch (e3) {}
  ...
  function wireSnapModalOnce() {
    ...
    // Engine toggle
    const sel = el('xray-snapshot-engine-select');
    if (sel && !(sel.dataset && sel.dataset.unifiedWired === '1')) {
      if (sel.dataset) sel.dataset.unifiedWired = '1';
    ...
    const ok = await confirmAction({
      ...
      details: wantRestart
        ? [
            ...
            'После этого будет запрошен перезапуск unified.',
          ]
        : 'Изменения будут записаны в конфиг без перезапуска unified.',
      okText: 'Восстановить',
      cancelText: 'Отменить',
      danger: false,
  ...
  function wireSnapshotsToolbarOnce() {
    const clearBtn = el('backups-clear-snapshots-btn');
    if (!clearBtn) return;
    if (clearBtn.dataset && clearBtn.dataset.unifiedWired === '1') return;
    if (clearBtn.dataset) clearBtn.dataset.unifiedWired = '1';

...
import { getXkeenTextsApi } from './unified_texts.js';
import {
  closeXkeenModal,
...
(() => {
  ...
  // Public API:
  //   UnifiedUI.github.init({ repoUrl })
  //   UnifiedUI.github.openExportModal(), closeExportModal()
  //   UnifiedUI.github.openCatalogModal(), closeCatalogModal()
  //   UnifiedUI.github.exportUserConfigsToGithub(), importUserConfigById(id)
  ...
  async function refreshAfterImport() {
    ...
    try {
      document.dispatchEvent(new CustomEvent('unified-github-imported'));
    } catch (e) {}
  }
  ...
  async function exportUserConfigsToGithub() {
    ...
    const payload = {
      title: 'UnifiedUI config ' + new Date().toLocaleString(),
      description: desc,
      tags: tag ? [tag] : [],
    };
    ...
    try {
      ...
      if (res.ok && data.ok) {
        const msg =
          'Конфигурация ' +
          (data.cfg_id || cfgId) +
          ' загружена. Не забудьте перезапустить unified после проверки.';
        setStatus(msg, false);

...
(() => {
  ...
  const TAB_DEFS = Object.freeze([
    ...
    { key: 'view:protocol-subscription', label: 'Подписка' },
    ...
    { key: 'view:protocol-vmess', label: 'VMess' },
    { key: 'view:protocol-shadowsocks', label: 'Shadowsocks' },
    { key: 'view:protocol-mieru', label: 'Mieru' },
    { key: 'view:protocol-naiveproxy', label: 'NaiveProxy' },
    { key: 'view:protocol-telegram', label: 'Telegram MTProxy' },
    { key: 'view:unified', label: 'Порты и Исключения' },
  ...
  function loadPrefs() {
    ...
    try {
      const raw = localStorage.getItem('unified-layout-v1');
  ...
  function saveApply(next) {
    ...
    if (c && typeof c.save === 'function') c.save(next);
      ...
      try { localStorage.setItem('unified-layout-v1', JSON.stringify(next || {})); } catch (e) {}
    }
  ...
  function wireTabsDnD(ul) {
    if (!ul || ul.dataset && ul.dataset.unifiedWired === '1') return;
    if (ul.dataset) ul.dataset.unifiedWired = '1';
  ...
  function initControls() {
    ...
    if (elResetTabs && (!elResetTabs.dataset || elResetTabs.dataset.unifiedWired !== '1')) {
      ...
      if (elResetTabs.dataset) elResetTabs.dataset.unifiedWired = '1';
    }

    // Sync UI if prefs were changed in another tab
    try {
      window.addEventListener('storage', (ev) => {
        if (!ev || ev.key !== 'unified-layout-v1') return;
        sync();
      });
    } catch (e) {}

    try {
      document.addEventListener('unified-ui-prefs-applied', sync);
    } catch (e) {}
  }

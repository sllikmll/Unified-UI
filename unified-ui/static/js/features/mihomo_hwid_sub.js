...
import { getXkeenFilePath, getXkeenLazyRuntimeApi } from './unified_runtime.js';
...
(() => {
  ...
  const COLLAPSE_PREFS = Object.freeze({
    diag: { key: 'unified.mihomo.hwid.diag.open.v1', defaultOpen: false },
    compare: { key: 'unified.mihomo.hwid.compare.open.v1', defaultOpen: false },
  });
  ...
  function getLegacyMihomoImportApi() {
    try {
      const features = window && window.UnifiedUI && window.UnifiedUI.features;
  ...
  function hwidSourceLabel(source) {
    ...
    if (!s) return '';
    if (s === 'UNIFIED_MIHOMO_HWID') return 'DevTools override';
    if (s === 'UNIFIED_HWID') return 'UNIFIED_HWID override';
  ...
  async function ensureCodeMirrorPreviewEditor() {
    ...
    try {
      const w = _previewCm && typeof _previewCm.getWrapperElement === 'function'
        ...
        : null;
      if (w) w.classList.add('unified-cm', 'xk-hw-preview-cm');
  ...
  async function doProbe() {
    ...
    try {
      ...
      try {
        ...
        if (!hwid) {
          tips.push('HWID не определён: provider будет без x-hwid. Если провайдер ожидает уже привязанный HWID, открой DevTools → ENV, найди через поиск HWID, заполни UNIFIED_MIHOMO_HWID и снова нажми «Проверить».');
        } else if (hwidWarning) {
    ...
    try {
      ...
      if (job) {
        try {
          ...
          await appendSharedRestartLog('⏳ Запуск unified -restart (job ' + job + ')\n');
        } catch (e) {}
        ...
        if (ok) {
          toastMsg(`Применено ✅ ${nm ? '(' + nm + ') ' : ''}unified перезапущен.`, 'success');
        } else {

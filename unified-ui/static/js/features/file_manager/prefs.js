...
(() => {
  ...
  const KEYS = {
    sort: 'unified.fm.sort',           // JSON: { key: 'name'|'size'|'perm'|'mtime', dir: 'asc'|'desc', dirsFirst: true }
    showHidden: 'unified.fm.dotfiles', // '1'|'0'
    mask: 'unified.fm.mask',           // last used selection mask (glob)
    bookmarksLocal: 'unified.fm.bookmarks.local', // JSON: [{ label: '...', value: '/path' }, ...]
    geom: 'unified.fm.geom_v3',        // JSON: { w: px, h: px, shiftX: px }

    // Panels state (persist between reloads)
    panels: 'unified.fm.panels_v1',    // JSON: { left:{cwd,target,filter}, right:{cwd,target,filter} }
    activeSide: 'unified.fm.activeSide', // 'left'|'right'
  };

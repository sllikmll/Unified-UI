...
(() => {
  ...
  const VIEW_STATE_LS_PREFIX = 'unified.fm.editor.viewstate.v1::';
  ...
  function isCm6Editor(cm) {
    try { return !!(cm && cm.__unifiedCm6Bridge); } catch (e) {}
    return false;
  }
    ...
    try {
      ...
      if (cm && cm._unifiedToolbarEl) {
        if (typeof window.unifiedPortalEditorToolbarForFullscreen === 'function') {
          window.unifiedPortalEditorToolbarForFullscreen(cm._unifiedToolbarEl, !!isFs);
        }
  ...
  function ensureHeaderFs(ui) {
    try {
      ...
      bar.className = 'unified-cm-toolbar xk-monaco-fsbar';
      bar.style.margin = '0';
...
function syncToolbarForEngine(engine) {
  ...
  function ensureCm() {
    ...
    try {
      ...
      window.__unifiedEditors.push(cm);
    } catch (e) {}

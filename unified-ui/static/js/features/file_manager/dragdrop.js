...
(() => {
  ...
  function hasInternalFm(dt) {
    try {
      const types = Array.from((dt && dt.types) || []);
      return types.includes('application/x-unified-fm') || types.includes('text/x-unified-fm');
    } catch (e) {
      return false;
    }
  }
  ...
  function getInternalFm(dt) {
    let raw = '';
    try { raw = dt.getData('application/x-unified-fm') || ''; } catch (e) {}
    if (!raw) { try { raw = dt.getData('text/x-unified-fm') || ''; } catch (e) {} }
    if (!raw) { try { raw = dt.getData('text/plain') || ''; } catch (e) {} }
    raw = String(raw || '');
    if (raw.startsWith('unified-fm:')) raw = raw.slice('unified-fm:'.length);
    if (!raw) return null;
  ...
  function attachDragDrop(opts) {
    ...
    pd.list.addEventListener('dragstart', (e) => {
      ...
      try { e.dataTransfer.setData('application/x-unified-fm', JSON.stringify(payload)); } catch (e2) {}
      try { e.dataTransfer.setData('text/x-unified-fm', JSON.stringify(payload)); } catch (e2) {}
      try { e.dataTransfer.setData('text/plain', 'unified-fm:' + JSON.stringify(payload)); } catch (e2) {}

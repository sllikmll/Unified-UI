...
(() => {
  ...
  async function runXkeenCommand(flag) {
    ...
    const label = `unified ${flag}`;
    try {
      await Promise.resolve(openXkeenTerminal({ cmd: label, mode: 'unified' }));
      return true;
    } catch (e) {
      return false;
    }
  ...
  function buildPrereleaseUpdateCommand(flag, tag, coreLabel) {
    ...
    const lines = [
      ...
      `${buildShellPrintfLine(`[Unified UI] Запускаем обновление ${normalizedCore} до pre-release ${releaseLabel}.`)};`,
      `${buildShellPrintfLine('[Unified UI] Xkeen ниже выполнит установку и покажет свой прогресс.')};`,
      `if printf '%s\\n%s\\n' '9' ${shSingleQuote(normalizedTag)} | unified ${normalizedFlag}; then`,
      `  ${buildShellPrintfLine(`[Unified UI] Обновление ${normalizedCore} завершено.`)};`,
      versionSummaryCommand ? `  ${versionSummaryCommand};` : '',
      'else',
      '  __xk_prerelease_status="$?";',
      `  printf '%s\\n' "[Unified UI] Обновление ${normalizedCore} до pre-release ${releaseLabel} завершилось с кодом $__xk_prerelease_status. Проверьте вывод выше.";`,
  ...
  function buildMihomoPrereleaseInstallCommand(tag, installMeta, coreLabel) {
    ...
    const lines = [
      buildShellPrintfLine(''),
      buildShellPrintfLine(`[Unified UI] Установка ${normalizedCore} pre-release ${normalizedTag}`),
      buildShellPrintfLine(`[Unified UI] Архитектура: ${arch}${opkgArch ? ` | opkg: ${opkgArch}` : ''}${endian ? ` | endian: ${endian}` : ''}`),
      ...(note ? [buildShellPrintfLine(`[Unified UI] ${note}`)] : []),
      '__xk_tmpdir="$(mktemp -d /tmp/unified-mihomo-pre.XXXXXX 2>/dev/null || true)"',
      'if [ -z "$__xk_tmpdir" ]; then',
      '  __xk_tmpdir="/tmp/unified-mihomo-pre.$$"',
      ...
      '    return "$?"',
      '  fi',
      `  ${buildShellPrintfLine('[Unified UI] Не найден curl или wget для скачивания.')}`,
      '  return 127',
      '}',
      '__xk_unpack_gzip() {',
      '  _archive="$1"',
      ...
      '    return "$?"',
      '  fi',
      `  ${buildShellPrintfLine('[Unified UI] Не найден gzip или gunzip для распаковки .gz.')}`,
      '  return 127',
      '}',
      '__xk_checksum_file=""',
    ];
    ...
    lines.push(
      ...
      '  printf \'%s\\n\' "[Unified UI] Скачиваем $_asset_name..."',
      '  if ! __xk_fetch "$_asset_url" "$_archive"; then',
      '    printf \'%s\\n\' "[Unified UI] Не удалось скачать $_asset_name, пробуем следующий вариант."',
      '    return 1',
      '  fi',
      '  if ! __xk_verify_checksum "$_asset_name" "$_archive"; then',
      '    printf \'%s\\n\' "[Unified UI] Контрольная сумма не совпала для $_asset_name, пробуем следующий вариант."',
      '    return 1',
      '  fi',
      '  if ! __xk_unpack_gzip "$_archive" "$_binary"; then',
      '    printf \'%s\\n\' "[Unified UI] Не удалось распаковать $_asset_name, пробуем следующий вариант."',
      '    return 1',
      '  fi',
      ...
      '    printf \'%s\\n\' "[Unified UI] $_asset_name не запускается на этом роутере, пробуем следующий вариант."',
      '    return 1',
      '  fi',
    ...
    lines.push(
      ...
      `  ${buildShellPrintfLine('[Unified UI] Не удалось подобрать рабочий Mihomo pre-release для этого роутера.')}`,
      '  __xk_cleanup',
      '  exit 1',
      'fi',
      'printf \'%s\\n\' "[Unified UI] Выбран asset: $__xk_selected_asset_name"',
      ...
      `  ${buildShellPrintfLine('[Unified UI] Сохраняем резервную копию текущего Mihomo.')}`,
      '  if ! cp /opt/sbin/mihomo "$__xk_backup"; then',
      `    ${buildShellPrintfLine('[Unified UI] Не удалось сохранить резервную копию, продолжаем без неё.')}`,
      '    __xk_backup=""',
      '  fi',
      'fi',
      `  ${buildShellPrintfLine('[Unified UI] Останавливаем unified...')}`,
      'unified -stop',
      '__xk_stop_rc="$?"',
      'if [ "$__xk_stop_rc" -ne 0 ]; then',
      '  printf \'%s\\n\' "[Unified UI] unified -stop завершился с кодом $__xk_stop_rc, продолжаем замену бинарника."',
      'fi',
      `  ${buildShellPrintfLine('[Unified UI] Обновляем бинарник Mihomo...')}`,
      'if ! cp "$__xk_selected_binary" /opt/sbin/mihomo.new; then',
      `  ${buildShellPrintfLine('[Unified UI] Не удалось записать /opt/sbin/mihomo.new.')}`,
      '  unified -start >/dev/null 2>&1 || true',
      '  __xk_cleanup',
      '  exit 1',
      'fi',
      'chmod 755 /opt/sbin/mihomo.new || true',
      'if ! mv /opt/sbin/mihomo.new /opt/sbin/mihomo; then',
      `  ${buildShellPrintfLine('[Unified UI] Не удалось заменить /opt/sbin/mihomo.')}`,
      ...
      '  exit 1',
      'fi',
      `  ${buildShellPrintfLine('[Unified UI] Запускаем unified...')}`,
      'unified -start',
      '__xk_start_rc="$?"',
      'if [ "$__xk_start_rc" -ne 0 ]; then',
      '  printf \'%s\\n\' "[Unified UI] unified -start завершился с кодом $__xk_start_rc."',
      '  if [ -n "$__xk_backup" ] && [ -f "$__xk_backup" ]; then',
      `    ${buildShellPrintfLine('[Unified UI] Восстанавливаем предыдущий Mihomo из резервной копии.')}`,
      '    cp "$__xk_backup" /opt/sbin/mihomo >/dev/null 2>&1 || true',
      ...
      '  exit 1',
      'fi',
      `  ${buildShellPrintfLine('[Unified UI] Проверяем установленную версию Mihomo...')}`,
      '/opt/sbin/mihomo -v 2>&1',
      '__xk_final_rc="$?"',
      'if [ "$__xk_final_rc" -eq 0 ]; then',
      `  ${buildShellPrintfLine('[Unified UI] Установка Mihomo pre-release завершена.')}`,
      'else',
      '  printf \'%s\\n\' "[Unified UI] Mihomo заменён, но проверка версии завершилась с кодом $__xk_final_rc."',
      'fi',
      '__xk_cleanup',
  ...
  function configurePrereleaseAction(btn, release, installedVersion, { flag, coreLabel } = {}) {
    ...
    if (isDirectAssetPrereleaseInstall(releaseInstall)) {
      ...
      btn.dataset.prereleaseMode = 'unified_prompt';
      btn.__xkPrereleaseInstallMeta = null;

...
(() => {
  mihomoGeneratorModuleApi = (() => {
    ...
    function init() {
      ...
      #######################################################################################################
      # Если UnifiedUI работает не на всех портах (unified -cp), то для корректной работы следующих сервисов выполните:
      # Для Discord:
      # unified -ap 80,443,2053:2096,8443,19200:19400,50000:50100
      # Для Whatsapp/Telegram:
      # unified -ap 80,443,596:599,1400,3478,5222
      #######################################################################################################
      ...
      `;
        ...
        const corePillEl = document.getElementById("unified-core-pill");
...
function showCmToolbar(show) {
  try {
    if (editor && editor._unifiedToolbarEl) editor._unifiedToolbarEl.style.display = show ? '' : 'none';
  } catch (e) {}
}
...
function syncToolbarForEngine(engine) {
  try {
    if (!editor || !editor._unifiedToolbarEl || !editor._unifiedToolbarEl.querySelectorAll) return;
    const bar = editor._unifiedToolbarEl;
...
function initEngineToggle() {
  ...
  try {
    if (ee && typeof ee.onChange === 'function') ee.onChange(onGlobal);
    else document.addEventListener('unified-editor-engine-change', (ev) => onGlobal(ev && ev.detail ? ev.detail : {}));
  } catch (e) {}
}
        ...
        function moveToolbarToHeader() {
          try {
            ...
            if (host && editor && editor._unifiedToolbarEl) {
              if (!(editor._unifiedToolbarEl.__unifiedFullscreenPortal && editor._unifiedToolbarEl.__unifiedFullscreenPortal.active)) {
                host.appendChild(editor._unifiedToolbarEl);
              }
            }
          } catch (e) {
            // ignore
        ...
        function resetToolbar() {
          if (!editor) return;
          try {
            if (editor._unifiedToolbarEl && editor._unifiedToolbarEl.parentNode) {
              editor._unifiedToolbarEl.parentNode.removeChild(editor._unifiedToolbarEl);
            }
          } catch (e) {}
          try { delete editor._unifiedToolbarEl; } catch (e) { editor._unifiedToolbarEl = null; }
        }
  ...
  try {
    ...
    console.warn("Failed to move cursor to error position", e);
        ...
        function enhanceEditorOptions() {
          if (!editor || editor._unifiedEnhanced) return;
          try { editor._unifiedEnhanced = true; } catch (e) {}
        ...
        function attachPreviewToolbar() {
          if (!editor || editor._unifiedToolbarAttached) return;
          try { editor._unifiedToolbarAttached = true; } catch (e) {}
        ...
        function wireLazyPreviewToolbar() {
          if (!editor || editor._unifiedToolbarLazyWired) return;
          try { editor._unifiedToolbarLazyWired = true; } catch (e) {}
        ...
        function initEditor() {
          ...
          // Mark as UnifiedUI editor so shared CSS fixes apply in light theme
          try {
            const w = editor.getWrapperElement && editor.getWrapperElement();
            if (w) w.classList.add('unified-cm');
          } catch (e) {}

          // Register in global list so main.js theme toggle can sync it
          try {
            window.__unifiedEditors = window.__unifiedEditors || [];
            window.__unifiedEditors.push(editor);
          } catch (e) {}
        ...
        // React on theme changes (main.js dispatches unified-theme-change)
        document.addEventListener('unified-theme-change', (e) => {
          if (!editor) return;
      ...
      // ----- proxies -----
        ...
        function createProxyCard(initial) {
          ...
          function updateTypeUI() {
            ...
            if (t === "auto") {
              ...
              dataArea.placeholder = "hostname: unified\n# auth-key: tskey-auth-...\nstate-dir: ./tailscale\nudp: true\naccept-routes: true";
        ...
        function wireSessionDraftLifecycle() {
          ...
          try {
            window.addEventListener("unified:top-level-nav-intent", () => persist("top-level-nav"));
          } catch (e) {}
      ...
      // ----- apply to router -----
        async function applyToRouter(notify = false) {
          ...
          try {
            ...
            if (jobId) {
              const baseMsg = 'Конфиг сохранён. Запущен перезапуск unified (в фоне).';
              setStatus(finalWarnings.length ? (baseMsg + "\n" + finalWarnings.join(" • ")) : baseMsg, finalWarnings.length ? 'warn' : 'ok');

              try {
                await clearSharedRestartLog();
                await appendSharedRestartLog('⏳ Запуск unified -restart (job ' + jobId + ')\n');
              } catch (e) {}
              try {
                appendValidationLog('');
                appendValidationLog('--- unified -restart (job ' + jobId + ') ---');
              } catch (e) {}
              ...
              if (CJ && typeof CJ.waitForCommandJob === 'function') {
                ...
                if (ok) {
                  const summary = finalWarnings.length
                    ? 'Конфиг сохранён, unified успешно перезапущен, но стоит проверить предупреждения ниже.'
                    : 'Конфиг сохранён и unified успешно перезапущен.';
                  setStatus(finalWarnings.length ? 'unified перезапущен с предупреждениями.' : 'unified перезапущен.', finalWarnings.length ? 'warn' : 'ok');
                  showMihomoResultModal({
                    ...
                    subtitle: jobId ? ('Перезапуск unified выполнялся фоновым заданием: ' + jobId + '.') : 'Конфиг сохранён и применён на роутере.',
                  ...
                  if (notify) try { toast(finalWarnings.length ? 'Конфиг применён с предупреждениями.' : 'unified перезапущен.', finalWarnings.length ? 'info' : 'success'); } catch (e) {}
                } else {
                  ...
                  showMihomoResultModal({
                    ...
                    subtitle: 'Файл уже записан на роутер, однако фоновый перезапуск unified не завершился штатно.',
                    ...
                    errors: [errMsg],
                    log: validationLogRaw,
                    source: 'Фоновый job unified -restart',
                    action: 'Применение',
                  });
                  try {
                ...
                showMihomoResultModal({
                  ...
                  subtitle: 'Роутер уже получил config.yaml, а перезапуск unified выполняется в фоне.',
                  leadTitle: 'Фоновое применение запущено',
                  leadDesc: 'Job ID: ' + jobId,
                  summary: 'Конфиг сохранён на роутере. Следите за логом проверки ниже, пока unified завершает перезапуск.',
                  warnings: finalWarnings,
                  log: validationLogRaw,
                  source: 'Фоновый job unified -restart',
                  action: 'Применение',
            ...
            const baseMsg = "Конфиг отправлен на роутер, unified перезапускается.";
            setStatus(finalWarnings.length ? (baseMsg + "\n" + finalWarnings.join(" • ")) : baseMsg, finalWarnings.length ? 'warn' : 'ok');
            showMihomoResultModal({
              kind: finalWarnings.length ? 'warning' : 'success',
              kicker: 'Mihomo / apply',
              title: finalWarnings.length ? 'Конфиг применён с предупреждениями' : 'Конфиг успешно применён',
              subtitle: 'Сервер принял config.yaml и запустил перезапуск unified.',

...
(() => {
  ...
  const FEATURE_META = {
    routing: {
      ...
      onActivate: (api, options) => {
        try { api.init(); } catch (e) {
          try { console.error('[UnifiedUI] routing config shell init failed', e); } catch (e2) {}
          return false;
        }
    ...
    inbounds: {
      ...
      onActivate: (api) => {
        try { api.init(); } catch (e) {
          try { console.error('[UnifiedUI] inbounds config shell init failed', e); } catch (e2) {}
          return false;
        }
    ...
    outbounds: {
      ...
      onActivate: (api, options) => {
        try { api.init(); } catch (e) {
          try { console.error('[UnifiedUI] outbounds config shell init failed', e); } catch (e2) {}
          return false;
        }
  ...
  async function ensureFeatureReady(featureName) {
    ...
    _featurePromises[key] = meta.load()
      ...
      .catch((error) => {
        try { console.error('[UnifiedUI] config shell failed to load feature', key, error); } catch (e) {}
        throw error;
      })
      .finally(() => {

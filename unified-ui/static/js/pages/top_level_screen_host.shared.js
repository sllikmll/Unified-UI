import { ensureXkeenRoot } from '../features/unified_runtime.js';
...
function buildPageConfigRegexSource() {
  return /var\s+pageConfig\s*=\s*(\{[\s\S]*?\});\s*window\.UnifiedUI\.pageConfig\s*=\s*pageConfig\s*;/;
}
...
function extractPageConfigFromDocument(doc) {
  ...
  for (const script of scripts) {
    ...
    if (!text.includes('window.UnifiedUI.pageConfig')) continue;
...
export function captureCurrentDocumentScreenSnapshot(name) {
  ...
  return {
    name: String(name || ''),
    root,
    pageConfig: cloneJson(getWindowRef()?.UnifiedUI?.pageConfig || null),
...
export async function fetchTopLevelScreenSnapshot(name, route) {
  const response = await fetch(String(route || ''), {
    ...
    credentials: 'same-origin',
      cache: 'no-store',
    headers: {
      'X-Requested-With': 'UnifiedUITopLevelScreen',
    },
  });
  if (!response.ok) {
...
export function applyScreenDocumentState(snapshot) {
  ...
  try {
    ...
    win.UnifiedUI.pageConfig = cloneJson(snapshot.pageConfig || null);
  } catch (error) {}

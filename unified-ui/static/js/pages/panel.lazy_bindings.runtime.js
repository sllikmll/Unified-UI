...
const panelFeatureSpecs = Object.freeze({
  ...
  unifiedTexts: {
    load: () => import('../features/unified_texts.js'),
...
export function ensurePanelLazyFeature(name) {
  ...
  if (spec) {
    ...
    panelFeatureEnsurePromises[key] = Promise.resolve()
      ...
      .catch((error) => {
        try { console.error('[UnifiedUI] panel lazy feature failed:', key, error); } catch (secondaryError) {}
...
export async function openPanelTerminal(mode) {
  ...
  const terminalRoot = (getXkeenTerminalRoot() || (window.UnifiedUI ? window.UnifiedUI.terminal : null) || null);
  safe(() => {
  ...
  attempts.push(async () => {
    const root = getXkeenTerminalRoot() || (window.UnifiedUI ? window.UnifiedUI.terminal : null) || null;
...
export function wirePanelLazyFeatureClicks() {
  ...
  document.addEventListener('click', (event) => {
    ...
    const serviceTrigger = raw.closest('#unified-start-btn, #unified-stop-btn, #unified-restart-btn, #unified-core-text');
    ...
    const unifiedAction = raw.closest('#port-proxying-save-btn, #port-exclude-save-btn, #ip-exclude-save-btn, #unified-config-save-btn');
    if (unifiedAction && !isPanelLazyFeatureReady('unifiedTexts')) {
      event.preventDefault();
      event.stopImmediatePropagation();
      ensurePanelLazyFeature('unifiedTexts').then((ready) => {
        if (!ready) return;
        fireDeferredClick(unifiedAction);
      });
      return;
    }

...
async function resolveFileManagerModuleApi() {
  try {
    ...
    try { console.error('[UnifiedUI] file manager module resolve failed', error); } catch (e) {}
  }
  return null;
}
...
export function applyPanelViewRuntime(name) {
  ...
  if (viewName === 'mihomo') {
    initViewOnce('mihomo', () => {
      ...
      try { console.error('[UnifiedUI] view init failed:', viewName, error); } catch (e) {}
    });
  }
  ...
  if (viewName === 'unified') {
    initViewOnce('unified', async () => {
      const ready = await ensurePanelLazyFeature('unifiedTexts');
      if (!ready) throw new Error('unified texts not ready');
    }).catch((error) => {
      try { console.error('[UnifiedUI] view init failed:', viewName, error); } catch (e) {}
    });
  }
  ...
  if (viewName === 'xray-logs') {
    ...
    if (logsShell) {
      Promise.resolve(activateLogsShellView({ reason: 'tab' })).catch((error) => {
        try { console.error('[UnifiedUI] logs shell activate failed', error); } catch (e) {}
      });
    }
  } else {
    const logsShell = getLogsShellApi();
  ...
  if (viewName === 'files') {
    ensureFileManagerReady().then(async (ready) => {
      ...
      try { console.error('[UnifiedUI] files view activation failed', error); } catch (e) {}
    });
  }
...
export function bindPanelShellViewRuntime(sharedShell) {
  ...
  try { initAuthPasswordControls(); } catch (error) { try { console.error('[UnifiedUI] auth password controls init failed', error); } catch (e) {} }

  document.addEventListener('unified:panel-view-changed', (event) => {

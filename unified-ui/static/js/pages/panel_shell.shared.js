...
(() => {
  ...
  function renderHeaderAsyncShell(snapshot) {
    ...
    setHeaderAsyncChipLoading(document.getElementById('unified-core-text'), loading.currentCore);
  ...
  const LIGHT_DONATE_HIDE_KEY = 'unified_ui_hide_donate';
  ...
  function getGlobalAutorestartCheckbox() {
    return document.getElementById('global-autorestart-unified');
  }
  ...
  function bindLightweightAutorestartCheckbox() {
    ...
    try {
      if (window.localStorage) {
        const stored = localStorage.getItem('unified_global_autorestart');
        if (stored === '1') cb.checked = true;
        else if (stored === '0') cb.checked = false;
      }
    } catch (e) {}

    if (cb.dataset && cb.dataset.unifiedBound === '1') return;

    cb.addEventListener('change', () => {
      try {
        if (!window.localStorage) return;
        localStorage.setItem('unified_global_autorestart', cb.checked ? '1' : '0');
      } catch (e) {}
    });

    if (cb.dataset) cb.dataset.unifiedBound = '1';
  }

  function setLightweightXkeenHeaderState(state, core) {
    const lamp = document.getElementById('unified-service-lamp');
    const textEl = document.getElementById('unified-service-text');
    const coreEl = document.getElementById('unified-core-text');
  ...
  const LIGHT_UNIFIED_STATUS_POLL_MS = 15000;
  ...
  async function refreshLightweightXkeenHeaderStatus(opts) {
    ...
    try {
      ...
      const http = getCoreHttp();
      if (http && typeof http.fetchJSON === 'function') {
        data = await http.fetchJSON('/api/unified/status', {
          method: 'GET',
          timeoutMs: 6000,
          retry: 1,
        }).catch(() => null);
      } else {
        const res = await fetch('/api/unified/status', { cache: 'no-store' });
        data = await res.json().catch(() => null);
        if (!res.ok) data = null;
      }
  ...
  const LAST_VIEW_KEY = 'unified.panel.last_view.v1';
  let _currentView = '';
  ...
  function getSections() {
    return {
      ...
      'protocol-subscription': document.getElementById('view-protocol-subscription'),
      'protocol-wireguard': document.getElementById('view-protocol-wireguard'),
      'protocol-amnezia': document.getElementById('view-protocol-amnezia'),
      'protocol-hysteria2': document.getElementById('view-protocol-hysteria2'),
      'protocol-vless': document.getElementById('view-protocol-vless'),
      'protocol-trojan': document.getElementById('view-protocol-trojan'),
      'protocol-vmess': document.getElementById('view-protocol-vmess'),
      'protocol-shadowsocks': document.getElementById('view-protocol-shadowsocks'),
      'protocol-mieru': document.getElementById('view-protocol-mieru'),
      'protocol-naiveproxy': document.getElementById('view-protocol-naiveproxy'),
      'protocol-telegram': document.getElementById('view-protocol-telegram'),
      ...
      unified: document.getElementById('view-unified'),
  ...
  function applyShellView(name) {
    ...
    try {
      const routingFocusGroup = document.querySelector('.unified-ctrl-group-routing-focus');
  ...
  function emitViewChanged(nextView, prevView) {
    try {
      document.dispatchEvent(new CustomEvent('unified:panel-view-changed', {
        detail: {
  ...
  function wireTabs() {
    ...
    buttons.forEach((btn) => {
      if (btn.dataset && btn.dataset.unifiedWiredTabs === '1') return;
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        showView(btn.dataset.view);
      });
      if (btn.dataset) btn.dataset.unifiedWiredTabs = '1';
    });

    const devtoolsBtn = document.getElementById('devtools-open-btn');
    if (devtoolsBtn && (!devtoolsBtn.dataset || devtoolsBtn.dataset.unifiedWiredTabs !== '1')) {
      devtoolsBtn.addEventListener('click', (e) => {
        e.preventDefault();
        showView('devtools');
      });
      if (devtoolsBtn.dataset) devtoolsBtn.dataset.unifiedWiredTabs = '1';
    }
  ...
  function toastPanelShell(message, kind) {
    ...
    try {
      if (kind === 'error') console.error('[UnifiedUI]', msg);
      else console.log('[UnifiedUI]', msg);
    } catch (error) {}
  }
  ...
  async function openRoutingDatContentsFallback(kind) {
    try {
      const lazy = window.UnifiedUI && window.UnifiedUI.runtime ? window.UnifiedUI.runtime.lazy : null;
    ...
    try {
      if (window.UnifiedUI && window.UnifiedUI.ui && window.UnifiedUI.ui.datContents && typeof window.UnifiedUI.ui.datContents.open === 'function') {
        window.UnifiedUI.ui.datContents.open(String(kind || 'geosite'));
        return true;
      }
    } catch (error) {}

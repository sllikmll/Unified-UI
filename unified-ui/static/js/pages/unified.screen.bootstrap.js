...
import { bootXkeenPage } from './unified.init.js';
import { getServiceStatusApi } from '../features/service_status.js';
import { getXkeenTextsApi } from '../features/unified_texts.js';
...
function emitXkeenEditorsReady() {
  try {
    document.dispatchEvent(new CustomEvent('unified-editors-ready'));
  } catch (error) {}
}
...
function createXkeenTopLevelApi(apis) {
  ...
  const unifiedTexts = apis && apis.unifiedTexts ? apis.unifiedTexts : null;
  ...
  return {
    activate(context) {
      ...
      if (unifiedTexts && typeof unifiedTexts.activate === 'function') {
        try { unifiedTexts.activate(context); } catch (error) {}
      }
    ...
    deactivate(context) {
      let state = null;
      if (unifiedTexts && typeof unifiedTexts.deactivate === 'function') {
        try { state = unifiedTexts.deactivate(context) || state; } catch (error) {}
      }
    ...
    serializeState(context) {
      return Object.assign(
        ...
        readScrollState() || {},
        {
          unifiedTexts: unifiedTexts && typeof unifiedTexts.serializeState === 'function'
            ? (unifiedTexts.serializeState(context) || null)
            : null,
    ...
    restoreState(state, context) {
      ...
      if (unifiedTexts && typeof unifiedTexts.restoreState === 'function' && next.unifiedTexts) {
        try { unifiedTexts.restoreState(next.unifiedTexts, context); } catch (error) {}
      }
...
let _unifiedTopLevelApi = null;

export async function bootXkeenScreen() {
  bootXkeenPage();

  if (!_unifiedTopLevelApi) {
    _unifiedTopLevelApi = Object.assign({
      serviceStatus: getServiceStatusApi(),
      unifiedTexts: getXkeenTextsApi(),
  ...
  return _unifiedTopLevelApi;
}

export function getXkeenTopLevelApi() {
  if (_unifiedTopLevelApi) return _unifiedTopLevelApi;
  _unifiedTopLevelApi = Object.assign({
    serviceStatus: getServiceStatusApi(),
    unifiedTexts: getXkeenTextsApi(),
  }, createXkeenTopLevelApi({

...
void bootTopLevelShell({
  ...
  onError(error) {
    console.error('[UnifiedUI] panel feature bundle bootstrap failed', error);
  },
}).then(() => {
  try { registerPanelMihomoTopLevelScreens(); } catch (error) {
    try { console.error('[UnifiedUI] panel/mihomo screen registration failed', error); } catch (secondaryError) {}
  }
});

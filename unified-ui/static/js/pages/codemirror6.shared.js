...
const IMPORTMAP_SELECTOR = 'script[type="importmap"][data-unified-cm6-importmap="1"]';
...
function ensureSkippedRuntime(reason) {
  ...
  const existing = win.UnifiedUI.ui.cm6Runtime;
...
async function ensureCodeMirror6Runtime() {
  ...
  try {
    ...
    try { console.error('[unified] failed to load CM6 runtime bridge', e); } catch (e2) {}
    return false;
  }
}

...
#
# Ручной запуск:
#   sh /opt/etc/unified-ui/scripts/update_unified_ui.sh
#
# Настройки через env:
#   UNIFIED_UI_UPDATE_REPO=sllikmll/Unified-UI
#   UNIFIED_UI_UPDATE_CHANNEL=stable   # stable | main
#   UNIFIED_UI_UPDATE_BRANCH=main      # used when channel=main
#   UNIFIED_UI_UPDATE_ASSET_NAME=unified-ui-routing.tar.gz
#   UNIFIED_UI_UPDATE_DIR=/opt/var/lib/unified-ui/update
#   UNIFIED_UI_BACKUP_DIR=/opt/var/backups/unified-ui
#   UNIFIED_UI_BACKUP_KEEP=1
#   UNIFIED_UI_UPDATE_SKIP_BACKUP=0     # 1: run update without pre-update backup
#
# Важно:
# - Скрипт пишет лог и status.json в update-dir.
# - Делает backup текущего /opt/etc/unified-ui перед установкой.
...
UI_INIT_SCRIPT_LEGACY="/opt/etc/init.d/S99unified-ui"
UI_INIT_SCRIPT="${UNIFIED_UI_INIT_SCRIPT:-$UI_INIT_SCRIPT_DEFAULT}"
...
REPO="${UNIFIED_UI_UPDATE_REPO:-sllikmll/Unified-UI}"
CHANNEL="${UNIFIED_UI_UPDATE_CHANNEL:-stable}"
ASSET_NAME="${UNIFIED_UI_UPDATE_ASSET_NAME:-}"
ACTION="${UNIFIED_UI_UPDATE_ACTION:-update}"
API_BASE="${UNIFIED_UI_GITHUB_API_BASE:-https://api.github.com}"
ASSET_URL_OVERRIDE="${UNIFIED_UI_UPDATE_ASSET_URL:-}"
TAG_OVERRIDE="${UNIFIED_UI_UPDATE_TAG:-}"
SHA_URL_OVERRIDE="${UNIFIED_UI_UPDATE_SHA_URL:-}"
SHA_KIND_OVERRIDE="${UNIFIED_UI_UPDATE_SHA_KIND:-}"
SKIP_BACKUP="${UNIFIED_UI_UPDATE_SKIP_BACKUP:-0}"
...
ALLOW_HOSTS_RAW="${UNIFIED_UI_UPDATE_ALLOW_HOSTS:-github.com,release-assets.githubusercontent.com,objects.githubusercontent.com,codeload.github.com}"
ALLOW_HTTP="${UNIFIED_UI_UPDATE_ALLOW_HTTP:-0}"

# Timeouts
CONNECT_TIMEOUT="${UNIFIED_UI_UPDATE_CONNECT_TIMEOUT:-10}"
DOWNLOAD_TIMEOUT="${UNIFIED_UI_UPDATE_DOWNLOAD_TIMEOUT:-300}"
API_TIMEOUT="${UNIFIED_UI_UPDATE_API_TIMEOUT:-10}"

# Size limits (bytes)
MAX_BYTES="${UNIFIED_UI_UPDATE_MAX_BYTES:-62914560}"            # 60 MiB
MAX_CHECKSUM_BYTES="${UNIFIED_UI_UPDATE_MAX_CHECKSUM_BYTES:-1048576}"  # 1 MiB

# SHA policy
SHA_STRICT="${UNIFIED_UI_UPDATE_SHA_STRICT:-1}"     # 1: fail if checksum file exists but no matching entry
REQUIRE_SHA="${UNIFIED_UI_UPDATE_REQUIRE_SHA:-1}"   # 1: require checksum for stable channel
...
RUNNER_PID="$$"
export UNIFIED_UI_UPDATE_RUNNER_PID="$RUNNER_PID"
...
pick_update_dir() {
  if [ -n "${UNIFIED_UI_UPDATE_DIR:-}" ]; then
    echo "$UNIFIED_UI_UPDATE_DIR"
    return
  fi

  # приоритет: /opt/var/lib/... (роутер) -> /opt/var/log/... -> UI_DIR fallback -> /tmp
  for d in \
    "/opt/var/lib/unified-ui/update" \
    "/opt/var/log/unified-ui/update" \
    "$UI_DIR/var/lib/unified-ui/update" \
    "/tmp/unified-ui-update"; do
  ...
  echo "/tmp/unified-ui-update"
}

pick_backup_dir() {
  if [ -n "${UNIFIED_UI_BACKUP_DIR:-}" ]; then
    echo "$UNIFIED_UI_BACKUP_DIR"
    return
  fi

  for d in \
    "/opt/var/backups/unified-ui" \
    "$UI_DIR/var/backups/unified-ui" \
    "/tmp/unified-ui-backups"; do
    if ensure_dir "$d"; then
      echo "$d"
      return
    fi
  done
  echo "/tmp/unified-ui-backups"
}
...
runner_pid_raw = os.environ.get("UNIFIED_UI_UPDATE_RUNNER_PID") or ""
try:
...
base = {
  ...
  "error": None,
  "pid": runner_pid,
  "op": os.environ.get("UNIFIED_UI_UPDATE_ACTION") or "update",
...
acquire_lock() {
  ...
  if [ "${UNIFIED_UI_LOCK_PRECREATED:-}" = "1" ]; then
    "$PY" - "$LOCK_FILE" <<'PY'
import json, os, sys, time
p = sys.argv[1]
...
# Start request
req = urllib.request.Request(url, headers={"User-Agent": "unified-ui-updater"})
start = time.time()
last_emit = 0.0
last_bytes = 0
last_t = start
...
PY
  ...
  if [ "$downloaded_by_python" != "1" ]; then
    ...
    if command -v curl >/dev/null 2>&1; then
      ...
      curl -fsS -L --connect-timeout "$CONNECT_TIMEOUT" --max-time "$DOWNLOAD_TIMEOUT" -A "unified-ui-updater" -o "$out" "$url" >>"$LOG_FILE" 2>&1 || dl_rc=$?
...
trim_backups() {
  ...
  i=0
  for f in $(ls -1t "$BACKUP_DIR"/unified-ui-*.tgz 2>/dev/null || true); do
    i=$((i+1))
    if [ "$i" -gt "$keep" ]; then
...
if [ "$ACTION" = "rollback" ]; then
  ...
  rb_file="${UNIFIED_UI_ROLLBACK_FILE:-}"
  backup_pick=""
  if [ -n "$rb_file" ] && [ -f "$rb_file" ]; then
    backup_pick="$rb_file"
  else
    backup_pick="$(ls -1t "$BACKUP_DIR"/unified-ui-*.tgz 2>/dev/null | head -n 1 || true)"
  fi
  ...
  if [ -n "$cur_mv" ] && [ -d "$cur_mv" ]; then
    keep_cur="${UNIFIED_UI_ROLLBACK_KEEP_CURRENT:-0}"
    if [ "$keep_cur" = "1" ]; then
...
ts="$(date -u +%Y%m%d-%H%M%S 2>/dev/null || date +%Y%m%d-%H%M%S)"
backup_file="$BACKUP_DIR/unified-ui-$ts.tgz"
...
COMMIT=""
COMMIT_AT=""
BRANCH="${UNIFIED_UI_UPDATE_BRANCH:-}"
...
headers = {"User-Agent": "unified-ui-updater", "Accept": "application/vnd.github+json"}
...
PYMAIN
  ...
  ASSET_NAME="unified-ui-${BRANCH}-${SHORT_SHA}.tar.gz"
...
if [ -n "$ASSET_NAME" ]; then
  ...
  if [ "$ASSET_NAME" = "unified-ui.tar.gz" ]; then
    ASSET_CANDIDATES="$ASSET_NAME unified-ui-routing.tar.gz"
  elif [ "$ASSET_NAME" = "unified-ui-routing.tar.gz" ]; then
    ASSET_CANDIDATES="$ASSET_NAME unified-ui.tar.gz"
  else
    ASSET_CANDIDATES="$ASSET_NAME unified-ui-routing.tar.gz unified-ui.tar.gz"
  fi
else
  # default project convention first
  ASSET_CANDIDATES="unified-ui-routing.tar.gz unified-ui.tar.gz"
fi
...
req = urllib.request.Request(url, headers={"User-Agent": "unified-ui-updater", "Accept": "application/vnd.github+json"})
try:
    with urllib.request.urlopen(req, timeout=timeout) as r:
...
ua = {"User-Agent": "unified-ui-updater"}
...
if [ "$CHANNEL" = "stable" ] && [ "$REQUIRE_SHA" = "1" ] && [ -z "${SHA_URL:-}" ]; then
  log "[!] Checksum is required (UNIFIED_UI_UPDATE_REQUIRE_SHA=1), but sha file was not found in release assets."
...
if command -v mktemp >/dev/null 2>&1; then
  WORK_DIR="$(mktemp -d /tmp/unified-ui-update.XXXXXX 2>/dev/null || true)"
fi
if [ -z "$WORK_DIR" ]; then
  WORK_DIR="/tmp/unified-ui-update-$$"
...
(
  cd "$INSTALL_DIR"
  UNIFIED_UI_UPDATE_REPO="$REPO" \
  UNIFIED_UI_UPDATE_CHANNEL="$CHANNEL" \
  UNIFIED_UI_UPDATE_BRANCH="${BRANCH:-}" \
  UNIFIED_UI_VERSION="${TAG:-}" \
  UNIFIED_UI_COMMIT="${COMMIT:-}" \
  sh "$INSTALL_SH"
) >>"$LOG_FILE" 2>&1 || {

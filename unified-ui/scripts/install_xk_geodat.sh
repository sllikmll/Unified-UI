...
# Where to install the binary. Backend/UI may override this via UNIFIED_GEODAT_BIN.
DEST_DEFAULT="/opt/etc/unified-ui/bin/xk-geodat"
DEST="${UNIFIED_GEODAT_BIN:-$DEST_DEFAULT}"
...
#   UNIFIED_GEODAT_INSTALL=1  -> install without asking
#   UNIFIED_GEODAT_INSTALL=0  -> skip
#   (unset) -> ask if TTY, else try install (non-blocking)
INSTALL="${UNIFIED_GEODAT_INSTALL:-}"
...
# Optional overrides:
#   UNIFIED_GEODAT_ASSET          -> override asset name
#   UNIFIED_GEODAT_TAG            -> download from specific tag
#   UNIFIED_GEODAT_URL            -> full URL to binary
#   UNIFIED_GEODAT_LOCAL          -> install from existing local file
#   UNIFIED_GEODAT_SHA256SUMS_URL -> override URL to SHA256SUMS
if [ -n "${UNIFIED_GEODAT_ASSET:-}" ]; then
  ASSET="$UNIFIED_GEODAT_ASSET"
fi

# -------------------- Install from local file --------------------

if [ -n "${UNIFIED_GEODAT_LOCAL:-}" ] && [ -f "$UNIFIED_GEODAT_LOCAL" ]; then
  echo "xk-geodat: installing from local file $UNIFIED_GEODAT_LOCAL"
  backup_existing

  cp "$UNIFIED_GEODAT_LOCAL" "$DEST" || { echo "xk-geodat: copy failed — пропуск"; restore_backup; exit 0; }
...
BASE=""
if [ -n "${UNIFIED_GEODAT_URL:-}" ]; then
  URL="$UNIFIED_GEODAT_URL"
elif [ -n "${UNIFIED_GEODAT_TAG:-}" ]; then
  BASE="https://github.com/$REPO/releases/download/$UNIFIED_GEODAT_TAG/"
  URL="${BASE}${ASSET}"
else
...
if [ -n "${UNIFIED_GEODAT_SHA256SUMS_URL:-}" ]; then
  SUMS_URL="$UNIFIED_GEODAT_SHA256SUMS_URL"
elif [ -n "$BASE" ]; then

...
def _build_happ_headers() -> tuple[dict[str, str], str]:
    ...
    hwid = str(os.environ.get("UNIFIED_HAPP_HELPER_HWID") or info.get("hwid") or "").strip()
    if not hwid:
        raise RuntimeError("missing_hwid")
    ua = str(os.environ.get("UNIFIED_SUBSCRIPTION_HAPP_USER_AGENT") or DEFAULT_HAPP_UA).strip()
    headers = {
        "User-Agent": ua,
        "X-HWID": hwid,
...
def main(argv: list[str]) -> int:
    ...
    timeout_raw = str(os.environ.get("UNIFIED_HAPP_HELPER_TIMEOUT") or "").strip()
    try:

...
MIHOMO_REPO="${UNIFIED_MIHOMO_REPO:-sllikmll/mihomo}"
MIHOMO_FALLBACK_REPO="${UNIFIED_MIHOMO_FALLBACK_REPO:-MetaCubeX/mihomo}"
MIHOMO_TAG="${UNIFIED_MIHOMO_TAG:-latest}"
MIHOMO_BIN="${UNIFIED_MIHOMO_BIN:-/opt/sbin/mihomo}"
MIHOMO_ROOT="${MIHOMO_ROOT:-/opt/etc/mihomo}"
MIHOMO_FORCE="${UNIFIED_INSTALL_MIHOMO_FORCE:-0}"
MIHOMO_START="${UNIFIED_INSTALL_MIHOMO_START:-1}"
MIHOMO_CONTROLLER="${UNIFIED_MIHOMO_CONTROLLER:-0.0.0.0:9090}"
MIHOMO_TEST_URL="${UNIFIED_MIHOMO_TEST_URL:-https://www.gstatic.com/generate_204}"
...
req = urllib.request.Request(url, headers={"User-Agent": "unified-ui-mihomo-installer"})
...
resolve_asset_url() {
  if [ -n "${UNIFIED_MIHOMO_ASSET_URL:-}" ]; then
    echo "$UNIFIED_MIHOMO_ASSET_URL"
    return 0
  fi

  regex="${UNIFIED_MIHOMO_ASSET_REGEX:-$(arch_asset_regex)}"
  if [ -z "$regex" ]; then
    warn "Не знаю asset Mihomo для архитектуры $(uname -m 2>/dev/null || echo unknown)."
    warn "Задай UNIFIED_MIHOMO_ASSET_URL вручную."
    return 1
  fi
...
def pick(repo_name: str) -> str:
    ...
    req = urllib.request.Request(api, headers={"User-Agent": "unified-ui-mihomo-installer", "Accept": "application/vnd.github+json"})
...
write_ui_env_defaults() {
  env_file="/opt/etc/unified-ui/devtools.env"
...
entries = {
    ...
    "UNIFIED_RESTART_CMD": f"{root}/restart-mihomo.sh",
    "MIHOMO_RESTART_CMD": f"{root}/restart-mihomo.sh",
    "MIHOMO_RESTART_TIMEOUT": "90",
    "UNIFIED_MIHOMO_REPO": os.environ.get("UNIFIED_MIHOMO_REPO", "sllikmll/mihomo"),
    "UNIFIED_MIHOMO_FALLBACK_REPO": os.environ.get("UNIFIED_MIHOMO_FALLBACK_REPO", "MetaCubeX/mihomo"),
}
try:
...
start_and_verify() {
  ...
  if [ "$MIHOMO_START" != "1" ]; then
    log "UNIFIED_INSTALL_MIHOMO_START=$MIHOMO_START — Mihomo не запускаю."
    return 0
  fi

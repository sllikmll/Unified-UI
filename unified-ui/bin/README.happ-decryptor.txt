...
Контракт команды:
- UnifiedUI передаёт входящую ссылку последним аргументом или заменяет `%LINK%`.
...
```sh
UNIFIED_HAPP_DECRYPTOR_REMOTE_URL='https://example.com/api/decrypt'
```
...
```sh
UNIFIED_HAPP_DECRYPTOR_REMOTE_URL='https://happy-decoder.cc/p/%LINK_ENCODED%'
```
...
Репозиторий UnifiedUI не хранит готовый Happ decryptor и ключевой материал. Для
...
```text
unified-ui/bin/happ-decrypt-universal
unified-ui/bin/happ-decrypt-universal.assets/
```
...
```sh
UNIFIED_HAPP_DECRYPTOR_CMD='/opt/bin/node /opt/etc/unified-ui/bin/happ-decrypt-universal %LINK%'
```

Если `node` уже есть в `PATH` процесса UI и launcher сохраняет auto-detect имя
`happ-decrypt-universal`, задавать `UNIFIED_HAPP_DECRYPTOR_CMD` необязательно.
...
```text
/opt/etc/unified-ui/bin/happ-decrypt-universal
/opt/etc/unified-ui/bin/happ-decrypt-universal.assets/
```
...
```sh
mkdir -p /opt/etc/unified-ui/bin
mv /tmp/happ-decrypt-universal /opt/etc/unified-ui/bin/happ-decrypt-universal
rm -rf /opt/etc/unified-ui/bin/happ-decrypt-universal.assets
mv /tmp/happ-decrypt-universal.assets /opt/etc/unified-ui/bin/happ-decrypt-universal.assets
chmod 755 /opt/etc/unified-ui/bin/happ-decrypt-universal
```

Быстрая проверка на роутере:

```sh
/opt/bin/node /opt/etc/unified-ui/bin/happ-decrypt-universal
```
...
UnifiedUI должен найти их автоматически. Для явной настройки через DevTools -> ENV
укажите:

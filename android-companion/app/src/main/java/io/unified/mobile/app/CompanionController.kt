...
internal class CompanionController(
    ...
    private suspend fun performServiceAction(action: ServiceAction) {
        ...
        val pendingMessage = when (action) {
            ServiceAction.Start -> "Запускаем unified и ждём подтверждение сервера…"
            ServiceAction.Stop -> "Останавливаем unified и ждём подтверждение сервера…"
            ServiceAction.Restart -> "Перезапускаем unified и ждём подтверждение сервера…"
        }
        state = state.copy(
            serviceOperation = ServiceOperationState(
                phase = ServiceOperationPhase.Pending,
        ...
        state = state.copy(
            routing = state.routing.copy(
                write = RoutingWriteState(
                    ...
                    message = "Применяем сохранённую revision и ждём restart unified…",
                ),
            ),
        )
        try {
            ...
            state = state.copy(
                routing = state.routing.copy(
                    ...
                    write = RoutingWriteState(
                        phase = RoutingWritePhase.Success,
                        message = "Routing применён; restart unified подтверждён сервером.",
                    ),
                ),
                dashboard = state.dashboard.copy(
                    lastOperation = result.lastOperation,

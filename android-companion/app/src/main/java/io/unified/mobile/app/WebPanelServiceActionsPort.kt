...
internal class WebPanelServiceActionsPort(
    ...
    override suspend fun switchCore(baseUrl: String, core: String): CoreSwitchResult {
        ...
        return CoreSwitchResult(
            ...
            eventSubtitle = "${snapshot.activeCore} активно после подтвержденного перезапуска unified",
            logMessage = "Сервер подтвердил ядро ${snapshot.activeCore} и работающий unified",
        )
    }
        ...
        val response = transport.post(
            CompanionHttpRequest(
                ...
                endpoint = when (action) {
                    ServiceAction.Start -> "/api/unified/start"
                    ServiceAction.Stop -> "/api/unified/stop"
...
internal fun parseServiceRuntimeStatus(body: String): ServiceRuntimeStatus {
    ...
    if (!payload.optBoolean("ok", false)) {
        throw ServiceActionException("Сервер не подтвердил состояние сервиса unified.")
    }
    val status = payload.optString("status").trim().lowercase()
    if (!payload.has("running") && status !in setOf("running", "stopped")) {
        throw ServiceActionException("В ответе сервера отсутствует состояние сервиса unified.")
    }

...
internal fun parseOutboundLinkSnapshot(body: String): OutboundLinkSnapshot {
    ...
    return OutboundLinkSnapshot(
        ...
        managedKind = when {
            Regex("Generated\\s+by\\s+UnifiedUI\\s+UI\\s+subscription", RegexOption.IGNORE_CASE).containsMatchIn(text) ->
                "subscription"
            Regex("Generated\\s+by\\s+UnifiedUI\\s+UI\\s*\\(outbounds\\s+pool\\)", RegexOption.IGNORE_CASE).containsMatchIn(text) ->
                "pool"
            else -> null
        },
    )

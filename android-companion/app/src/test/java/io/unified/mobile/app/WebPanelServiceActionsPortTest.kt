...
class WebPanelServiceActionsPortTest {
    ...
    fun stopPostsCommandThenReturnsFreshServerStatusAndCore() = runTest {
        val transport = ServiceFakeTransport(
            responses = mapOf(
                RequestKey("POST", "/api/unified/stop") to jsonResponse("""{"ok":true}"""),
                RequestKey("GET", "/api/unified/status") to jsonResponse(
                    """{"ok":true,"running":false,"status":"stopped","core":null}""",
                ),
                RequestKey("GET", "/api/unified/core") to jsonResponse(
        ...
        assertEquals(
            listOf(
                RequestKey("POST", "/api/unified/stop"),
                RequestKey("GET", "/api/unified/status"),
                RequestKey("GET", "/api/unified/core"),
            ),
    ...
    fun coreSwitchSendsCanonicalPayloadAndRequiresMatchingRuntimeCore() = runTest {
        val transport = successfulActionTransport("/api/unified/core", runtimeCore = "mihomo")
    ...
    fun coreSwitchWaitsForXrayToLeaveTransientStoppedState() = runTest {
        val transport = ServiceFakeTransport(
            responses = mapOf(
                RequestKey("POST", "/api/unified/core") to jsonResponse(
            ...
            responseSequences = mapOf(
                RequestKey("GET", "/api/unified/status") to listOf(
                    jsonResponse(
        ...
        assertEquals(
            2,
            transport.requestKeys.count { it == RequestKey("GET", "/api/unified/status") },
        )
    }
    ...
    fun acceptedCommandWithMismatchedConfirmedStateFails() = runTest {
        val transport = ServiceFakeTransport(
            responses = mapOf(
                RequestKey("POST", "/api/unified/start") to jsonResponse("""{"ok":true}"""),
                RequestKey("GET", "/api/unified/status") to jsonResponse(
                    """{"ok":true,"running":false,"status":"stopped","core":null}""",
                ),
                RequestKey("GET", "/api/unified/core") to jsonResponse(

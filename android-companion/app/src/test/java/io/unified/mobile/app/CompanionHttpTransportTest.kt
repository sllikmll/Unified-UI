...
class CompanionHttpTransportTest {
    ...
    fun normalizesBaseUrlAndKeepsAnOptionalBasePath() {
        val endpoint = resolveCompanionEndpoint(
            baseUrl = " HTTPS://lab.lan:8443/unified/ ",
            endpoint = "/api/routing/fragments",
        )

        assertEquals("https://lab.lan:8443/unified/api/routing/fragments", endpoint.toString())
        assertEquals("https://lab.lan:8443/unified", normalizeCompanionBaseUrl("https://lab.lan:8443/unified/").toString())
    }

    @Test
    fun rejectsUnsafeOrIncompleteBaseAndEndpointUrls() {
        val missingScheme = assertTransportFailure {
            resolveCompanionEndpoint("lab.lan:8443", "/api/unified/core")
        }
        val queryInBase = assertTransportFailure {
            resolveCompanionEndpoint("https://lab.lan?next=/login", "/api/unified/core")
        }
        val escapingEndpoint = assertTransportFailure {
            resolveCompanionEndpoint("https://lab.lan/unified", "../api/unified/core")
        }

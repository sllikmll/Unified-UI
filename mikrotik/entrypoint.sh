...
def group(name, *, extra_direct=True):
    g = {'name': name, 'type': 'select'}
    if use:
        g['use'] = use
    proxies = ['DIRECT']
    if name not in ('Маршрутизация', 'Остальное'):
        proxies.append('Маршрутизация')
    if name == 'Остальное':
...
cfg = {
    'mixed-port': int(os.environ.get('MIHOMO_MIXED_PORT', '1080')),
    'allow-lan': True,
    'bind-address': '*',
    'mode': 'rule',
    ...
    'rule-providers': {
        'manual-proxy': {
            ...
            'behavior': 'classical',
            'format': 'yaml',
            'path': '/etc/mihomo/rules/manual-proxy.yaml',
        }
    },
    'proxy-groups': [
        group('Маршрутизация'),
    ...
    'rules': [
        ...
        # Keep first boot independent from external GeoIP/MMDB downloads.
        # DAT/GeoIP can be installed/updated later from the panel.
...
if sub:
    cfg['proxy-providers']['subscription_1'] = {
        'type': 'http',
        'url': sub,
        'interval': 3600,
